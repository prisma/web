...
Written 2026-09-10 for an agent with no prior context. Repos: `prisma/create-prisma` (the scaffolder) and `prisma/prisma-cli` (the unified `prisma` binary, whose `init` and `orm init` commands are involved). A read-only clone of create-prisma is at `wip/create-prisma-src` in this worktree.
...
3. `prisma orm init` on the existing-project path asks the same question when interactive, and honours `--skills` for parity with `--skip-skills`.
...
- `prisma orm init --skills none` on an existing project behaves the same.
...
- `prisma orm init --skip-skills` skips the skill install on the existing-project path.
...
- Docs pages describing current behaviour: `wip/web/apps/docs/content/docs/cli/init.mdx`, `cli/orm-init.mdx`, `cli/skills.mdx`, `cli/configuration.mdx`
