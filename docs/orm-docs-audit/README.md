...
Citations in `audit/`, `changes.md`, and the briefs use the worktree layout the audit was run in: `wip/prisma-src/` is prisma/orm (formerly prisma/prisma; the old name still redirects) at `v8.0.0-rc.9`, `wip/create-prisma-src/` is prisma/create-prisma, `wip/web/` is this repository, and `wip/docs-audit/` is this directory. Strip the prefix to get the path in the named repository.
