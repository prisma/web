...
To **bootstrap a new Prisma ORM project**, run the following command in your terminal:

```npm
npx prisma init
```

Then create a Prisma Postgres database and update your `.env` file with the `postgres://...` connection string from the CLI output:

```npm
npx create-db
```

Follow [this page](/v6/prisma-postgres/from-the-cli) to complete the setup for your first Prisma Postgres project.
