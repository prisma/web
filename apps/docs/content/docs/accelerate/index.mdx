...
Prisma ORM supports connection pooling and query caching through Accelerate, so you don't need a separate pooler or caching layer. Accelerate is included with [Prisma Postgres](/postgres) and also available as a standalone add-on for external databases.
