...
| [Prisma 8](/orm) | [prisma/prisma](https://github.com/prisma/prisma) (`skills/`) | `npx skills add prisma/prisma/skills` |
...
If you use the Prisma 8 CLI, [`prisma agent install`](/cli/agent) wraps the same registry: with no flags it installs every `prisma/skills` skill for Claude Code and Codex, and `agent status` shows what is installed.
...
[Prisma 8](/orm) ships its own skills from the [prisma/prisma](https://github.com/prisma/prisma) repository, versioned with the product. Projects scaffolded with `create-prisma` or [`orm init`](/cli/init) install them automatically for every agent runtime the installer supports (Claude Code, Cursor, Codex, Windsurf); skip that step with `orm init --skip-skills`. Add them to an existing project with:
