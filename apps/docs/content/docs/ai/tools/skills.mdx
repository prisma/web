...
| [Prisma ORM](/orm) | [prisma/prisma](https://github.com/prisma/orm) (`skills/`) | `npx skills add prisma/prisma/skills` |
...
The Prisma ORM CLI's [`skills` commands](/cli/skills) do not install from this registry. They sync the skills that ship inside the Prisma ORM packages a project installs, so use `npx skills add prisma/skills` directly for the v7 skills above.
...
Helps you move a MongoDB project from Prisma ORM 6 to [Prisma ORM 8](/orm).
...
## Available skills for Prisma ORM [#available-skills-for-prisma-8]
