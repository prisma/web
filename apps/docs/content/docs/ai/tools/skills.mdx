...
[Prisma 8](/orm/v8) ships its own skills, versioned with the product. Projects scaffolded with `create-prisma` or [`orm init`](/cli/v8/init) install them automatically; add them to an existing project with:
