...
2. From the project directory, define a small schema in `prisma/schema.prisma`, then run `npx prisma@prev migrate dev --name init` and `npx prisma@prev generate`. If migrate dev asks to reset the database, stop and ask me first.
