...
1. Scaffold: `npx create-prisma@latest create my-app --template hono --provider postgres --yes`. Then run `npx prisma@next init` in `my-app` so the Prisma agent skills are installed and stay current, and use them.
