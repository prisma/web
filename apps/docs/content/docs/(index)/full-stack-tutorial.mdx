...
Answer the prompts for contract authoring style and package manager, or pass `--authoring psl|typescript` and `--package-manager npm|pnpm|yarn|bun` to skip them (Deno is supported for local minimal PostgreSQL projects only, and this tutorial deploys to Prisma Compute, which Deno projects cannot do yet). Then enter the project:
