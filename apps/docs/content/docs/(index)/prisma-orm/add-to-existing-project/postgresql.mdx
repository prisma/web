...
`db sign` also points the ref named `db` at the contract you signed. That is the starting point [migration plan](/cli/migration-plan) uses, so your first migration will contain only what you change from here, not the tables you already have.
