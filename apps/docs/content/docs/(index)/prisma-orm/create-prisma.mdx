...
The prompts cover the project name, the app template, the database provider, the contract authoring style, the package manager, and whether to deploy right away. Prisma writes skill files for coding agents into your repo. To remove them, set the [`skills.agents`](/cli/configuration#agent-skills) config field to `[]` and run [`skills sync`](/cli/skills). Generated Node.js projects expect Node.js 22.18 or newer (on the 24 line, 24.11 or newer); Node.js 24 is recommended.
