...
Setup gives you the app template, a starter contract, `prisma-8.md`, project-level Prisma ORM skills for your coding agent, and package scripts for the database steps below. To remove the agent skill files, set the [`skills.agents`](/cli/configuration#agent-skills) config field to `[]` and run [`skills sync`](/cli/skills). Sample users are seeded automatically the first time the app queries the database, so there is no separate seed step.
