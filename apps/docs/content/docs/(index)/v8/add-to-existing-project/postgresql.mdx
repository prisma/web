...
This guide shows how to add Prisma 8 to a project that already uses PostgreSQL. You will run `orm init`, infer a contract from the live schema, sign the database, and run a couple of queries.
...
```npm
npx @prisma/cli@next orm init --target postgres
```
