...
Create a Postgres client, connect it to get a runtime, then pass that runtime to `orm(...)`. Model accessors hang off `.public` and use the contract's **root names**, which match your PSL model names (`orm.User`, `orm.Post`).
...
- `sum()` and `avg()` require a field with the numeric codec trait. Over a to-many relation with no rows, they resolve to `null`, not `0`.
...
const updated = await db.Post.where({ id: postId })
  ...
  .update({
    tags: (tag) => tag.disconnect([{ id: ormTagId }]),
  });
...
- Exactly ten factories exist: `of`, `eq`, `neq`, `gt`, `lt`, `gte`, `lte`, `in`, `nin`, `isNull`, `isNotNull`. The inequality factory is `neq`, **not** `ne` (`MongoFieldFilter.ne` is `undefined`).
