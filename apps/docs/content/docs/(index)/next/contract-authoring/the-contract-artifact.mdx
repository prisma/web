...
url: /next/contract-authoring/the-contract-artifact
...
Both files are generated. Do not edit them; change the [PSL](/next/contract-authoring/psl-syntax) or [TypeScript](/next/contract-authoring/typescript-schema-builder) source and re-run `contract emit`. The files carry a generated-file notice that says exactly this.
...
- **`capabilities`** and **`extensionPacks`**: the database and extension features available to this contract, merged from the target, adapter, and extension packs at emit time. Query APIs consult these before using gated features; see [Capabilities](/next/contract-authoring/capabilities).
...
- Learn how the contract's [capabilities](/next/contract-authoring/capabilities) gate database features.
