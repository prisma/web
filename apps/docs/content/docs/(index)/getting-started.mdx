...
`app deploy` is part of the earlier beta CLI (`@prisma/cli@latest`). The unified CLI (`@prisma/cli@next`) that the [Compute docs](/compute) describe deploys through a git push, the Console, or [Prisma Composer](/composer) instead; both CLIs talk to the same platform. The [deploy quickstart](/prisma-compute/deploy) covers the git-push path.
