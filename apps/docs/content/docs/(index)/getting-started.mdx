...
Starting a new project? [Prisma Next](/next) is the recommended path for new apps. It is the next major version of Prisma ORM, available now in Early Access. The [getting started page](/) walks the full stack path.
...
1. Scaffold a new app with `npx create-prisma@latest` (Prisma 7 templates: next, hono, nuxt, astro, nest, svelte, tanstack-start, elysia, turborepo) and pick PostgreSQL. Or add Prisma 7 to an existing app with `npx prisma@latest init --db`, which provisions a Prisma Postgres database.
2. Define a small schema in `prisma/schema.prisma`, then run `npx prisma migrate dev --name init` and `npx prisma generate`.
