...
2. Sign in with `npx prisma@next auth login`.
3. Build your server bundle, then run `npx prisma@next deploy module.ts` with `DATABASE_URL` exported; the first deploy copies input values from your shell up to the deployment. See [Deploying](/composer/deploying).
