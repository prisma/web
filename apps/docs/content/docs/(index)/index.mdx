...
<HeroGrid>
  <HeroPitch
    badge="Recommended"
    primaryHref="/next/quickstart/postgresql"
    primaryLabel="Start the quickstart"
    secondaryHref="/prisma-compute/deploy"
    secondaryLabel="Deploy your first app"
  >
    Build and ship with the complete Prisma stack: [Prisma Next](/next) to model and query your data, [Prisma Postgres](/postgres) to store it, and [Prisma Compute](/compute) to run your app next to it. One scaffold command wires up all three, for you and your coding agent.
  </HeroPitch>
  <StackDiagram caption="One project. Type-safe, end to end.">
    ...
    <StackLayer title="Prisma Next" href="/next/quickstart/postgresql" icon={<Sparkles />}>
      Model and query. The next major version of Prisma ORM, in Early Access.
    </StackLayer>
...
# pick a framework template and Prisma Postgres at the prompts, then:
cd my-app
npm run db:init
npm run db:seed
npm run dev
