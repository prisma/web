...
Prisma is a complete TypeScript stack with one workflow: **build** your app and run all of it on your machine, then **deploy** it to the Prisma platform with one command. Jump straight to the [full-stack tutorial](/full-stack-tutorial), [Prisma 8](/getting-started) or [Prisma 7](/v7/getting-started) docs.
...
1. Run `npx prisma@latest orm init`. It writes `prisma.config.ts`, a starter contract and `db.ts` under `src/prisma/`, and installs Prisma 8 skills for you.
...
3. Evolve the starter contract under `src/prisma/` into my schema, then run `npm run contract:emit`, `npx prisma@latest migration plan`, and `npx prisma@latest migrate --yes`.
