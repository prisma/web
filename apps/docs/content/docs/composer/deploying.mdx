...
| You want to... | Run |
| --- | --- |
| Deploy to production | `npx prisma deploy module.ts` |
| Deploy an isolated environment | `npx prisma deploy module.ts --stage <name>` |
| Deploy under a different app name | `npx prisma deploy module.ts --name demo-42` |
...
A fresh checkout with those two variables set deploys successfully once the app's project exists. The very first deploy creates that project, and a new project needs a region: set it in the deploy config or in the environment, as described in the [`deploy` reference](/cli/deploy). Once the project exists it keeps its region, and later deploys, stages included, inherit it. The CLI finds or creates everything else. Keep the values out of the repo (an `.env` you source at deploy time, or CI secrets). When `PRISMA_SERVICE_TOKEN` is set it takes priority over any stored session.
...
```npm
npx prisma deploy module.ts --stage "pr-$PR_NUMBER"    # on push
```
...
This happens when your app, or another dependency of it, pins a different `effect` than `@prisma/composer` does and your package manager hoists that copy where the deploy engine resolves it. A plain Composer app never hits it: `@prisma/composer` and `@prisma/composer-prisma-cloud` pin every `effect`-family package the engine would otherwise float, so a fresh install resolves a single `effect`. The fix is to use the same `effect` as Composer: match your own `effect` dependency to the exact version the error names (`dependencies.effect` in `@prisma/composer`), or, when a dependency you cannot change pins another version, force Composer's with an `overrides` entry for `effect` in your `package.json` (`resolutions` in Yarn, under `"pnpm"` for pnpm) and reinstall. The pin goes stale when you upgrade `@prisma/composer`; the error names the new version each time.
...
- **The `cron` scheduler stays awake on purpose.** It holds Compute's keep-awake guard for its whole lifetime, so it never scales to zero: one warm instance per app is the cost of the clock. Every other service, the runner included, sleeps as usual.
