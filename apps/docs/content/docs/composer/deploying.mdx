...
A fresh checkout with those two variables set deploys successfully, with one addition on the very first deploy: the CLI creates the app's project then, and a new project needs a region. Give it one as `prismaCloud({ region: 'us-east-1' })` in the deploy config or as a `PRISMA_REGION` environment variable (the config option wins when both are set; the region ids are listed under [Limitations](/compute/limitations)). Without it the deploy stops with `project "<name>" does not exist yet and no deploy region is configured`. Once the project exists it keeps its region, and later deploys, stages included, inherit it. The CLI finds or creates everything else. Keep the values out of the repo (an `.env` you source at deploy time, or CI secrets). When `PRISMA_SERVICE_TOKEN` is set it takes priority over any stored session.
