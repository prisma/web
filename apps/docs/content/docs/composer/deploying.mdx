...
2. **The project and its branches.** Those are the containers an environment deploys into: created if absent on deploy, and never deleted by one. Tearing down a whole environment is the [`destroy` operation](#destroying); deleting an entire project is [`project delete`](/cli/v8/project), which asks you to repeat the project id with `--confirm <project-id>` because it destroys the project's databases too.
