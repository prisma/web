...
| You want to... | Run |
| --- | --- |
| Deploy to production | `npx prisma@latest composer deploy module.ts` |
| Deploy an isolated environment | `npx prisma@latest composer deploy module.ts --stage <name>` |
| Deploy under a different app name | `npx prisma@latest composer deploy module.ts --name demo-42` |
| Tear down an isolated environment | `npx prisma@latest composer destroy module.ts --stage <name>` |
| Tear down production's resources | `npx prisma@latest composer destroy module.ts --production` |
...
```npm
npx prisma@latest composer destroy module.ts --stage staging  # staging only; production untouched
npx prisma@latest composer destroy module.ts --production     # production's resources
```
...
```bash
npx prisma@latest composer deploy module.ts --stage "pr-$PR_NUMBER"    # on push
npx prisma@latest composer destroy module.ts --stage "pr-$PR_NUMBER"   # on close
```
...
- [`composer` command reference](/cli/composer): every flag on deploy, destroy, dev, and log.
