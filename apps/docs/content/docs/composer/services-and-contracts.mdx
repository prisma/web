...
`composer dev` runs the same pipeline as a deploy, so local runs enforce the keys too: a direct `curl` to a provider's RPC endpoint returns `401` even on `localhost`. Only bare runs and [tests](/composer/testing) skip enforcement, because nothing is provisioned there.
