...
A **Prisma App** is a tree of Modules composed in TypeScript. At the leaves are **services**, the units that run your code, and **resources**, stateful things like a Postgres database. A parent Module wires them together; your code never participates in the wiring, it just receives the results.
...
A Module can also declare boundary `deps`: inputs its parent must supply, wired exactly like a service's. The root module you already have is just the outermost Module, with no boundary at all.
