---
title: Databases
description: Give a service a Postgres database, either as a plain connection or typed by a Prisma ORM contract with managed migrations.
url: /composer/databases
metaTitle: Databases | Prisma Composer
metaDescription: Add Prisma Postgres to a Composer service with rawPostgres() for a plain connection, or postgres() for a Prisma ORM-typed client with deploy-time migrations.
---
...
Composer's [service contracts](/composer/services-and-contracts) describe an RPC API between services. A Prisma ORM **data contract** describes a database schema. This page uses both: `postgres()` ties a database dependency to a Prisma ORM data contract.
...
If you want typed queries and managed migrations, make the database a [Prisma ORM](/prisma-orm) one. `load()` then returns `{ url, client }`: the raw connection string, plus a client generated from your data contract. Queries like `db.client.orm.public.Product.where({ id }).first()` are compile-time checked, with no SQL strings and no row mapping. The client is constructed only on first access, so a service that brings its own Postgres client can still read `db.url` and keep contract-checked wiring and deploy-time migrations.

The schema workflow runs once per schema change, using the [Prisma ORM CLI commands](/cli):
...
- [Prisma ORM CLI reference](/cli): the contract and migration commands in detail.
