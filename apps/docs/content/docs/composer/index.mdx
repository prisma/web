...
Two choices shape the rest of the framework. Application code does not read `process.env` or hardcode URLs; dependencies arrive typed through `service.load()`, so production, an isolated stage, and a test all run the same code with different injected values. And Composer does not bundle or transform your code: you build with your own bundler, and the deploy assembles what you built.
