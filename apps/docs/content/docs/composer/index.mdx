...
Prisma Composer is in [Early Access](/console/more/feature-maturity#early-access). APIs and commands can change between releases. Composer ships inside the [Prisma CLI](/cli/v8/deploy) as the root-level `dev` and `deploy` commands: run them with `npx prisma@next dev module.ts` and `npx prisma@next deploy module.ts`.
...
`npx prisma@next deploy module.ts` loads the root module, compares it with the deploy state stored for that environment, and creates or updates the services on Prisma Compute and the databases on Prisma Postgres. Re-running a deploy applies only the difference:
