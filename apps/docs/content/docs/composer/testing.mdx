...
- **You do not need a service key.** Only a deploy (or `composer dev`) provisions keys; in a test nothing checks, every call reaches your handler, and there is nothing to put in `inputs`.
