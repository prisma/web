...
Re-deploying is idempotent. For an isolated copy of the whole app, deploy a stage; tear it down with the `destroy` operation of the [control API](/cli/v8/deploy#the-control-api) when you are done:

```npm
npx prisma@next deploy module.ts --stage demo
```
