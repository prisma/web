...
`init` also adds a `postinstall` hook that keeps the skill matching the installed version; see [`skills`](/cli/v8/skills).
...
The CLI creates a project named `my-app` in your workspace (if one already exists, the deploy reuses it and can stop with `HostedStateBootstrapError`; pass `--name <unique-name>` in that case), provisions both services on Prisma Compute, points the gateway's `quotes` dependency at the deployed quotes service, and starts everything. The deploy finishes by printing what it made: your module names, the platform resource each became, and the public URLs. Open the gateway's URL and you get a quote.
