...
A new project needs a region, so give it one before the first deploy, in the deploy config or in the environment. The [`deploy` reference](/cli/deploy) shows both ways.
...
```npm
npx prisma deploy module.ts --stage demo
```
