...
You do not need to pin `effect` yourself. `@prisma/composer` and `@prisma/composer-prisma-cloud` pin every `effect`-family package their deploy engine needs, so a fresh install resolves a single copy. Only an app that pins a different `effect` itself can hit `DEPS.EFFECT_VERSION_CONFLICT`; see [When a deploy stops on an effect version conflict](/composer/deploying#when-a-deploy-stops-on-an-effect-version-conflict).
