...
Run your install again after adding the block. (Yarn users: the equivalent field is `"resolutions"`, but this guide is tested with npm and pnpm only.)
...
```npm
npx prisma@next composer deploy module.ts --stage demo
npx prisma@next composer destroy module.ts --stage demo
```
