...
You do not need to pin `effect` yourself. `@prisma/composer` and `@prisma/composer-prisma-cloud` pin every `effect`-family package their deploy engine needs, so a fresh install resolves a single copy. `DEPS.EFFECT_VERSION_CONFLICT` only appears when something in your dependency tree, your own `effect` pin or another dependency's, hoists a different `effect` where the deploy engine resolves it; see [When a deploy stops on an effect version conflict](/composer/deploying#when-a-deploy-stops-on-an-effect-version-conflict).
...
A new project needs a region, so before the first deploy change `prismaCloud()` in `prisma-composer.config.ts` to `prismaCloud({ region: 'us-east-1' })`, or export `PRISMA_REGION=us-east-1`. The region ids are listed under [Limitations](/compute/limitations).
...
- [Databases](/composer/databases): add a Postgres, plain or typed by a Prisma ORM contract.
