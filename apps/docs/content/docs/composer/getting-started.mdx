...
- **[Bun](https://bun.sh).** Prisma Compute runs Bun, so the server code targets it (`Bun.serve`). `bun build` is also the fastest way to produce the built output.
...
Then pin the `effect` family in your `package.json` to `4.0.0-beta.103`, the version current `@prisma/composer` releases (0.8.0 through 0.11.0) pin internally. This works around an upstream bug in Composer's deploy engine, whose own version ranges float past the versions its code supports. Without the pins, a fresh install ends up with two conflicting copies of `effect`, and every Composer command refuses to run. On other Composer versions, match whatever `dependencies.effect` says in `node_modules/@prisma/composer/package.json`:
...
Run your install again after adding the block. (Yarn users: the equivalent field is `"resolutions"`. Bun reads the npm-style top-level `"overrides"`.)
...
Without `alchemy`, `composer dev` and `composer deploy` stop at `DEPLOY.ALCHEMY_BIN_MISSING`; without `@prisma/dev`, local dev stops with "local dev needs @prisma/dev". A single-package Bun project is not affected.
...
```npm
npx prisma@latest composer deploy module.ts --stage demo
npx prisma@latest composer destroy module.ts --stage demo
```
