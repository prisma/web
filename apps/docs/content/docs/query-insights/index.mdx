...
Then add it as a Prisma Client extension:

```ts
import "dotenv/config";
import { PrismaPg } from "@prisma/adapter-pg";
import { PrismaClient } from "../generated/prisma/client";
import { withQueryInsights } from "@prisma/sqlcommenter-query-insights";

const connectionString = `${process.env.DATABASE_URL}`;

const adapter = new PrismaPg({ connectionString });
const prisma = new PrismaClient({ adapter }).$extends(withQueryInsights());
```
...
Query Insights is included with Prisma Postgres at no extra cost and is currently in Early Access. You can try it today in the [Prisma Console](https://console.prisma.io).
