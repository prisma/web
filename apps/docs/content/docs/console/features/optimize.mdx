...
:::warning
**Sunsetting Optimize** — Optimize is being retired on March 23. We're replacing it with [Query Insights](/query-insights), which is ready to use out of the box.
:::
...
7. Copy the API key and store it securely. This will be used in your project's `.env` file via the `"OPTIMIZE_API_KEY"` environment variable. Finally, click the **"I've stored it securely"** button.
