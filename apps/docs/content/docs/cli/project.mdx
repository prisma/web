...
Committed configuration lives in `prisma.compute.ts` or `prisma.compute.json`. The [`init`](/cli/platform-init) command creates this configuration, and the [`service` commands](/cli/service) use it.
