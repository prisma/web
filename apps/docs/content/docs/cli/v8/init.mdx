...
The runtime files are ES modules. If your `package.json` declares `"type": "commonjs"`, `orm init` keeps it and warns; set `"type": "module"` so the scaffolded `db.ts` loads.
