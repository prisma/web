...
`deploy` and [`dev`](/cli/v8/dev) are root-level commands of the unified Prisma CLI. There is no `composer` command group.
...
npx prisma@next deploy module.ts --stage feat-auth
```
