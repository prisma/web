...
```npm
npx prisma@next db verify --db "$DATABASE_URL"
npx prisma@next db verify --db "$DATABASE_URL" --strict
npx prisma@next db verify --db "$DATABASE_URL" --schema-only
npx prisma@next db verify --db "$DATABASE_URL" --marker-only
```
