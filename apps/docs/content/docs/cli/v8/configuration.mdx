...
Prisma 8 CLI commands read `prisma.config.ts` in your project root. The file has one section per part of the CLI. The Prisma 8 data commands read the `orm` section; the [agent skills commands](/cli/v8/skills) read the `skills` section.
...
[`orm init`](/cli/v8/orm-init) writes this file for you. Pass `--config` when your config file is not at `./prisma.config.ts`:
...
The `skills` section controls the [agent skills commands](/cli/v8/skills) and the staleness check:
...
[`init`](/cli/v8/init) scaffolds this section for you.
...
| `PRISMA_SKILLS_CHECK=0` | Disables the [agent skills staleness check](/cli/v8/skills#the-staleness-check). |
