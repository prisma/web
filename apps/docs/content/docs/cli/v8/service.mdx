...
Use `service` commands to manage the services in a project. A deploy produces a **version** of a service; one version is live at a time. Deploys start from a git push, the [Console](https://pris.ly/pdp), or [`deploy`](/cli/v8/deploy) for a Composer app. The commands below inspect and manage services and the versions that already exist.
...
npx prisma@next service show my-service
```
