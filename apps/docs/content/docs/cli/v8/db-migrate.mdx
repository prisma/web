...
```npm
npx prisma@next db migrate --show
npx prisma@next db migrate --show --from @contract --to production
```
...
Manage refs with [`migration ref`](/cli/v8/migration-ref).
