...
Use `migration ref` commands to manage named refs stored with your migration history. A ref maps a logical environment name, such as `staging` or `production`, to a contract hash. Other commands can then target that environment by name: [`db migrate --to production`](/cli/v8/db-migrate), [`db update --to production`](/cli/v8/db-update), or [`db sign production`](/cli/v8/db-sign).
...
```npm
npx prisma@next migration ref set production 4cb4256
npx prisma@next migration ref list
npx prisma@next migration ref delete production
```
...
```npm
npx prisma@next migration ref set production 20260101T1000_add_user
npx prisma@next migration status --db "$DATABASE_URL" --to production
npx prisma@next db migrate --db "$DATABASE_URL" --to production
```
