...
The `prisma dev` command starts a [local Prisma Postgres](/local-development/postgres) database that you can run Prisma ORM commands against. It's useful for development and testing and allows easy migration to [Prisma Postgres](/postgres) in production.
