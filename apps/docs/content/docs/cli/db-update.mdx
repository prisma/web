---
title: db update
description: Update a database to match the current Prisma ORM contract.
url: /cli/db-update
metaTitle: db update | Prisma ORM CLI
metaDescription: Learn how to reconcile a database with the current Prisma ORM contract.
---
...
```npm
npx prisma db update --db "$DATABASE_URL" --no-interactive --confirm appdb
```
