...
- [`db sign`](/cli/db-sign) advances it after a successful signature, with or without `--db`. `--no-advance-ref` turns that off.
- [`db migrate --advance-ref db`](/cli/db-migrate) advances it after an apply. Plain `db migrate` never advances anything, which is deliberate: a deploy or CI apply should not change a repository ref.
