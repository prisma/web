---
title: migration ref
description: Manage named Prisma ORM refs that point at contracts.
url: /cli/migration-ref
metaTitle: migration ref | Prisma ORM CLI
metaDescription: 'Learn how to set, list, and delete named Prisma ORM contract refs.'
---
...
The `db` ref has a special meaning: it records the contract state you expect your local database to match. When you run [`migration plan`](/cli/migration-plan) without `--from`, Prisma ORM assumes you mean from the `db` ref, so as long as the ref is kept up to date, each plan contains only your latest change.

The `db` ref is updated automatically in the following situations:

- [`db init`](/cli/db-init) and [`db update`](/cli/db-update) update it when you run them without `--db`, so that the connection comes from `prisma.config.ts`. With `--db`, they leave it alone unless you also pass `--advance-ref db`.
- [`db sign`](/cli/db-sign) updates it after a successful signature, with or without `--db`. `--no-advance-ref` turns that off.
- [`db migrate --advance-ref db`](/cli/db-migrate) updates it after an apply. Plain `db migrate` never touches it, on purpose: a deploy or CI run should not change a file in your repository.
