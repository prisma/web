...
Use `ref` commands to manage named refs stored with your migration history. A ref maps a logical environment name, such as `staging` or `production`, to a contract hash. Other commands can then target that environment by name: [`migrate --to production`](/cli/migration-apply), [`db update --to production`](/cli/db-update), or [`db sign production`](/cli/db-sign).
...
```npm
npx prisma@latest ref set production 4cb4256
npx prisma@latest ref list
npx prisma@latest ref delete production
```
...
```npm
npx prisma@latest ref set production 20260101T1000_add_user
