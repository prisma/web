...
Any other default is a SQL expression. `db verify` cannot work out whether two SQL expressions give the same value, so it compares their text, ignoring letter case and spaces. PostgreSQL often rewrites an expression when it stores it, for example by adding casts, so write a `sql` default in your contract the way PostgreSQL stores it. To see that form, run `npx prisma contract infer --output ./inferred.prisma` and copy the column's `@default` from that file.

`db verify` reads the database with the time zone set to UTC and dates in ISO format, whatever the server or your database role sets. It compares check constraints and the `WHERE` clause of a partial index as exact text. So if your contract was inferred before `8.0.0-rc.12` from a server that was not set to UTC, a `timestamptz` value inside a check constraint or an index's `WHERE` clause can be reported as a difference. Copy the new text from a fresh `npx prisma contract infer --output ./inferred.prisma` into your contract, emit the contract, and run `db sign`.
