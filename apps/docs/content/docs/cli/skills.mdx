...
url: /cli/skills
metaTitle: skills | Prisma ORM CLI
...
After a package upgrade that nobody follows with a sync, every `prisma` command appends one line to stderr (or, under `--format markdown`, a short section to stdout, shown below):

```text
Prisma agent skills are out of date (installed @prisma/orm-postgres 8.1.0, synced 8.0.0). Run: prisma skills sync
```

The notice never changes the exit code. It is not conditioned on a TTY: agents run without one, and they are who it is for.

Under `--format markdown` the notice moves to stdout, after the command's output, so an agent still reads one stream:
...
Every other format keeps it on stderr and writes nothing to stdout. It is suppressed by any of:

1. `prisma skills sync --disable` (persistent, per project).
2. `skills: { check: false }` in `prisma.config.ts`.
3. The [`PRISMA_SKILLS_CHECK=0`](/cli/environment-variables) environment variable.
