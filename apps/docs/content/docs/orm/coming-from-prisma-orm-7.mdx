...
`npm install prisma` now installs version 8. If your project still uses version 7, that replaces its command-line tool. To keep both, skip the commands below and follow step 1 of [Migrate from Prisma ORM 7 to 8](/guides/upgrade-prisma-orm/postgresql) instead; the tables on this page apply either way. Leave `prisma/schema.prisma`, `prisma/migrations`, and `@prisma/client` in place; the guide removes them at the end.
...
It writes `prisma.config.ts`, a starter `src/prisma/contract.prisma`, `src/prisma/db.ts`, and a quick reference at `prisma-next.md`. It also sets `module`, `moduleResolution`, `resolveJsonModule`, and `types` in `tsconfig.json`, installs `dotenv`, and writes `.env.example`. It asks whether to write `.env` too. Answer yes (`npx prisma orm init --write-env` skips the question), then put your connection string in it:

```txt title=".env"
DATABASE_URL="postgresql://user:password@localhost:5432/mydb"
```

The starter contract is a placeholder. If you have an existing database, generate your contract from it with `npx prisma contract infer --output src/prisma/contract.prisma`, which overwrites the placeholder (it warns first), and check the result against the table below. If you would rather convert your `schema.prisma` by hand, the table is the mapping. Either way, run `npx prisma contract emit` afterwards; `db.ts` imports the two files it writes.

Do not run `db init` or `db update` against a database that already has your Prisma ORM 7 tables: they would alter those tables to match the contract. Step 4 of the migration guide covers that database.
...
| (nothing) | `// use prisma-next` as the first line | `prisma orm init` writes it; add it yourself if you write the contract by hand. Without it the contract still works, but the Prisma editor extension does not recognise the file. `prisma-next` was the name of Prisma ORM 8 before release, and a few URLs still use it |
