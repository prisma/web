...
| (nothing) | `// use prisma-8` as the first line | `prisma orm init` writes it; add it yourself if you write the contract by hand. Without it the contract still works, but the Prisma editor extension does not recognise the file. Contracts written before `8.0.0-rc.10` start with `// use prisma-next` instead; that header still works, and the editor's format action rewrites it to the new one. `prisma-next` was the name of Prisma ORM 8 before release, and a few URLs still use it |
...
Prisma ORM 7 features that have no direct form in Prisma ORM 8, with what to do instead. The status column says which: not available today, available in a different form, or (for `$extends`) not coming.
...
| `Prisma.UserGetPayload<...>` and the generated model types | available in a different form | `contract.d.ts` exports a `Models` namespace with one member per model, `Models.public_User`. `Scalars<Models.public_User>` is the row a plain fetch returns, `Shape<Models.public_User, { '-': 'passwordHash'; posts: { '+': 'id' \| 'title' } }>` is a row with chosen fields and relations, and `ResultType<typeof query>` is what a query returns. `Scalars` and `Shape` come from `@prisma/orm-postgres/family-contract/types`, `ResultType` from `@prisma/orm-postgres/components/runtime`. Needs `8.0.0-rc.10` and a fresh `prisma contract emit`; see [Named model types](/orm/reference/orm-client#named-model-types) |
