...
| (nothing) | `// use prisma-8` as the first line | `prisma orm init` writes it; add it yourself if you write the contract by hand. `prisma contract emit` reads only the files that start with it, and the [Prisma editor extension](/orm/contract-authoring/editor-support) does not recognise a file without it. |
