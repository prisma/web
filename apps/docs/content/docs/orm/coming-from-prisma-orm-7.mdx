...
To move a running application, follow [Migrate from Prisma ORM 7 to 8](/guides/upgrade-prisma-orm/postgresql) instead. It runs both versions side by side against the same database and moves one part of the app at a time. To decide whether to move at all, see [Release status](/orm/release-status).

All examples use PostgreSQL.

## Before you start

If you are adding Prisma ORM 8 to an application that runs Prisma ORM 7, follow [Migrate from Prisma ORM 7 to 8](/guides/upgrade-prisma-orm/postgresql). It installs both versions side by side and takes over your existing database. Do not run the commands below in that project: `npm install prisma` installs version 8 and replaces the version 7 command-line tool. On PostgreSQL, [`prisma orm init --from-prisma7-schema prisma/schema.prisma`](/cli/orm-init#on-a-prisma-orm-7-project) does that setup for you, and Prisma ORM 8 then reads your `schema.prisma` as its contract.
...
| (nothing) | `// use prisma-8` as the first line | `prisma orm init` writes it; add it yourself if you write the contract by hand. `prisma contract emit` reads only the files that start with it, and the [Prisma editor extension](/orm/contract-authoring/editor-support) does not recognise a file without it. Contracts from before `8.0.0-rc.10` say `// use prisma-next` instead, which still works; the editor extension rewrites it when you format |
...
| `@default(dbgenerated("gen_random_uuid()"))` | ``@default(sql`gen_random_uuid()`)`` | `dbgenerated` is rejected. Any SQL expression is a `sql` tagged literal, except `now()` and `autoincrement()`, which you write as those functions. [Default values](/orm/contract-authoring/psl-syntax#default-values) covers every kind of default |
...
| `Json @default("{}")` | ``Jsonb @default(json`{}`)`` | a JSON default is a `json` tagged literal. A quoted string is text, and a JSON column does not accept it |
...
- `contract`: the path to your contract file, or a glob for a [contract in several files](/orm/contract-authoring/psl-syntax#split-the-contract-across-several-files). On PostgreSQL it can also be `prisma7Schema("prisma/schema.prisma")`, which reads your Prisma ORM 7 schema as it is; see [Use a Prisma ORM 7 schema](/cli/configuration#use-a-prisma-orm-7-schema).
- `db`: the connection.
...
- `db.sql`: your tables, by table name (`db.sql.public.User`; the table name is the model name, unless you set `@@map`). You need it only for raw SQL.
...
| `where: { title: { search: "cat & dog" } }`, with the `fullTextSearchPostgres` preview feature | `.where((p) => p.title.fullTextMatches(toTsquery("cat & dog")))`, with `toTsquery` from `@prisma/orm-postgres/target/full-text`. `websearchToTsquery(input)` reads search-box text. Sort by relevance with `.orderBy((p) => p.title.fullTextRank(query).desc())`, where `query` is the same `toTsquery(...)` value. Add `@@fullTextIndex([title], name: "...")` to the model so PostgreSQL can use an index. See [Full-text search](/orm/reference/orm-client#full-text-search) |
...
The table is `User`, as in Prisma ORM 7: a model without `@@map` names its table after the model. Every raw query that returns rows needs `.returnsRow(...)` with a type per column, and you take the type from the table, as above. For a computed column, write the type name instead, such as `pg/int4@1` or `pg/text@1`; the `@1` is a version number and is always `1` today. [Raw queries](/orm/reference/raw-queries) lists the names. Finish the query with `.build()` and pass it to `db.runtime().query()` for rows or `db.runtime().execute()` for a count.
...
| --- | --- | --- |
| `skipDuplicates` on `createMany` | available in a different form | `.createAll(rows, { onConflict: "skip" })` or `.createAndCount(rows, { onConflict: "skip" })` skips each row that collides with a unique constraint; add `conflictOn: ["email"]` to name the constraint. `createAll` returns only the rows the database wrote. Run `prisma contract emit` once before you use it, because a contract emitted before `8.0.0-rc.12` does not list the capability it needs |
...
- [Release status](/orm/release-status), for how close Prisma ORM 8 is to its final release, and how to stay on Prisma ORM 7.
