...
On PostgreSQL, the following command does the guide's first phase for you, and sets up Prisma ORM 8 to read your `schema.prisma` as its contract. `schema.prisma` does not need a `// use prisma-8` line for this.

```npm
npx prisma@latest orm init --from-prisma7-schema prisma/schema.prisma
```

It does not change `prisma/` or the database. [`orm init` on a Prisma ORM 7 project](/cli/orm-init#on-a-prisma-orm-7-project) lists what it changes, and the guide covers the rest of the move.
...
| (nothing) | `// use prisma-8` as the first line | required in every contract file; see the note below the table |
...
| `@default(dbgenerated("gen_random_uuid()"))` | ``@default(sql`gen_random_uuid()`)`` | `dbgenerated` is rejected. Write any SQL expression as `` sql`...` `` instead, but keep writing `now()` and `autoincrement()` as plain functions. [Default values](/orm/contract-authoring/psl-syntax#default-values) covers every kind of default |
...
:::note[The first line of a contract file]

Every contract file must start with `// use prisma-8`. `prisma orm init` writes the line for you, so you add it yourself only when you create a contract file by hand. `prisma contract emit` reads only the `.prisma` files that start with this line and skips the others without a warning, and the [Prisma editor extension](/orm/contract-authoring/editor-support) does not recognize a file without it.

:::

:::note[Enums in a database that Prisma ORM 7 created]
...
Filter a `pg.enum(...)` field with `eq` or `in`, because it has no `like` or `ilike`. You can switch to a plain `enum` later, but that changes the column type, so it needs a migration.
...
- `contract`: the path to your contract file, or a glob for a [contract in several files](/orm/contract-authoring/psl-syntax#split-the-contract-across-several-files). On PostgreSQL it can also be `prisma7Schema("prisma/schema.prisma")`, with `prisma7Schema` imported from `@prisma/orm-postgres/config` next to `defineConfig`. It reads your Prisma ORM 7 schema as it is, with no `// use prisma-8` line; see [Use a Prisma ORM 7 schema](/cli/configuration#use-a-prisma-orm-7-schema).
- `db`: the connection.
...
- `db.sql`: the [SQL query builder](/orm/reference/sql-query-builder), with your tables by table name (`db.sql.public.User`; the table name is the model name, unless you set `@@map`). On this page it appears only in the raw SQL examples, which take the type of each returned column from it.
- `db.raw.sql`: writes a raw SQL query as text.
...
The table is `User`, as in Prisma ORM 7: a model without `@@map` names its table after the model. A value you put in the SQL with `${...}`, such as `${id}`, is sent to the database as a query parameter, not pasted into the SQL text, as with Prisma ORM 7's `$executeRaw`.
...
Add `@@fullTextIndex([title], name: "post_title_search")` to the `Post` model so PostgreSQL can use an index for the search. [Full-text search](/orm/reference/orm-client#full-text-search) covers the other options.
...
Here `conflictOn` is a list of field names. In `upsert` it is an object of fields and values instead. If you emitted your contract before `8.0.0-rc.12`, run `prisma contract emit` again before you use `onConflict`; otherwise the call fails with `ORM.CAPABILITY_MISSING`.
