...
- traversing relationships via the fluent API ([Fluent API](/orm/v6/prisma-client/queries/relation-queries#fluent-api))
- nested writes that enable updating/creating connected records ([Nested writes](/orm/v6/prisma-client/queries/relation-queries#nested-writes))
- applying filters on related records ([Relation filters](/orm/v6/prisma-client/queries/relation-queries#relation-filters))
- type-safe querying of nested data without writing the underlying SQL ([Nested reads](/orm/v6/prisma-client/queries/relation-queries#nested-reads))
- creating nested TypeScript typings based on models and their relations ([Type safety](/orm/v6/prisma-client/type-safety))
- intuitive modeling of relations in the data model via relation fields ([Relations](/orm/v6/prisma-schema/data-model/relations))
- implicit handling of relation tables (also sometimes called JOIN, link, pivot or junction tables) ([Implicit many-to-many relations](/orm/v6/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations))
