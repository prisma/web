...
| ---------------------- | ------------------------ | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `where`                | `UserWhereUniqueInput`   | **Yes**  | Wraps all fields of a model so that a record can be selected (learn more about how to [filter on non-unique fields with `UserWhereUniqueInput`](#filter-on-non-unique-fields-with-userwhereuniqueinput)). <br></br>Before version 4.5.0, this type only wraps _unique_ fields of a model.                                                                                         |
...
- From version 4.6.0, Prisma ORM hands over upsert queries to the database where possible. Learn more about [database upserts](#database-upserts).
...
You can learn more about join strategies in [Relation load strategies (Preview)](/orm/v6/prisma-client/queries/relation-queries#relation-load-strategies-preview).
...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  From version 4.5.0, this type exposes all fields on the model. This means that when you filter for a single record based on a unique field, you can check additional non-unique and unique fields at the same time. Learn more about how to [filter on non-unique fields with `UserWhereUniqueInput`](#filter-on-non-unique-fields-with-userwhereuniqueinput).
