...
For faster integration tests, you can wrap each test in an [interactive transaction](/orm/v6/prisma-client/queries/transactions#interactive-transactions) and roll it back at the end. This keeps the test database available for the whole test run while still isolating data changes from one test to the next.

This pattern works best when the code under test accepts a Prisma Client instance or transaction client as a dependency:
...
export async function createCustomer(db: DbClient, email: string) {
...
Then, in your test, run the assertions inside an interactive transaction and intentionally roll back after the assertions complete:

```ts title="order-service.test.ts"
import { PrismaClient } from "../generated/prisma/client";
import { PrismaPg } from "@prisma/adapter-pg";
import { createCustomer } from "./order-service";
...
test("creates a customer", async () => {
  await prisma
    .$transaction(async (tx) => {
      const customer = await createCustomer(tx, "alice@example.com");
