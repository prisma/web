...
- Deno v1.29.4 or later installed. Learn more about [installing Deno](https://docs.deno.com/runtime/#install-deno).
- (Recommended) Latest version of Prisma ORM.
- (Recommended) Deno extension for VS Code. Learn more about [the Deno VS Code extension](https://docs.deno.com/runtime/reference/vscode/).
...
```bash
deno install npm:@prisma/client@6.19.3
deno install npm:@prisma/adapter-pg@6.19.3
```
...
```bash
deno run -A npm:prisma@6.19.3 migrate dev --name init
```
...
import { PrismaPg } from "npm:@prisma/adapter-pg@6.19.3";
...
1. In the **Build Step** add `deno run -A npm:prisma@6.19.3 generate` to generate the Prisma Client.
