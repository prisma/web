...
```bash
deno install npm:@prisma/client@6
deno install npm:@prisma/adapter-pg@6
```
...
```bash
deno run -A npm:prisma@6 migrate dev --name init
```
...
import { PrismaPg } from "npm:@prisma/adapter-pg@6";
...
1. In the **Build Step** add `deno run -A npm:prisma@6 generate` to generate the Prisma Client.
