...
```npm
npm install @prisma/adapter-better-sqlite3@6.19.3
```
...
SQLite doesn't have a dedicated Boolean type. While this table shows `BOOLEAN`, columns are assigned a **NUMERIC affinity** (storing `0` for false and `1` for true). Learn more about [SQLite's boolean handling](https://www.sqlite.org/datatype3.html#boolean).
