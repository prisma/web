...
Learn more about Neon in [Neon's introduction docs](https://neon.tech/docs/introduction).
...
A Neon compute has two main states: _Active_ and _Idle_. Active means that the compute is currently running. If there is no query activity for 5 minutes, Neon places a compute into an idle state by default. Refer to Neon's docs to learn more about [compute lifecycle](https://neon.tech/docs/introduction/compute-lifecycle).
