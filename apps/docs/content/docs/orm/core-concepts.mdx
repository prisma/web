...
A **migration** is a recorded change to the database. Most migrations change the database schema from what one contract describes to what another describes. A migration that only changes rows starts and ends at the same contract state. It is a directory in your repository containing the change as editable TypeScript (`migration.ts`), the compiled operations that Prisma runs (`ops.json`), and metadata recording which contract hash it starts `from` and ends at, `to`.

One command applies them: [`db migrate`](/cli/db-migrate) applies the recorded migrations to a live database. The other `migration ...` commands never touch a database; they create and inspect the migration files in your repository.
