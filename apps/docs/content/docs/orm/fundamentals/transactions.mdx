...
Under `tx.sql.public`, unlike under `tx.orm.public`, the property name is the table name: the model's `@@map` value, or the model name exactly as written when the model has no `@@map`. So the model `Post` is `tx.sql.public.Post`, and a model with `@@map("post")` is `tx.sql.public.post`.
