...
Two things differ from `tx.orm`. Under `tx.sql.public` the property name is the table name, not the model name. The table name is the model name exactly as written, so the model `Post` has the table `Post` and it is `tx.sql.public.Post`, unless the model sets `@@map`: a model with `@@map("post")` is `tx.sql.public.post`.
...
On MongoDB there is no schema segment in the path, and the property name is the collection name. The collection name is the model name exactly as written, so a model `User` is `db.orm.User`, unless the model sets `@@map`. `db.orm.users` above is the collection name; the model is `User` with `@@map("users")`.
