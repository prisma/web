...
Start from a collection with `db.query.from(...)`, which takes the collection name as a string: the model's `@@map` value, or the model name exactly as written when the model has no `@@map`. Every model above sets `@@map`, so the collections are `user`, `post`, `tag`, and `post_tag`. That is the same name the ORM API uses, as in `db.orm.post`.
