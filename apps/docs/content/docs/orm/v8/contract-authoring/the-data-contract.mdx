...
**[PSL](/orm/v8/contract-authoring/psl-syntax)** is the preferred way to author the contract: a compact language purpose-built for describing data. It is what `create-prisma` scaffolds, what [`contract infer`](/cli/v8/contract-infer) writes when you start from an existing database, and what the examples throughout these docs use.

For the cases PSL does not cover, define models with the **[TypeScript builder](/orm/v8/contract-authoring/typescript-schema-builder)** instead. Reach for it when model definitions must be composed, generated, or shared as ordinary TypeScript modules.
