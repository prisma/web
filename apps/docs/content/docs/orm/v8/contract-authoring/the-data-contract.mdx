...
The contract also carries identity. Emission computes hashes over the contract's content, and [`db sign`](/cli/v8/db-sign) records them in a small marker inside the database. Before executing queries, Prisma 8 compares the contract it was built against with the marker in the database it is talking to. A mismatch, such as a deploy against an unmigrated database, fails verification instead of failing at query time.
...
- The migration tooling diffs two contracts to plan schema changes, and [`db verify`](/cli/v8/db-verify) checks a live database against the current contract.
