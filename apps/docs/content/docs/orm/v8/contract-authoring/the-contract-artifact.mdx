...
url: /orm/v8/contract-authoring/the-contract-artifact
...
[`prisma-next contract emit`](/cli/v8/contract-emit) writes both next to the source by default:
...
Both files are generated. Do not edit them; change the [PSL](/orm/v8/contract-authoring/psl-syntax) or [TypeScript](/orm/v8/contract-authoring/typescript-schema-builder) source and re-run `contract emit`. The files carry a generated-file notice that says exactly this.
...
Determinism is also why the contract source must stay pure. A schema that read the clock or the environment would emit differently each time, and every hash-based guarantee below would collapse. The [TypeScript authoring page](/orm/v8/contract-authoring/typescript-schema-builder#keep-the-contract-file-pure) lists the concrete rules.
...
The hashes are how Prisma 8 connects the contract to a live database. [`prisma-next db sign`](/cli/v8/db-sign) checks that the database satisfies the contract and records the hashes in a marker inside the database. From then on, [`prisma-next db verify`](/cli/v8/db-verify) and the runtime compare the contract they hold against the marker, so a stale deploy or an unmigrated database is caught by verification rather than by a failing query.
...
- **`capabilities`** and **`extensionPacks`**: the database and extension features available to this contract, merged from the target, adapter, and extension packs at emit time. Query APIs consult these before using gated features; see [Capabilities](/orm/v8/contract-authoring/capabilities).
...
- Learn how the contract's [capabilities](/orm/v8/contract-authoring/capabilities) gate database features.
- Apply the contract to a fresh database with [`prisma-next db init`](/cli/v8/db-init).
- Check a live database against the contract with [`prisma-next db verify`](/cli/v8/db-verify).
- Plan schema changes between contract versions with [`prisma-next migration plan`](/cli/v8/migration-plan).
