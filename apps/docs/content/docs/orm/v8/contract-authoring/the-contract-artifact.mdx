...
[`contract emit`](/cli/v8/contract-emit) writes both next to the source by default:
...
The hashes are how Prisma 8 connects the contract to a live database. [`db sign`](/cli/v8/db-sign) checks that the database satisfies the contract and records the hashes in a marker inside the database. From then on, [`db verify`](/cli/v8/db-verify) and the runtime compare the contract they hold against the marker, so a stale deploy or an unmigrated database is caught by verification rather than by a failing query.
...
- Apply the contract to a fresh database with [`db init`](/cli/v8/db-init).
- Check a live database against the contract with [`db verify`](/cli/v8/db-verify).
- Plan schema changes between contract versions with [`migration plan`](/cli/v8/migration-plan).
