...
PSL is the preferred way to author the Prisma 8 [data contract](/orm/v8/contract-authoring/the-data-contract). You write a Prisma schema file, usually `prisma/contract.prisma`, and [`prisma-next contract emit`](/cli/v8/contract-emit) turns it into `contract.json` and `contract.d.ts`.
...
If the database already exists, don't write the schema by hand. [`prisma-next contract infer`](/cli/v8/contract-infer) reads the live schema and writes a starter `contract.prisma` for you to review and edit.
...
- Apply the contract to a database with [`prisma-next db init`](/cli/v8/db-init) or plan changes with [`prisma-next migration plan`](/cli/v8/migration-plan).
