...
url: /orm/v8/reference/pipeline-builder
...
The pipeline builder gives you a typed way to build MongoDB aggregation pipelines. You reach it through `db.query`, chain aggregation stages onto a starting collection, and resolve the pipeline with a terminal. It is the MongoDB counterpart to the [SQL query builder](/orm/v8/reference/sql-query-builder): a lower-level surface for the queries the [ORM client](/orm/v8/reference/orm-client) doesn't express.
...
The pipeline builder targets MongoDB only. PostgreSQL has no pipeline builder: use the [SQL query builder](/orm/v8/reference/sql-query-builder) for PostgreSQL joins and aggregates, and the [ORM client](/orm/v8/reference/orm-client) for everyday queries on either database.
:::
...
- **Read results are codec-decoded.** Unlike [raw queries](/orm/v8/reference/raw-queries), a pipeline built through `db.query` carries a result shape, so top-level `_id` fields come back as decoded hex strings, and other fields are decoded to their contract types.
...
- For the full raw MongoDB surface (raw collection methods, untyped writes, and the undecoded-results behavior), see [Raw queries](/orm/v8/reference/raw-queries).
