...
url: /orm/v8/reference/raw-queries
...
Raw queries are the escape hatches for the queries the typed surfaces can't express. Reach for a typed surface first: the [ORM client](/orm/v8/reference/orm-client) for everyday reads and writes, the [SQL query builder](/orm/v8/reference/sql-query-builder) for PostgreSQL joins and aggregates, and the [pipeline builder](/orm/v8/reference/pipeline-builder) for MongoDB aggregation. When none of them reaches the SQL clause or MongoDB command you need, drop down to raw.
...
Raw SQL lives inside the [SQL query builder](/orm/v8/reference/sql-query-builder), not as a standalone statement. You write a SQL fragment as a tagged template and splice it into a `select()`, `where()`, `orderBy()`, or `update()` call. There is no way to run a bare raw SQL string on its own; every raw fragment is part of a builder query that you [`build()`](/orm/v8/reference/sql-query-builder#build) and run with `runtime.execute(...)`.

The examples in this section run against the `user` / `post` schema from the SQL query builder page. See its [example schema](/orm/v8/reference/sql-query-builder#example-schema) for the full model definitions. As on that page, create the client with `postgres(...)`, connect it for a runtime, and reach tables through `db.sql.public`:
...
`.returns(codecId)` is a compile-time type annotation only. It declares how the fragment's result is typed on the JavaScript side; it does not cast the value in SQL or change how the driver decodes it. See the SQL query builder's [`fns.raw` and `.returns()`](/orm/v8/reference/sql-query-builder#fnsraw-and-returns) for the full treatment.
...
The examples in this section run against the `users` / `posts` schema from the pipeline builder page. See its [example schema](/orm/v8/reference/pipeline-builder#example-schema) for the full model definitions.
...
`db.query.rawCommand(command)` runs a raw MongoDB aggregate command through the [pipeline builder](/orm/v8/reference/pipeline-builder), bypassing the typed AST. Its rows come back as `unknown`; you type them yourself.
