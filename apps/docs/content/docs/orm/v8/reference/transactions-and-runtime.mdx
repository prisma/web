...
url: /orm/v8/reference/transactions-and-runtime
...
For a task-oriented walkthrough, see the Fundamentals guide to [Transactions](/orm/v8/fundamentals/transactions). For the query surfaces you run inside a transaction, see the [ORM client reference](/orm/v8/reference/orm-client), the [SQL query builder reference](/orm/v8/reference/sql-query-builder), and the [raw queries reference](/orm/v8/reference/raw-queries).
...
- `db.execute(plan)` runs any `MongoQueryPlan`, including a plan built by the [pipeline builder](/orm/v8/reference/pipeline-builder) (`db.query`).
...
To run a multi-document MongoDB transaction today, use the MongoDB driver directly, sharing one `MongoClient` between Prisma 8 and your driver code. See the [Fundamentals transactions guide](/orm/v8/fundamentals/transactions#transactions-on-mongodb) for the driver-session pattern.
:::
...
A result is consumed once per mode. Re-`await`ing a buffered result is safe (it returns the cached array), but switching between awaiting and iterating a consumed result throws `RUNTIME.ITERATOR_CONSUMED`. For the full single-consumption rules, shared identically by PostgreSQL and MongoDB, see [`AsyncIterableResult`](/orm/v8/reference/orm-client#asynciterableresult) in the ORM client reference.
