...
The `prisma-cli` in that hint is the binary's own name for itself. When you invoke it as `npx prisma@next`, run the suggested subcommand the same way (`npx prisma@next migration status`).
...
npx prisma@next db migrate --show --db $DATABASE_URL
...
```npm
npx prisma@next db migrate --to prod --db $DATABASE_URL          # a ref
npx prisma@next db migrate --to sha256:e6b5c28 --db $DATABASE_URL # a hash prefix
npx prisma@next db migrate --to 20260707T1005_init --db $DATABASE_URL
```
...
```npm
npx prisma@next migration plan --name my_change && npx prisma@next db migrate --advance-ref db
```
...
npx prisma@next db migrate --show --db $DATABASE_URL   # log what's about to run
npx prisma@next db migrate --db $DATABASE_URL
```
...
- [Generating a migration](/orm/v8/migrations/generating-a-migration): producing what `db migrate` runs
