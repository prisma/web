...
The connection comes from `db.connection` in `prisma.config.ts`, or `--db` to override:
...
If the graph has branched and more than one tip is reachable, `migrate` stops and asks for an explicit `--to`. That's the graph protecting you: two feature branches may both be valid futures, and picking one is a human decision.
