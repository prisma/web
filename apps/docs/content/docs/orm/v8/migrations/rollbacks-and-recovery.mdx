...
- **For a database with no history at all** (a fresh environment, or adopting Prisma 8 on an existing schema), `prisma-next db init` bootstraps it to the current contract and signs the marker.
