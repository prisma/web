...
```bash
npx @prisma/cli@next migrate --to 20260707T1008_add_display_name^
```
...
- **In development**, `db update` reconciles the database directly to your contract without walking the graph: quick, but off the record, so treat it as a dev-only reset.
