...
url: /orm/v8/migrations/the-migration-graph
...
If none of these apply yet, read [How migrations work](/orm/v8/migrations/how-migrations-work) instead and come back when they do.
...
Most days you never touch the graph directly. To see it, run `migration graph`. To check where a database is and what is pending, run `migration status`. To move a database to a specific state, run `migrate --to <ref>`. The rest of this page explains what those commands are showing you.
...
(The summary line counts contract *spaces*: independent migration lanes, one for your app plus one per [database extension](/orm/v8/migrations/applying-a-migration#extension-spaces).)
...
npx prisma@next migration graph --config fixtures/showcase/prisma.config.ts --legend
```
...
```npm
npx prisma@next migration ref set prod sha256:f9a41d7...
npx prisma@next migration ref list
npx prisma@next db migrate --to prod
```
...
One ref name is special. A ref called `db` is the default starting point `migration plan` uses when you do not pass `--from`. Keep it current with `migrate --advance-ref db` and planning stays incremental on its own.
...
An edge can point backwards, from a later contract state to an earlier one. Rolling back is not a special mode: it is planning one more migration whose destination is a state you have already been in. See [Rollbacks and recovery](/orm/v8/migrations/rollbacks-and-recovery).
...
| Move a database to a named state | `migrate --to prod` |
| Name the current state `prod` | `ref set prod @db` |
| List the refs you have named | `ref list` |
| Roll a database back one state | `migrate --to <earlier-ref-or-hash>` |
...
- [How migrations work](/orm/v8/migrations/how-migrations-work): the plan-review-apply loop
- [Applying a migration](/orm/v8/migrations/applying-a-migration): how the runner walks the graph
- [Studio with Prisma 8](/studio/prisma-next): browse the same ledger visually in Prisma Studio, one contract edge per migration
- [Rollbacks and recovery](/orm/v8/migrations/rollbacks-and-recovery): backwards edges in practice
