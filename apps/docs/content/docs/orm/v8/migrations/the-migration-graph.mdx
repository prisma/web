...
When you run `migrate`, the runner walks the graph for you:
...
npx @prisma/cli@next migration graph --config fixtures/showcase/prisma-next.config.ts --legend
```
...
```bash
npx @prisma/cli@next ref set prod sha256:f9a41d7...
npx @prisma/cli@next ref list
npx @prisma/cli@next migrate --to prod
```
