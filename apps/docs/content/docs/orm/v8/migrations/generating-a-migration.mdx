...
(The `prisma-cli` in the hint is the binary's own name for itself; when invoking it as `npx prisma@next`, run the suggested subcommand the same way: `npx prisma@next migrate`.)
