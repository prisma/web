...
url: /orm/v8/data-modeling/mongodb
...
Making that choice well is the core of MongoDB data modeling, and it is what this page focuses on. If you are new to models and keys, start with the [data modeling overview](/orm/v8/data-modeling).
...
This is a one-to-many by reference: many posts point at one user, and each post is queryable on its own. When you [include the relation in a query](/orm/v8/fundamentals/relations-and-joins), Prisma 8 resolves it with a `$lookup` aggregation.
...
Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent; the `prisma-next-contract` skill covers document modeling. Prompts that map to each section:
...
- [Query documents](/orm/v8/fundamentals/reading-data) and [resolve references](/orm/v8/fundamentals/relations-and-joins) with `.include(...)`.
- Aggregate and reshape documents with the [pipeline builder](/orm/v8/fundamentals/advanced-queries#mongodb-pipeline-builder).
- To model relational data, see the [relational data modeling guide](/orm/v8/data-modeling/relational-databases).
