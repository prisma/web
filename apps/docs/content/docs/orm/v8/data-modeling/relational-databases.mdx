...
url: /orm/v8/data-modeling/relational-databases
...
This page shows how to model each relation shape and, for each one, where the foreign key belongs. If you are new to models and keys, start with the [data modeling overview](/orm/v8/data-modeling).
...
See [Relations and joins](/orm/v8/fundamentals/relations-and-joins#one-to-many) for querying in both directions.
...
Declare the relation on the side that holds the foreign key (`Profile` here). The mirror field on the other model (`profile Profile?` on `User`) is not supported yet, so [query the relation from the profile side](/orm/v8/fundamentals/relations-and-joins#one-to-one).
...
Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent; the `prisma-next-contract` skill covers relation modeling. Prompts that map to each section:
...
- [Query relations](/orm/v8/fundamentals/relations-and-joins) with `.include(...)`, relation predicates, and junction traversal.
- To model for MongoDB, see the [MongoDB data modeling guide](/orm/v8/data-modeling/mongodb).
