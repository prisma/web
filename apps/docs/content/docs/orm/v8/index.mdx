...
Prisma 8 is the next major version of Prisma ORM, rewritten in TypeScript. It keeps the schema-first workflow and model-first queries from earlier versions, and introduces a new query API, an extension system, and a revised migration workflow.

The central change is how your schema reaches your code. You still author models in a Prisma schema file (or in TypeScript), but instead of generating a client package, Prisma 8 emits the schema as a **contract**: a JSON document plus TypeScript types that your application, the migration tools, and coding agents all read. The contract is a plain artifact you can inspect and diff.
...
<Cards>
  <Card href="/v8/quickstart/postgresql" title="Quickstart" icon={<Rocket className="text-primary" />}>
...
The design behind Prisma 8 is documented in a nine-part blog series:
