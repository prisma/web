...
url: /orm/v8/fundamentals/reading-data
...
On MongoDB, `.where(...)` does not take a lambda. It accepts the object form, plus lower-level `MongoFieldFilter` expressions from `@prisma/orm-mongo/query-ast/execution` that cover comparisons and boolean logic; see [Filter conditions and operators](/orm/v8/reference/orm-client#filter-conditions-and-operators) in the reference. For a lambda-style operator set, go one level down to the [pipeline builder](/orm/v8/fundamentals/advanced-queries#mongodb-pipeline-builder), where `.match(...)` covers the same filters:
...
There is no `.count()` method on the query chain. On MongoDB, count with a `$group` stage in the [pipeline builder](/orm/v8/fundamentals/advanced-queries#mongodb-pipeline-builder).
...
- [Write data](/orm/v8/fundamentals/writing-data): create, update, delete, and upsert records.
- [Read related records](/orm/v8/fundamentals/relations-and-joins) in the same query with `.include(...)`.
- [Use advanced queries](/orm/v8/fundamentals/advanced-queries) when a shape needs the SQL query builder or a MongoDB pipeline.
