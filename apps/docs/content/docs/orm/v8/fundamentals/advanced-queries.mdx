...
url: /orm/v8/fundamentals/advanced-queries
...
**Prefer the ORM API when** the query is CRUD, filtered reads, or relation traversal. The [reading](/orm/v8/fundamentals/reading-data), [writing](/orm/v8/fundamentals/writing-data), and [relations](/orm/v8/fundamentals/relations-and-joins) pages cover that surface, with less code and the same type safety.
...
Chain more joins for multi-hop traversals. This is how you get a flat post-tag list through a [many-to-many junction table](/orm/v8/fundamentals/relations-and-joins#many-to-many), one row per pair:
...
Plans execute through `db.runtime().execute(plan)` on PostgreSQL and `(await db.runtime()).execute(plan)` on MongoDB. Inside a [transaction](/orm/v8/fundamentals/transactions), use `tx.execute(plan)`.

## Prompt your coding agent

Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent; the `prisma-next-queries` skill covers both builders and the choice between them and the ORM API. Prompts that map to each section:
...
- [Read data](/orm/v8/fundamentals/reading-data): the ORM happy path these builders back up.
- [Understand relationships](/orm/v8/fundamentals/relations-and-joins) before reaching for explicit joins.
- [Run SQL builder plans atomically](/orm/v8/fundamentals/transactions) inside a transaction.
