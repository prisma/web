...
url: /orm/v8/fundamentals/writing-data
...
Array operations such as `.push(...)` and `.pull(...)` are not verified yet; test them against your own schema before relying on them. See [Field update operations](/orm/v8/reference/orm-client#field-update-operations) in the reference.
...
The `Count` forms skip re-reading the affected records, so prefer them for large batches. The `All` forms return their records as a result you can `await` into an array or [stream with `for await`](/orm/v8/fundamentals/reading-data#stream-large-results):
...
If the second write fails, the first has already committed, and you're left with half the operation. When writes must succeed together, run them in a [transaction](/orm/v8/fundamentals/transactions): one callback, one commit, or one rollback.

### Passing an array of queries to a transaction

Prisma 7 supported `$transaction([query1, query2])`. Prisma 8 does not: there is no `$transaction`, and queries don't queue up in arrays. Put the calls inside one `db.transaction(...)` callback instead; the [Transactions page](/orm/v8/fundamentals/transactions) shows the pattern.

## Prompt your coding agent

Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent; the `prisma-next-queries` skill covers everything on this page. Prompts that map to each section:
...
- [Run several writes atomically](/orm/v8/fundamentals/transactions) with `db.transaction(...)`.
- [Read data](/orm/v8/fundamentals/reading-data) to filter, sort, paginate, and select fields from your models.
- [Use the SQL builder](/orm/v8/fundamentals/advanced-queries#postgresql-sql-query-builder) for inserts and updates with explicit `RETURNING` clauses.
