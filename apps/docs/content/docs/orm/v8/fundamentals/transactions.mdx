...
url: /orm/v8/fundamentals/transactions
...
[SQL builder](/orm/v8/fundamentals/advanced-queries) plans run inside a transaction through `tx.execute(...)`, with `tx.sql` mirroring `db.sql`:
...
- [Write data](/orm/v8/fundamentals/writing-data): the single-record and bulk mutations you group in a transaction.
- [Use advanced queries](/orm/v8/fundamentals/advanced-queries) to run SQL builder plans inside or outside transactions.
