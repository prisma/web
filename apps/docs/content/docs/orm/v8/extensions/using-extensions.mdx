...
All target PostgreSQL. ParadeDB and Supabase are experimental (ParadeDB supports the `key_field` index option only so far). The rest are part of the Release Candidate. Extension names link to each package's README on GitHub.
