...
url: /orm/v8/middleware/authoring-custom-middleware
...
A custom middleware is a plain object with a `name` and the hooks you implement. There is no base class and no registration API beyond the `middleware` array you already know from [How middleware works](/orm/v8/middleware/how-middleware-works).
...
- A Prisma 8 project with a working `src/prisma/db.ts`. The [PostgreSQL quickstart](/v8/quickstart/postgresql) sets one up in a few minutes:
...
Leave `familyId` out only when the middleware sticks to family-neutral surfaces (`intercept`, `onRow`, `afterExecute` on the shared result shape). Such a middleware runs on PostgreSQL and MongoDB alike; the [cache](/orm/v8/middleware/built-in-cache) works this way.
...
[How middleware works](/orm/v8/middleware/how-middleware-works#the-five-hooks) walks the order with an animation. Two hooks deserve a closer look here, because they change the query rather than observe it.
...
- Throwing from `afterExecute` on a successful query fails the call even though the database work already happened. Reserve it for cases where failing loudly is the point, as the [budgets](/orm/v8/middleware/built-in-budgets) latency check does.
...
Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent. Prompts that map to this guide:
...
- [How middleware works](/orm/v8/middleware/how-middleware-works): the lifecycle your hooks plug into
- [Built-in: budgets](/orm/v8/middleware/built-in-budgets), [Built-in: lints](/orm/v8/middleware/built-in-lints), and [Built-in: cache](/orm/v8/middleware/built-in-cache) as production examples of the hook surface
