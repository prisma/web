...
url: /orm/v8/middleware/how-middleware-works
...
| --- | --- | --- |
| [`budgets`](/orm/v8/middleware/built-in-budgets) | `@prisma/orm-postgres/family-runtime` | Blocks queries that would read too many rows, and reports queries that overrun a latency budget |
| [`lints`](/orm/v8/middleware/built-in-lints) | `@prisma/orm-postgres/family-runtime` | Blocks or warns on risky query shapes, such as `DELETE` without `WHERE` |
| [`cache`](/orm/v8/middleware/built-in-cache) | `@prisma/orm-extension-middleware-cache` | Serves repeated reads from an in-memory store, opted in per query |

To write your own, follow the [authoring guide](/orm/v8/middleware/authoring-custom-middleware): it builds a working query logger step by step.
...
Projects scaffolded with `create-prisma@next` install [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent. Prompts that map to this page:
...
- [Authoring custom middleware](/orm/v8/middleware/authoring-custom-middleware): build and test a query logger step by step
- [Writing data](/orm/v8/fundamentals/writing-data): the mutations that lints and budgets guard
- [Built-in: budgets](/orm/v8/middleware/built-in-budgets), [Built-in: lints](/orm/v8/middleware/built-in-lints), [Built-in: cache](/orm/v8/middleware/built-in-cache)
- [Using extensions](/orm/v8/extensions/using-extensions) for adding database capabilities rather than wrapping queries
