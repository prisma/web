...
`migration log` reads the **ledger**, so it tells you what actually ran rather than what should have run, and a rollback appears there as one more applied migration instead of erasing anything. Run `migration check` in your pull-request checks, because it catches a hand-edited migration file or a ref naming a state that does not exist before anything is deployed, and it needs no database connection. `db migrate` also refuses to run a hand-edited migration file. See [Reviewing what you planned](/orm/migrations/generating-a-migration#reviewing-what-you-planned) for its exit codes.
...
## Adding a migration you write yourself
