...
Not every command takes every token: `npx prisma db migrate --show --from` accepts all three, while `migration plan --from` accepts only `@empty` of the three, since it is offline and cannot read a database. So if you want to see the path from the contract you have now to the state `prod` names, without changing anything, run `npx prisma db migrate --show --from @contract --to prod`.
