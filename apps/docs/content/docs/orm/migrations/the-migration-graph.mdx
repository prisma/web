...
- **`migration new` starts from the `db` ref.** `npx prisma migration new` writes an empty migration for a change you write yourself, such as a data update, and without `--from` it starts where `migration plan` would: at the `db` ref. To build on another state, such as `e377d00` in the drawing above, pass a prefix of it: `npx prisma migration new --name backfill --from e377d00`.
