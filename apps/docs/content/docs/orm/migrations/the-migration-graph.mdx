...
`npx prisma migration new` writes an empty migration for a change you write yourself, such as a data update. Without `--from` it starts where `migration plan` would, at the `db` ref. Its `--from` accepts less than the one on `migration plan`: it takes only the contract hash in the `to` field of an existing migration's `migration.json`, or the first few characters of it, and not a ref name, a migration directory name, or one of the `@` tokens. So to build on `e377d00` in the drawing above, run `npx prisma migration new --name backfill --from e377d00`.
