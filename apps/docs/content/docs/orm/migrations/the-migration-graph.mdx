...
The [Prisma ORM repository](https://github.com/prisma/orm) has branched histories, including a `diamond` fixture with the same `init`, `alice_add_phone`, and `bob_add_avatar` migrations, in `examples/prisma-8-demo/fixtures/`. To draw one, build the repository and pass the example's `prisma.config.ts` to `migration graph` with `--config`.
...
Two branches can plan migrations at the same time without either one knowing about the other. After the merge, one migration planned from `prod` ends at the merged contract, a branch migration that no longer leads to it stays in the graph without ever running, and a development database that applied that one gets the merged contract from `npx prisma db update`, as [A worked example](#a-worked-example) shows.
...
- "Two feature branches both added migrations. Draw the graph and plan the one migration production needs from the `prod` ref."
