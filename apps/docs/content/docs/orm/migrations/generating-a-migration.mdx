...
Commit `migrations/app/refs/db.json` with the migration files. On a fresh clone it then points at the contract state the team's development databases are at, so the first `migration plan` chains from there instead of from an empty database. When a merge conflicts on that file, take the hash your own database is at, which `npx prisma migration status` prints as the current contract, or set the ref afterwards with `npx prisma migration ref set db <dir>`.
