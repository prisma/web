...
`ops.json` is the compiled form of the migration: when `migration.ts` runs, Prisma turns the TypeScript steps into JSON operations. The planner runs it for you, but after an edit, re-run it yourself with `node migration.ts`. The migration runner reads only this file.
...
These commands live under the [Prisma ORM CLI](/cli), and you run them with `npx prisma@latest <command>`. The planning and inspection commands work offline, because they read files, not your database:
...
Projects scaffolded with `create-prisma@latest` install [Prisma ORM skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent. Ask your agent to:
