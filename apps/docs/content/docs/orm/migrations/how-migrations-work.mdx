...
A table name is the model name exactly as written unless the model sets `@@map`, so `User` is the table `"User"` and `UserProfile` is `"UserProfile"`, the same names Prisma ORM 7 used. Before `8.0.0-rc.12`, a model without `@@map` named its table with a lowercase first letter, `"userProfile"`. If your tables were created with those names, run the `add-model-map` script from the [upgrade recipe](https://github.com/prisma/orm/blob/v8.0.0-rc.12/skills/prisma-8/upgrading/app/upgrades/8.0.0-rc.11-to-8.0.0-rc.12/) over every `.prisma` file, including the `contract.prisma` copies under `migrations/`, before you plan. It adds `@@map("<current table name>")` to each model that has none. Without it, `migration plan` stops with [`MIGRATION.TABLE_NAME_CASE_CHANGED`](/orm/reference/error-reference#MIGRATION.TABLE_NAME_CASE_CHANGED) rather than drop the table and create an empty one.
