...
In the same migration directory, add `intermediate.config.ts`, a second config file. Its `contract` is `intermediate.prisma`, and it has no `db` setting, because emitting a contract never connects to a database. `parent: false` makes the CLI use this file on its own. Without it, the CLI also loads your project's `prisma.config.ts` and adds that file's `orm` settings, such as `db`, to the ones written here:
...
Emit the intermediate contract with `--config` pointing at that file. A relative path in a config file resolves from the directory of that file, so `./intermediate.prisma` is found wherever you run the command from:
...
import type { Contract as Start } from '../../snapshots/866ab885a8a4bd8d9f5adc47784c068be7895a7182c55cc29f9f9967d3019484/contract';
import startContractJson from '../../snapshots/866ab885a8a4bd8d9f5adc47784c068be7895a7182c55cc29f9f9967d3019484/contract.json' with { type: 'json' };
...
`migration new` always ends at the contract state in your `contract.json`, and a data-only migration starts there too, so you pass that state's hash with `--from`. If you leave `--from` off, `migration new` starts from the `db` ref, and when the ref already names your current contract, as it does after `npx prisma db migrate --advance-ref db`, the command stops with an error whose `code` is `MIGRATION.NO_CHANGES`. `--from` takes the contract hash in the `to` field of an existing migration's `migration.json`, or the first few characters of it. To find your current contract's hash, run `npx prisma migration graph` and look for the row labelled `@contract`. It shows the first 7 characters of the hash, and you can pass those to `--from`. The [`migration new` reference](/cli/migration-new) lists the rest of the options:
