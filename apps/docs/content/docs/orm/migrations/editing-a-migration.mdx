...
Keep `// use prisma-8` as the first line of the copy, because `contract emit` reads only files that start with it.

In the same migration directory, add `intermediate.config.ts`, a second config file that names `intermediate.prisma` as the contract and needs no database connection. `parent: false` stops the CLI from also reading your project's `prisma.config.ts`, which it would otherwise find in a parent directory and merge in:
...
Emit the intermediate contract with `--config` pointing at that file. A relative path in a config file resolves from the directory of that file, so `./intermediate.prisma` is found wherever you run the command from. Commit all four `intermediate.*` files along with the migration, since the migration no longer recompiles without them.
...
A data-only migration starts and ends at the contract state your database already matches. Without `--from`, `migration new` starts from the `db` ref, which is that state when you apply migrations with `--advance-ref db`. To start from another state, pass `--from` with the hash of the migration that ends there. `npx prisma migration graph` marks your contract's hash `@contract`. The [`migration new` reference](/cli/migration-new) lists the rest of the options:
