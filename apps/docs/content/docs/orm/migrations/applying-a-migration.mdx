...
npx prisma@latest migrate --show --db $DATABASE_URL
...
```npm
npx prisma@latest migrate --to prod --db $DATABASE_URL          # a ref
npx prisma@latest migrate --to sha256:e6b5c28 --db $DATABASE_URL # a hash prefix
npx prisma@latest migrate --to 20260707T1005_init --db $DATABASE_URL
```
...
```npm
npx prisma@latest migration plan --name my_change && npx prisma@latest migrate --advance-ref db
```
...
npx prisma@latest migrate --show --db $DATABASE_URL   # log what's about to run
npx prisma@latest migrate --db $DATABASE_URL
```
...
- [Generating a migration](/orm/migrations/generating-a-migration): producing what `migrate` runs
