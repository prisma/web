...
**In CI and production**, you never plan a migration, because migrations arrive through your repository, already reviewed and merged. For that to work, commit these files:
...
Deploy with the three commands in [Check before, preview, then apply](#check-before-preview-then-apply). The point of the `migration check` step is to tell you that what you are about to apply is still what was reviewed.

When a migration's files no longer match the hash stored when it was written, `migration check` fails with an error whose `code` is `MIGRATION.CHECK_HASH_MISMATCH`, as [Editing a migration](/orm/migrations/editing-a-migration) explains. To fix it, either restore the migration's files from Git, or move the hand edit into `migration.ts` and recompile.

When a contract snapshot under `migrations/snapshots/` no longer matches the hash in its directory name, `migration check` fails with `MIGRATION.CHECK_SNAPSHOT_CONTENT_MISMATCH`. A command that needs that snapshot, such as `migration plan --from` or `migration ref set` naming its contract state, stops with `MIGRATION.CONTRACT_SNAPSHOT_CONTENT_MISMATCH`. Recompiling `migration.ts` does not fix this, so restore the edited snapshot from Git. MongoDB snapshots are not checked yet.
