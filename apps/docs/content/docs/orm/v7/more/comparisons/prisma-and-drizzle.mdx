...
In contrast, Drizzle’s schema is built via TypeScript code, which makes it harder to visualize your full data model, increases cognitive load, and can lead to inconsistencies in how models are defined across the codebase. Read more in [the Prisma Schema Language blog post](https://www.prisma.io/blog/prisma-schema-language-the-best-way-to-define-your-data) about why we are bullish on PSL (Prisma Schema Language).
...
- traversing relationships via the fluent API ([Fluent API](/orm/v7/prisma-client/queries/relation-queries#fluent-api))
- nested writes that enable updating/creating connected records ([Nested writes](/orm/v7/prisma-client/queries/relation-queries#nested-writes))
- applying filters on related records ([Relation filters](/orm/v7/prisma-client/queries/relation-queries#relation-filters))
- type-safe querying of nested data without writing the underlying SQL ([Nested reads](/orm/v7/prisma-client/queries/relation-queries#nested-reads))
- creating nested TypeScript typings based on models and their relations ([Type safety Overview](/orm/v7/prisma-client/type-safety))
- intuitive modeling of relations in the data model via relation fields ([Relations](/orm/v7/prisma-schema/data-model/relations))
- implicit handling of relation tables (also sometimes called JOIN, link, pivot or junction tables) ([Implicit many-to-many relations](/orm/v7/prisma-schema/data-model/relations/many-to-many-relations#implicit-many-to-many-relations))
