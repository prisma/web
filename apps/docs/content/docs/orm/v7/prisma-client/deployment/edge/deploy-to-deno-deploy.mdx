...
deno run -A npm:prisma@prev init --db
```
...
```json title="deno.json"
{
  ...
  "imports": {
    ...
    "prisma": "npm:prisma@prev"
  },
  "tasks": {
    "dev": "deno run -A --env=.env --watch index.ts",
    "db:generate": "deno run -A --env=.env npm:prisma@prev generate",
    "db:push": "deno run -A --env=.env npm:prisma@prev db push",
    "db:migrate": "deno run -A --env=.env npm:prisma@prev migrate dev",
    "db:studio": "deno run -A --env=.env npm:prisma@prev studio"
  }
}
```
...
3. Configure the deployment:
   ...
   - **Build command**: `deno run -A npm:prisma@prev generate`
   - **Entrypoint**: `index.ts`
