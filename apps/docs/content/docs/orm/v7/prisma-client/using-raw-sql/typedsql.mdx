...
In this example, the columns to be selected are defined dynamically and included in the SQL query. While this approach provides flexibility, it requires careful attention to security, particularly to [avoid SQL injection vulnerabilities](/orm/v7/prisma-client/using-raw-sql/raw-queries#sql-injection-prevention). Raw SQL queries also forego the type safety of TypedSQL.

## Acknowledgements

This feature was heavily inspired by [PgTyped](https://github.com/adelsz/pgtyped) and [SQLx](https://github.com/launchbadge/sqlx). SQLite parsing is also handled by SQLx.
