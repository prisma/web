...
- Prisma maps any database values returned by `$queryRaw` and `$queryRawUnsafe` to their corresponding JavaScript types. Learn more about [raw query type mapping](#raw-query-type-mapping).

- `$queryRaw` does not support dynamic table names in PostgreSQL databases. Learn more about [dynamic table names in PostgreSQL](#dynamic-table-names-in-postgresql)
...
Note that if you use `$queryRawUnsafe` in conjunction with user inputs, you risk SQL injection attacks. Learn more about [`$queryRawUnsafe()`](#queryrawunsafe).
