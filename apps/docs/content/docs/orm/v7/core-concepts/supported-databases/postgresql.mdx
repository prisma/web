...
`DATABASE_URL` has to be set in the process environment when this code runs. Node.js does not read `.env` files on its own, so add `import "dotenv/config";` at the top of your entry file or start Node with `--env-file=.env`. If the variable is missing, the adapter passes an empty configuration to `pg`, which fails with `SASL: SCRAM-SERVER-FIRST-MESSAGE: client password must be a string`.
