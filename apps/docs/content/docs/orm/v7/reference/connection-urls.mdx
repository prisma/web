...
url: /orm/v7/reference/connection-urls
...
Prisma ORM needs a connection URL to be able to connect to your database, e.g. when sending queries with [Prisma Client](/orm/v7/prisma-client/setup-and-configuration/introduction) or when changing the database schema with [Prisma Migrate](/orm/v7/prisma-migrate).
...
- [Microsoft SQL Server](/orm/v7/core-concepts/supported-databases/sql-server)
...
For Microsoft SQL Server, you must [escape special characters](/orm/v7/core-concepts/supported-databases/sql-server#connection-details) in any part of your connection string.
...
However, in this case the `API_KEY` doesn't provide authentication details. Instead, it encodes information about the local Prisma Postgres instance. You can obtain a local connection string via the [`prisma dev`](/orm/v7/reference/prisma-cli-reference#dev) command.
