...
| ---------------------- | ------------------------ | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `where`                | `UserWhereUniqueInput`   | **Yes**  | Wraps all fields of a model so that a record can be selected (learn more about how to [filter on non-unique fields with `UserWhereUniqueInput`](#filter-on-non-unique-fields-with-userwhereuniqueinput)). |
...
- `UserWhereUniqueInput` This type works by exposing any unique fields on the model. A field assigned `@id` is considered unique,
  ...
  This type exposes all fields on the model. This means that when you filter for a single record based on a unique field, you can check additional non-unique and unique fields at the same time. Learn more about how to [filter on non-unique fields with `UserWhereUniqueInput`](#filter-on-non-unique-fields-with-userwhereuniqueinput).
