...
All examples on this page run against the schema below. It is the same MongoDB schema as the [ORM client](/orm/reference/orm-client) page, so examples carry over between the two pages. `Article` and `Tutorial` are variants of `Post`. They are stored in the same `posts` collection and told apart by the value of the `kind` field. See [`variant()`](/orm/reference/orm-client#variant) on the ORM client page for how to query one variant. You do not need `variant()` on this page, because `kind` is an ordinary field you can filter and group on.
...
`db` is the client you create with `mongo(...)`, whose `dbName` option is the name of the MongoDB database to use. See [Transactions and runtime](/orm/reference/transactions-and-runtime#mongooptions) for every option and for how to create the client.
...
- **After `lookup()` or `group()`, compare ids with `String(a._id) === String(b._id)`**, where `a` is a looked-up sub-document and `b` is the parent document it came from. Both `_id` values are the driver's own `ObjectId` objects there, and two `ObjectId` objects are never `===` even when they hold the same id.
- **Read results are decoded.** If every stage in your pipeline is one of `match()`, `sort()`, `limit()`, `skip()`, `sample()`, `project()`, `addFields()`, and `vectorSearch()`, results are decoded: `_id` is a hex string and dates are your contract's date type. If any stage is not in that list, including `lookup()` and `group()`, every document comes back raw: `_id` is the driver's `ObjectId` and dates are the driver's `Date`.
...
- Neither a hex string nor a driver `ObjectId` matches in `match()`. The builder sends the value as a plain object, so an `ObjectId` arrives as its bytes. See the warning below.
...
- A **callback** form computes a projection spec (`project((f) => ({ title: 1, shout: fn.toUpper(f.title) }))`). Write `1` to keep a field, `0` to drop it, or an expression to compute it. MongoDB rejects a mix of `1` and `0` in one projection, except for `_id: 0`.
...
const plan = db.query
  .from('posts')
  .project((f) => ({ _id: 0, title: 1, shout: fn.toUpper(f.title) }))
  .build();
...
- `f.rawPath(path)` names a field or a MongoDB `$$` variable by its raw string, with no checking against your model. It prefixes `$` to the string you give it, so `f.rawPath('$KEEP')` reaches MongoDB as `$$KEEP`. There is no expression helper for a `$$` variable, and `fn.literal(...)` cannot stand in. The server rejects both `fn.literal('KEEP')` and `fn.literal('$$KEEP')`.
...
| The condition argument of `fn.cond()`, `redact()`, and the option-object stages | The `.node` property of the `fn.*` value. |
...
- `bucketAuto({ groupBy, buckets, output?, granularity? })` sorts them into a given number of ranges, chosen so each holds about the same number of documents. `groupBy` is a raw expression. `granularity` picks the bucket size from one of MongoDB's preferred-number series.
...
- `setWindowFields({ partitionBy?, sortBy?, output })` computes a value for each document from the documents around it, such as a running total. `partitionBy` is a raw expression. `output` is an object mapping each new field name to `{ operator, window? }`, where `operator` is the accumulator or expression to compute.
...
- `fill({ partitionBy?, partitionByFields?, sortBy?, output })` replaces missing or null values with a value you choose. `partitionBy` is a raw expression. `output` is an object mapping each field name to `{ method }` or `{ value }`, where `value` is an expression.
...
- Build a raw expression with the `fn.*` helpers and pass its `.node` property, or name a field with `MongoAggFieldRef.of('duration')`.
- The stages you pass to `facet` and `unionWith` are classes: `MongoMatchStage`, `MongoProjectStage`, `MongoSortStage`, `MongoLimitStage`, `MongoSkipStage`, `MongoLookupStage`, `MongoUnwindStage`, `MongoGroupStage`, `MongoAddFieldsStage`, `MongoReplaceRootStage`, `MongoCountStage`, `MongoSortByCountStage`, `MongoSampleStage`, `MongoRedactStage`, `MongoOutStage`, `MongoUnionWithStage`, `MongoBucketStage`, `MongoBucketAutoStage`, `MongoGeoNearStage`, `MongoFacetStage`, `MongoGraphLookupStage`, `MongoMergeStage`, `MongoSetWindowFieldsStage`, `MongoDensifyStage`, `MongoFillStage`, `MongoSearchStage`, `MongoSearchMetaStage`, and `MongoVectorSearchStage`. Import them, and `MongoAggFieldRef`, from `@prisma/orm-mongo/query-ast/execution`. Each stage class takes the same arguments as the MongoDB stage it builds. The three used below are `MongoCountStage(field)`, `MongoSortStage(spec)`, and `MongoLimitStage(n)`.
...
- `output` is the expression whose value you want back from each chosen document. `sortBy` is a plain object such as `{ createdAt: 1 }`, where `1` sorts ascending and `-1` sorts descending.
...
Expression helpers (`fn.*`) build the computed values used inside stages like `addFields()`, `project()`, `group()` accumulators, and `match()` (via `expr()`). An expression is an `f.field` value or an `fn.*` value, never a plain JavaScript value. Stage callbacks and these helpers take expressions. The update operators on the [write methods](#write-methods) and the `MongoFieldFilter` methods take plain JavaScript values instead. Import the helpers from `@prisma/orm-mongo/query-builder`:
...
| MongoDB operator | Helper | Why |
|---|---|---|
| `$in` | `fn.isIn` | `in` is a reserved word in JavaScript. |
| `$type` | `fn.typeOf` | `typeof` is a reserved word in JavaScript. |
| `$toString` | `fn.toString_` | `toString` is already a method on every JavaScript object. |
...
`fn.literal(value)` wraps a constant so MongoDB reads it as a value and not as the name of a field. Every `fn.*` helper returns a typed wrapper, and `.node` is the raw MongoDB expression inside that wrapper. Arguments typed as an expression take the wrapper. Arguments typed as a raw MongoDB expression take `.node`, and those are the condition of `fn.cond()`, `redact()`, and the option-object stages: `bucket()`, `bucketAuto()`, `graphLookup()`, `setWindowFields()`, and `fill()`. Passing an `expr(...)` value where `.node` is expected throws a `TypeError`. This is what you pass at each call site:

| Call site | What you pass |
|---|---|
| `project()`, `addFields()`, and the arguments of an accumulator | The `fn.*` value itself. |
| The condition argument of `fn.cond()`, and `f.stage.set()`, `f.stage.replaceRoot()`, `f.stage.replaceWith()` | The `.node` property of the `fn.*` value. |
| The `groupBy`, `startWith`, and `partitionBy` options of the option-object stages | The `.node` property of the `fn.*` value. |
| `match()` | The `fn.*` value wrapped in `expr(...)`. |
...
- Each entry in those lists is a `MongoFieldFilter`, which has `eq`, `neq`, `gt`, `gte`, `lt`, `lte`, `in`, `nin`, `isNull`, and `isNotNull`. These are the same combinators the ORM client uses, described under [Combinators](/orm/reference/orm-client#combinators).
...
```typescript
const plan = db.query.from('posts')
  .project((f) => ({ shout: fn.concat(fn.toUpper(f.title), fn.literal('!')) })).build();
```
...
- `pipe(stage)` appends a raw stage that you construct yourself, and keeps the row type you already had. The stages are the `Mongo*Stage` classes, one per MongoDB stage, from `@prisma/orm-mongo/query-ast/execution`. The example below uses `MongoMatchStage` and `MongoCountStage`. The same module exports `MongoFieldFilter`, which builds the filter a `MongoMatchStage` takes.
...
The pipeline builder can write, not just read. Write methods are available at three points: on the root collection, after a `match()` filter, and as the last call in a pipeline. A write method returns the built query on its own, so there is no `build()` call after one. The examples below name that built query `plan`.
...
- An updater is either all operator form or all pipeline form. Operator form calls an operator on a field, as in `f.bio.set(value)`. Pipeline form calls `f.stage.*`, and can read other fields of the same document. Type `f.stage` exactly: `stage` is not a placeholder for a stage name. You cannot mix the two in one updater. TypeScript rejects the mixed array, and at run time it throws an error whose `code` is `ORM.ARGUMENT_INVALID`: `Cannot mix ...`.
...
- The document you pass to `insertOne` / `insertMany` is a plain record. TypeScript requires no particular field, and the builder does not check the record against your contract.
...
- [`match()`](#match) filter values follow the same rule as the values you insert. The one value you cannot filter on this way is `_id`, which needs [`rawCommand()`](#rawcommand). The last example on this page shows it.
...
- `findOneAndUpdate()` and `findOneAndDelete()` return the document exactly as the driver gives it, so its `_id` is an `ObjectId` from the `mongodb` package even though the TypeScript type says `string`. To get the id as a string, write `String(result._id)`.
...
await runtime.query(plan);
// on a collection with a numeric views field and an array tags field:
