...
These examples assume the file is beside `contract.json` in `src/prisma/`, so from anywhere else, change the relative paths. Write `./contract.d` exactly like that, with no extension. The line `import contractJson from './contract.json' with { type: 'json' }`, which `prisma orm init` writes, needs Node.js 22.18 or newer, `resolveJsonModule: true` in `tsconfig.json`, and a `module` setting that accepts import attributes. `prisma orm init` writes `module: "preserve"` with `moduleResolution: "bundler"`, which needs TypeScript 5.4 or later; `esnext` and `nodenext` also accept the attribute from TypeScript 5.3. The [upgrade guide](/guides/upgrade-prisma-orm/postgresql#26-include-the-generated-types) gives the setting for each kind of project. TypeScript types every `process.env` value as possibly undefined, so the examples write `!` after it, the same way `prisma orm init` does.
