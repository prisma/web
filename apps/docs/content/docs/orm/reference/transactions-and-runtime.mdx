...
`db` is the client `postgres(...)` or `mongo(...)` returns. The runtime is the connected object you run queries on, and `db.runtime()` returns it. The import path `@prisma/orm-postgres/runtime` is a package name and is not that object.

This page documents the client lifecycle (create, connect, close), transactions, prepared statements, and per-query execution options across PostgreSQL and MongoDB. Each method below has a Remarks list. If a method behaves differently on PostgreSQL and MongoDB, or exists on only one of them, that list says so. Transactions are fully supported on PostgreSQL. They are not available on MongoDB. To run one there, pass your own `MongoClient` with the `mongoClient` option and use the driver's `session.withTransaction(...)`. See [Transactions on MongoDB](/orm/fundamentals/transactions#transactions-on-mongodb) for the code. For a task-oriented walkthrough, see the Fundamentals guide to [Transactions](/orm/fundamentals/transactions). For the ways to query inside a transaction, see the [ORM client reference](/orm/reference/orm-client), the [SQL query builder reference](/orm/reference/sql-query-builder), and the [raw queries reference](/orm/reference/raw-queries).
...
- Tell the client which database to use in one of three ways: `url`, `pg`, or `binding`. A `binding` is one object that names the source and carries it: `{ kind: 'url', url }`, `{ kind: 'pgPool', pool }`, or `{ kind: 'pgClient', client }`. Use it when your code picks the source at run time. Pass exactly one of `url`, `pg`, and `binding`, or leave all three out and pass one to [`connect()`](#connect).
...
- `verifyMarker: 'onFirstUse'`, the default, makes the first query check that the database was set up for your contract. If it was not, Prisma ORM logs a warning and runs the query anyway. The usual fix is to apply your migrations with `npx prisma db update`. `verifyMarker: false` skips the check. See [How migrations work](/orm/migrations/how-migrations-work).
...
| `extensions` | Array of extension packages | No | The npm packages for the PostgreSQL extensions your contract's types need. Install each one, then import it and put it in this array. |
...
- `.enums` holds the enum values from your contract, by PostgreSQL schema: `db.enums.public.Role`. `.nativeEnums` holds the values of enum types that exist in PostgreSQL itself, created with `CREATE TYPE`, also by schema. Look the type up by name in brackets: `db.nativeEnums.public['AalLevel'].values` is `['aal1', 'aal2']`. A PostgreSQL enum type has values and no separate names, so each value is its own name: `db.nativeEnums.public['AalLevel'].nameOf('aal1')` is `'aal1'`.
- `.contract` is the contract as an object, and `.context` is internal. [`connect()`](#connect), [`runtime()`](#runtime), [`transaction()`](#dbtransactioncallback), [`prepare()`](#dbpreparedeclaration-callback), and [`close()`](#close) are documented on this page.
...
These examples assume the file is beside `contract.json` in `src/prisma/`. From anywhere else, change the relative paths. Write `./contract.d` exactly like that, with no extension. The line `import contractJson from './contract.json' with { type: 'json' }` is what `prisma orm init` writes, and it needs Node.js 22.18 or newer and TypeScript set up as `prisma orm init` sets it up. TypeScript types every `process.env` value as possibly undefined, so the examples write `!` after it, the same way `prisma orm init` does.
...
- Returns a `Promise<Runtime>`. The runtime is the connected object that runs queries. It has five calls:
  ...
  - `runtime.prepare(...)` prepares a statement you run many times with different values. The client has a `prepare()` too, and the two do the same thing. See [Prepared statements (PostgreSQL)](#prepared-statements-postgresql).
...
- Calling `connect()` after the client has been closed rejects with an error whose `code` is `DRIVER.NOT_CONNECTED` (`Postgres client is closed`).
...
- Returns a `Promise<void>`. If a connection is being opened when you call it, `close()` waits for that to finish, then releases the pool it created.
- After `close()`, `connect()` rejects and `runtime()` throws an error whose `code` is `DRIVER.NOT_CONNECTED` (`Postgres client is closed`).
- The client only owns pools it created from a `url`. When you passed your own `pg` pool or client, close that instance yourself.
...
- Tell the client which database to use in one of four ways: `url`, `uri`, `mongoClient`, or `binding`. A `binding` is one object that names the source and carries it, in two kinds: `{ kind: 'url', url, dbName }` and `{ kind: 'mongoClient', client, dbName }`. Use it when your code picks the source at run time. The `uri` path uses the `url` kind.
...
- With `url`, the database name comes from the path of the connection string, as in `mongodb://host:27017/app`. Add `dbName` to override that name, or when the string has no name in its path. With `uri`, Prisma ORM never reads the database name from the string, so `dbName` is required. That is the only difference between `uri` and `url`.
...
| `middleware` | Array of middleware | No | Code that runs around every query. See [How middleware works](/orm/middleware/how-middleware-works). |
| `mode` | `'strict'` or `'permissive'` | No | Defaults to `'strict'`. Only your own middleware reads it. |
...
- Call `db.close()`, not `runtime.close()`. `db.close()` marks the client closed and then closes the `MongoClient` Prisma ORM created. After `runtime.close()` the client still accepts calls and then fails.
...
A transaction groups several writes into one unit: they all commit together, or they all roll back. Use `db.transaction(...)` in application code. If you are writing a function that takes a runtime as a parameter instead of the client, use `withTransaction(...)`. Open a connection and drive the transaction yourself when you need statements on one connection outside a transaction, such as a `SET` before you begin.
...
- `db.transaction(...)` takes the callback and nothing else. There are no `isolationLevel`, `timeout`, or `maxWait` options, and Prisma ORM offers no way to set the isolation level. Nothing retries a failed transaction for you. A transaction has no time limit by default. To set one, set PostgreSQL's `idle_in_transaction_session_timeout` on the connection.
...
- Do not let an [`AsyncIterableResult`](#asynciterableresult) escape the callback unread. A result you return and then read after the transaction has ended rejects with an error whose `code` is `RUNTIME.TRANSACTION_CLOSED` (see the warning after the examples). To act on a code, catch the error and compare `error.code` with the string. Every code is listed in the [error reference](/orm/reference/error-reference).
...
- Imported from `@prisma/orm-postgres/family-runtime`, along with the `Runtime` type. That path holds the SQL runtime API, the same one for every SQL database.
- The callback receives a transaction handle whose only methods are `query` and `execute`. Unlike `db.transaction(...)`'s `tx`, it has no `.orm` or `.sql`, so build your queries with a SQL builder you already hold, such as the client's `db.sql`.
...
async function addTags(runtime: Runtime) {
...
await addTags(db.runtime());
```

### Manual connection and transaction control

The lowest level: acquire a connection, open a transaction on it, commit or roll back yourself, and return the connection to the pool. Use it when you need statements on one connection outside a transaction, such as a `SET` you run with `connection.execute(...)` before you open the transaction.

#### Remarks

- `runtime.connection()` returns a dedicated connection. `connection.transaction()` opens a transaction on it. Run built queries with `transaction.query(...)` for rows or `transaction.execute(...)` for writes that return no rows, then call `transaction.commit()` or `transaction.rollback()`. Always `release()` the connection when done, to return it to the pool. Commit or roll back first: release it in a `finally` block, as the examples do, so a statement that throws does not leak the connection.
- `connection.destroy()` throws that connection away instead of returning it to the pool. It does not close the pool or the runtime, and the runtime opens a replacement connection on the next query. Use `destroy()` only for a connection you no longer trust.

#### Options

These are the methods, not options.
...
- The declaration maps each parameter name to a type id, for example `{ label: 'pg/text@1' }`. A type id is the PostgreSQL type plus a version, always `@1` today, such as `pg/int4@1`, `pg/uuid@1`, or `pg/bool@1`. A few ids carry a longer name, such as `pg/timestamptz-temporal@1` for a `DateTime`. To take one straight from a column, read `db.sql.public.user.columns.id.codecId`, which is that id as a string: `prepare({ id: db.sql.public.user.columns.id.codecId }, ...)`. For the ids the common JavaScript values get, see the [list on the raw queries page](/orm/reference/raw-queries#binding-a-bare-value-with-param).
...
- You must pass `target`. It can be a runtime, a connection, or a transaction. One prepared statement runs against any of them.
...
- An abort that lands after rows have started arriving ends the stream with the same `RUNTIME.ABORTED` error. `details.phase` says where the abort landed: `'encode'`, `'decode'`, `'stream'`, `'beforeQuery'`, `'beforeExecute'`, or `'afterQuery'`.
- Inside `db.transaction(...)` the aborted statement throws, so your callback throws and the transaction rolls back.
- `scope` is set for you. Do not pass it.
...
- `telemetry()` returns `null` on a freshly-connected runtime, before any query has run. After a query, it returns an object of the shape `{ lane, target: 'postgres', fingerprint, outcome, durationMs? }`. `lane` is the name of the API you used, such as `orm-client` for `db.orm` or `raw` for `db.raw.sql`. Every run of the same query text shares one `fingerprint`, whatever values you pass, so you can group runs of one query together. `outcome` is `'success'` or `'runtime-error'`. `durationMs` is how long the query took, and it is present for every query the runtime ran. The object reflects only the **most recent** query, not a running history.
...
Use `await` unless you want to handle rows as they arrive. `for await` does not reduce memory on PostgreSQL: every row is loaded first.
