...
- The ORM query is built once. Each `query(target, params)` call runs it with the new values and returns what the same call without `.prepared` returns:
  - `all()` returns the rows.
  - `first()` returns a row, or `null`.
  - `aggregate(...)` returns the aggregate object.
  - `groupBy(...)` followed by `aggregate(...)` returns an array with one object per group.
...
In an ORM query, a prepared parameter works in `where(...)` filters, in the callback you pass to `include(...)`, and in `.limit(...)` and `.offset(...)`, so the same statement can page through results. Declare a page size or offset with a number type id that is not nullable, such as `{ take: 'pg/int4@1' }`, and pass `params.take` to `.limit(...)`. A nullable or text type id there is a type error.
