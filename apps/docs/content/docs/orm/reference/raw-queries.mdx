...
Each method returns a buildable command: call `.build()` to get a plan, then run it with `(await db.runtime()).query(plan)`. On MongoDB `db.runtime()` returns a promise, so you await it; on PostgreSQL it is synchronous. Because raw filters carry native BSON values, the examples below construct ids with the driver's `ObjectId` class:
...
- The MongoDB command AST carries `sort` and `returnDocument` fields, but the public `findOneAndUpdate()` wrapper does **not** expose them. Its options argument is `{ upsert?: boolean }` and nothing else, so TypeScript rejects an options literal that names either key, and a value that reaches the call untyped is dropped without a run-time error. A caller who needs either must build the raw command directly with `new RawFindOneAndUpdateCommand(...)` and run it through `db.query.rawCommand(...)`, bypassing this collection wrapper.
