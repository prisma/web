...
For task-oriented walkthroughs, see the Fundamentals guides: [Reading data](/orm/fundamentals/reading-data), [Writing data](/orm/fundamentals/writing-data), [Relations and joins](/orm/fundamentals/relations-and-joins), and [Transactions](/orm/fundamentals/transactions). If you are coming from Prisma ORM 7, [Coming from Prisma ORM 7](/orm/coming-from-prisma-orm-7) lists the packages to install and maps `PrismaClient` and `findMany` onto the methods on this page.

## Example schema

All examples on this page run against the schema below. The grouped-aggregate examples use a separate `Customer` / `Order` schema, shown under [Grouped aggregates](#grouped-aggregates).

Write this schema in `contract.prisma`, the file that replaces `schema.prisma` from Prisma ORM 7. This page calls that file your contract. After every change to it, run `npx prisma contract emit`. That command writes `contract.json` and `contract.d.ts` next to `contract.prisma`, and the examples below import both. A new project keeps all three files in `src/prisma/`.
...
- `@@type("pg/text@1")` on an `enum` block stores the enum as a text column. The name inside is a PostgreSQL type plus a version, and the version is always `@1` today. The MongoDB form of the same attribute is `@@type("mongo/string@1")`. This page calls an enum written this way a text-backed enum.
- A `native_enum` block stores the same members as a real PostgreSQL enum type instead. Write `native_enum` where you would write `enum`, and give every member a value, as in `Urgent = "urgent"`. A member with no value is rejected. This page calls an enum written this way a native enum. The two sort differently; see [`orderBy()`](#orderby).
...
- A variant is a model that reuses another model's fields and rows. `@@base(Task, "bug")` makes `Bug` a variant of `Task`. `@@discriminator(type)` names the column that records which variant each row is. `variant('Bug')` narrows a collection to that variant. Pass the variant's model name, not the string in `@@base`.
- `PostTag` is required. `Post.tags Tag[]` and `Tag.posts Post[]` are joined through `PostTag` because `PostTag` has a foreign key to each of the two models and its `@@id` lists exactly those two foreign key columns. A pair of list fields with no such model is rejected. See [Relations and joins](/orm/fundamentals/relations-and-joins#many-to-many).
...
Create a client with `postgres(...)` or `mongo(...)`, then read and write your models through `db.orm`. `db.orm` is where your models are. The same `db` also has a query builder, `db.sql` on PostgreSQL and `db.query` on MongoDB, and `db.raw`, which this page does not cover. Create `db` once per process, and close it with `await db.close()` when the process shuts down. The two databases have different entry points, and they name the models differently.

### PostgreSQL

Create a PostgreSQL client with `postgres(...)`. `db.orm` holds your models by model name, grouped by database schema: `db.orm.public.User`, `db.orm.public.Post`. `public` is the PostgreSQL schema your tables are in unless you put them in another one. The import path `./contract.d` is written exactly like that, with no `.ts` on the end, and it resolves to the emitted `contract.d.ts`.
...
To give a model your own methods on top of the built-in ones, import `Collection` from `@prisma/orm-postgres/orm-client`, subclass it, and create the client with `orm(...)` from that same package instead of `postgres(...)`. [Custom `Collection` subclass](#custom-collection-subclass) has the code.
...
The examples below follow each database's own form: `db.orm.public.User` on PostgreSQL, `db.orm.users` on MongoDB.

## Query-building methods

These methods narrow a query. Each returns a collection. On this page, a collection is the query object you chain more methods on, such as `db.orm.public.User` or `db.orm.public.User.where(...)`. On MongoDB the accessor name is also the name of the database collection the documents are in, which is why `db.orm.users` reads the way it does. You run the query with a [read method](#read-terminals) such as `all()` or `first()`. MongoDB supports fewer of these methods than PostgreSQL, and each method's Remarks say which databases it works on.
...
- On MongoDB, `select()` changes what the database returns but not the TypeScript type. Only the fields you named come back, while the type still lists every field on the model. Treat the fields you did not name as absent, whatever the type says.
...
- On MongoDB, a document's own `_id` comes back as a hex string. The `_id` on a related document loaded by `include()` comes back as an `ObjectId` object instead. Wrap that one in `String(...)` before you compare it.
...
On PostgreSQL, the callback you pass to `include()` receives the related rows as a collection. The heading calls that callback a refinement. You can filter, order, and paginate the related rows in it. You can also reduce them to a single value, or use `combine()` to return several results at once.
...
- `sum()` and `avg()` take a numeric column: an integer, a floating-point number, or a decimal. They also take an interval or a `Time` column. They do not take a date or a timestamp. TypeScript rejects a date or timestamp field name here.
...
- `avg()` over an integer column returns a `number`, which rounds. `avgDecimal(field)` returns the exact average as a decimal string. Pass that string to a decimal library, or call `Number(...)` on it when rounding is fine.
- When a to-many relation has no rows, `sum()`, `avg()`, `min()`, and `max()` come back as `null`, not `0`.
- `combine(shape)` takes an object. You choose the keys. Build every value from the collection the callback was given, which is `posts` in the example below. A value that is `posts` with more methods chained on it comes back as an array of rows. A value that is an aggregate, such as `posts.count()`, comes back as that single value.
...
`min('createdAt')` over a `DateTime` column compiles. It returns a `Temporal.Instant`, the standard JavaScript object for a point in time. Not every JavaScript engine has `Temporal` yet. Check with `typeof Temporal === 'undefined'`, and if yours does not, add a polyfill: `import 'temporal-polyfill/full/global'`.

`sum('createdAt')` and `avg('createdAt')` do not compile. TypeScript rejects `'createdAt'`, because PostgreSQL has no `SUM` or `AVG` over a date or a timestamp.
:::
...
- Always call `orderBy()` before `cursor()`. TypeScript rejects the call if you do not. Nothing checks this while the query runs, so a cursor on an unordered query is ignored and every row comes back with no error.
- The cursor object must name every column the `orderBy()` sorts on. Leave one out and the call throws an error whose `code` is `ORM.CURSOR_VALUE_MISSING`.
...
// priorities holds one row per distinct priority value, such as { priority: 'low' }
```
...
- On PostgreSQL, a variant that sets its own `@@map`, as `Bug` and `Feature` do, is stored in a second table beside the base model's table. A variant without `@@map` is stored in the base model's table. That changes how some writes behave; see [`createAndCount()`](#createandcount) and [`upsert()`](#upsert).
...
There is no `count()` method on either database. On PostgreSQL, count with `aggregate((a) => ({ n: a.count() }))`. On MongoDB, count with `db.query`, the [pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder). It is already on your client, so there is nothing extra to import. Both are shown under [Count records](/orm/fundamentals/reading-data#count-records).
...
- Each result can be used one way only. Awaiting it and then looping it with `for await`, or the reverse, throws an error whose `code` is `RUNTIME.ITERATOR_CONSUMED`. The rules for awaiting versus looping are under [Single consumption and mode switching](#single-consumption-and-mode-switching).
...
`first()` returns `null` on no match and does not check that only one row matched. For the throwing forms, `findUniqueOrThrow` and `findFirstOrThrow` map onto `all().firstOrThrow()`, which throws an error whose `code` is `RUNTIME.NO_ROWS`.
...
- `db.orm` cannot use your subclass. Build a second accessor by calling the `orm(...)` function with a `collections` object, and use that accessor for the models you subclassed.
- The accessor `orm(...)` returns has the same methods as `db.orm.public`. You keep the `postgres(...)` client as well, for `connect()` and for the `context` you pass to `orm(...)`.
...
Always call `where()` before an update or a delete. On MongoDB all six methods check for a filter and throw an error whose `code` is `ORM.WHERE_MISSING`, with a message naming the method you called, such as `updateAll() requires a .where() filter. Call .where() before .updateAll()`. On PostgreSQL nothing checks at run time. TypeScript refuses to compile these methods without a `where()`, so the risk is code that escapes the type check, such as JavaScript or a cast to `any`. Without a filter, `update()` and `delete()` change a single row and you cannot predict which one, and `updateAll()` and `deleteAll()` change every row.
:::
...
- On MongoDB, `create()` returns the values you passed plus the `_id` the server assigned, not the stored document. On PostgreSQL the returned row is read from the database.
...
On PostgreSQL, that object can also carry a `create` or `connect` callback for a related row, as shown below.
...
- Available for PostgreSQL and MongoDB.
- On PostgreSQL, `createAndCount()` does not work on a variant that is stored in its own table. Use `createAll()` instead.
- A variant is stored in its own table when its `@@map` names a different table than its base model does. `Bug` in the [example schema](#example-schema) is one: it maps to `bug` while `Task` maps to `task`.
- Calling it on such a variant throws an error whose `code` is `ORM.OPERATION_UNSUPPORTED`, with the message `createAndCount() is not supported for MTI variant "Bug" on model "Task". Use createAll() instead.` The message calls it an MTI variant, short for multi-table inheritance.
...
- On MongoDB, the update input covers the base model's fields only. A field declared on a variant, such as `duration` on `Tutorial`, is a TypeScript error even though it runs correctly. Put `// @ts-expect-error` on that line, as shown under [Field update operations](#field-update-operations).
...
- Returns `null` when no row matches. On PostgreSQL it also returns `null` when what you pass has no fields to change.
...
```typescript
// select() and include() shape the row you get back, nothing else.
...
- On MongoDB, `updateAll()` is three database operations. First it collects the ids of the documents that match your filter. Then it updates every document that matches that filter, not only the ids it collected. Then it reads back the documents whose ids it collected.
- Two things follow from that. A document that starts matching only after the first step is updated but not returned. A document that someone else changes after the update comes back with their values.
- Prisma ORM has no `db.transaction()` on MongoDB, so you cannot avoid this from the client. To control it, share one `MongoClient` with the MongoDB driver and group the writes in a driver session, shown under [Transactions on MongoDB](/orm/fundamentals/transactions#transactions-on-mongodb).
...
const count = await db.orm.posts
  .where({ authorId: aliceId })
  .updateAndCount((p) => [p.content.set('Rewritten')]);
```
...
- `conflictOn` takes one unique column with a value, such as `conflictOn: { label: 'typescript' }`. Only the column name is used. The value is required by the type and is then ignored, so pass any value of the right type.
...
- That advice is the library's own. `createAll()` inserts rows and has no update half, so use it when the rows are new.
...
| `conflictOn` | Object with one unique column and a value, PostgreSQL only | No | The column that decides insert or update. Omit to use the primary key. |

Only the column name in `conflictOn` is used. The value it is paired with is ignored.
...
const inserted = await db.orm.public.Tag.upsert({
  ...
  conflictOn: { label: 'ignored' }, // only the column name `label` is used
});
...
const updated = await db.orm.public.Tag.upsert({
  ...
  conflictOn: { label: 'ignored' }, // the same value works here, because it is ignored
});
```
...
PostgreSQL supports aggregation over a result set, both flat (`aggregate()`) and grouped (`groupBy().aggregate()`), with `having()` to filter groups. Neither method exists on a MongoDB collection. To aggregate on MongoDB, use the pipeline builder, which is this library's API for MongoDB's own aggregation pipeline. It is described in [MongoDB: Pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder).
...
- `sum()`, `avg()`, `min()`, and `max()` resolve to `null`, not `0`, over an empty result set. Handle the `null`. `sum('amount')` over an `Int` column is typed `number | null`.
...
The callback's `agg` argument gives you `count()`, `count(field)`, `sum(field)`, `avg(field)`, `min(field)`, and `max(field)`. It also gives you three exact variants, for numbers too large or too precise to hold in a JavaScript `number`. `agg.countBigInt()` takes no argument and returns a `bigint`. `agg.sumBigInt('amount')` returns a `bigint`. `agg.avgDecimal('amount')` returns the exact average as a string, such as `'10.3333333333333333'`. You import nothing to use these.
...
- `orderBy(...)` on a grouped collection sorts the groups by a field you grouped on. You cannot sort groups by an aggregate value. To do that, write the query with the SQL query builder (`db.sql`), described in [Advanced queries](/orm/fundamentals/advanced-queries).
...
| Return type | Example | Description |
|---|---|---|
| `GroupedCollection` | `db.orm.public.Order.groupBy('customerId')` | Not a promise. Call `.aggregate(...)` on it to run the query. |
...
- PostgreSQL only. Not available on MongoDB. Call it on the result of `groupBy()`, before `aggregate()`.
...
| Return type | Example | Description |
|---|---|---|
| `GroupedCollection` | `db.orm.public.Order.groupBy('customerId').having(...)` | The same grouped collection with this condition added. Call `.aggregate(...)` to run the query. |
...
A condition says which rows match. You write one inside `where()`, and inside a relation filter such as `u.posts.some(...)`. PostgreSQL and MongoDB write conditions differently.

- **PostgreSQL** compares a field with a method on that field, such as `u.email.eq(...)`, inside a callback. To combine conditions, import `and`, `or`, `not`, and `all` from `@prisma/orm-postgres/orm-client`.
- **MongoDB** builds each condition by calling a method on the `MongoFieldFilter` class itself, such as `MongoFieldFilter.eq('name', 'Alice')`. The full list is under [`MongoFieldFilter`](#mongofieldfilter). To combine conditions, use `.and()`, `.not()`, and `MongoOrExpr.of([...])`, described under [Combinators](#combinators).
...
- PostgreSQL. Which methods a field has depends on its type, and your editor's autocomplete is the answer: if it does not offer a method on that field, the type does not support it. `isNull` and `isNotNull` are on every field. `eq`, `neq`, `in`, and `notIn` need a type that can be compared for equality. `gt`, `lt`, `gte`, and `lte` need a type that can be ordered. `like` and `ilike` need a text type.
...
const withoutDescription = await db.orm.public.Task.where((t) => t.description.isNull()).all(); // description is String?
```
...
- On MongoDB, `.and(other)` and `.not()` are **methods on a condition you already built**, such as `MongoFieldFilter.eq('name', 'Alice').and(...)`. There is **no `.or()` method**. To OR conditions, build `MongoOrExpr.of([...])`.
- On MongoDB, `.and(other)` takes exactly one condition. For three, chain it: `a.and(b).and(c)`. The result is a `MongoAndExpr`, which you can also build directly with `MongoAndExpr.of([a, b, c])`.
...
- `some()` is also the method for a to-one relation, such as `Post.user`, which holds a single related row. There it reads as "the related row matches". It works from both sides of a relation, so `u.posts.some(...)` and `p.user.some(...)` are both valid.
...
- On PostgreSQL, a field whose type cannot be compared for equality throws an error whose `code` is `ORM.FILTER_UNSUPPORTED`. Filter that field with a comparison method instead.
...
Helpers that build one MongoDB filter condition. Use them for anything other than equality on a top-level field, which `.where({ name: 'Alice' })` already covers. To join two conditions with AND or OR, see [Combinators](#combinators).

#### Remarks

- MongoDB. Import from `@prisma/orm-mongo/query-ast/execution`. `MongoExistsExpr` is in the same module.
- The factories are `of`, `eq`, `neq`, `gt`, `lt`, `gte`, `lte`, `in`, `nin`, `isNull`, and `isNotNull`. There is no `ne`; use `neq`.
- `of(field, operator, value)` takes the MongoDB operator written out as a quoted string, such as `'$regex'`. Use it for operators the named factories do not cover, such as `MongoFieldFilter.of('tags', '$size', 3)`. It passes the operator string to MongoDB unchanged.
- `isNull(field)` matches documents where the field is `null` and documents where the field is missing. `isNotNull(field)` is its opposite. To test only whether a field is present, use `MongoExistsExpr.exists(field)` or `MongoExistsExpr.notExists(field)`. Those two are all `MongoExistsExpr` offers.
- There is no `regex`, `elemMatch`, `all`, or `size` factory. Write those operators through `of`.

#### Options

| Method | Type | Description |
|---|---|---|
| `of(field, operator, value)` | Field + MongoDB operator string + value | Any operator, written out, such as `'$regex'`. |
| `eq(field, value)` / `neq(field, value)` | Field + value | Equality / inequality. |
| `gt(field, value)` / `lt(field, value)` / `gte(field, value)` / `lte(field, value)` | Field + a value that can be compared, such as a number, a date, or a string | Greater than, less than, greater or equal, less or equal. |
| `in(field, values)` / `nin(field, values)` | Field + array | Membership / exclusion. |
| `isNull(field)` / `isNotNull(field)` | Field | Matches a `null` value and a missing field / neither of those. |
...
- MongoDB. Use `MongoFieldFilter.eq('address.city', 'San Francisco')`. It takes the path as a plain `string` and needs no cast.
- The plain object form, `.where({ 'address.city': 'San Francisco' })`, does the same thing when it runs, but it does not compile without a cast. The object you pass to `.where()` is typed against the model's own top-level field names, and a dotted key is not one of them.
...
The object form works at runtime too, with a cast. The cast goes through `unknown` because TypeScript refuses a direct cast between two types that do not overlap:
...
On MongoDB, `update()`, `updateAll()`, `updateAndCount()`, and `upsert()` accept a callback in place of a data object. The callback receives one argument. The examples below call it `u` or `t`, and the name is yours to pick. For a top-level field, read it as a property and call an operation on it: `u.bio.set('Hello')`. For a field inside an embedded object, call the argument as a function with the path: `u('address.city').set('San Francisco')`. Return an array of the calls you made. PostgreSQL has no callback form: pass an object to [`update()`](#update) instead.

All four methods require `.where()`. Without one they throw an error whose `code` is `ORM.WHERE_MISSING`. `update()` returns the updated document, or `null` when nothing matched. `updateAll()` returns an [`AsyncIterableResult`](#asynciterableresult) of the changed documents. `updateAndCount()` returns the number of documents it changed.

### Available operations (MongoDB)

#### Remarks

- MongoDB only. `set()`, `unset()`, `inc()`, and `mul()` are available.
...
- After `variant(...)`, TypeScript still knows only the base model's fields, so `t.duration.inc(10)` is a type error even though it runs correctly. Put `// @ts-expect-error` on the line directly above the `.update(...)` line, as the example below shows. If the type error ever goes away, TypeScript reports the unused comment instead.
...
`updateAndCount()` takes the callback in the same place. `upsert()` takes it as its `update` key, beside `create`: `upsert({ create: { ... }, update: (u) => [u.bio.unset()] })`. That callback cannot use a dot path. Given one, `upsert()` throws an error whose `code` is `ORM.OPERATION_UNSUPPORTED`.
...
The callback argument implements `set`, `unset`, `inc`, `mul`, `push`, `pull`, `addToSet`, and `pop` only. `min`, `max`, `rename`, and `currentDate` are absent from the API, and TypeScript rejects them: `rename` is not a property of `u.bio`. Do not use `min()`, `max()`, `rename()`, or `currentDate()`.
:::
...
- MongoDB. `push()`, `pull()`, `addToSet()`, and `pop()` are available on the callback argument. `push(value)` appends one element. `addToSet(value)` appends it only if it is not already there. `pop(1)` removes the last element, and `pop(-1)` removes the first.
...
- These operations need an array field, declared with `[]` in your contract, as in `tags String[]`. Applied to any other field, the write fails with an error from MongoDB rather than a Prisma ORM error code.
- The four lines below are separate calls because one update cannot apply two operations to the same field. One update can still change two different fields, such as pushing to `tags` and setting `bio`. The example schema has no array field, so these lines assume `tags String[]` on `User`.
...
`contract.d.ts` exports a `Models` namespace with one type per model. On PostgreSQL the schema is part of the name, `Models.public_User`; on SQLite, which has no schemas, it is `Models.User`. The same file exports a `models` constant for dotted access, `typeof models.public.User`. A model type carries every scalar field and every relation, so no query returns it directly. Three types derive what queries do return:

- `Scalars<M>` is the model without its relations: what `all()` and `first()` return when you do not `select` or `include`.
- `Shape<M, Spec>` is a row with chosen fields and relations. In `Spec`, `"+"` names scalars and relations to keep, `"-"` names scalars to drop, and any other key is a relation with its own nested spec. Naming only relations in `"+"` keeps every scalar.
- `ResultType<typeof query>` is one row of a query you already wrote.

`Scalars` and `Shape` come from `@prisma/orm-postgres/family-contract/types` (`@prisma/orm-mongo/family-contract/types` on MongoDB); `ResultType` comes from `@prisma/orm-postgres/components/runtime`.
...
// { id: number; name: string | null; posts: { id: number; title: string }[] }
...
Use `Shape` to declare a function's return type once and let the compiler check the body at the `return`. Wrong field names, a relation under `"-"`, or a scalar in `"+"` next to a `"-"` are compile errors on that key. These types replace Prisma ORM 7's `Prisma.User` and `Prisma.UserGetPayload<...>`.
...
- Switching from one way to the other throws an error whose `code` is `RUNTIME.ITERATOR_CONSUMED`. The message contains `already been consumed`. Awaiting a result and then looping it with `for await`, or the reverse, is what throws. There is no error class to test with `instanceof`: import `isRuntimeError` from the same module and check `error.code`.
...
`contract.d.ts` exports a `Models` namespace and a `models` constant with one member per model, named `<namespace>_<Model>` (`Models.public_User`, or `typeof models.public.User`; bare model names on SQLite). Two helpers from `@prisma/orm-postgres/family-contract/types` turn a model into a row type without a client in scope: `Scalars<M>` is the row a default fetch returns, and `Shape<M, Spec>` derives a data structure with chosen scalars and nested relations (`'-'` drops fields, `'+'` on a relation picks its fields). `ResultType` from `@prisma/orm-postgres/components/runtime` names what a query returns.
...
These need `8.0.0-rc.10` or newer. A `contract.d.ts` emitted by an earlier release lacks the `Models` namespace until you run `prisma contract emit` again. On MongoDB, import `Scalars` and `Shape` from the `@prisma/orm-mongo` equivalent of the types entrypoint.
...
- `aggregate((agg) => ({ ... }))` resolves to a **single object** keyed by your aliases. `db.orm.public.Order.aggregate((agg) => ({ total: agg.count() }))` resolves to one object, such as `{ total: 10 }`.
...
- `count()` is always a `number` (`0` over an empty set). `sum()`, `avg()`, `min()`, and `max()` resolve to `null`, not `0`, over an empty result set, so their TypeScript type includes `null`.
