...
Create a MongoDB client with `mongo(...)`. `db.orm` holds your models by collection name, with no schema in between: `db.orm.users`, `db.orm.posts`, not `db.orm.User`. The collection name is the one you set with `@@map`, or the model name exactly as written when you did not set one. `dbName` is the MongoDB database name.
...
There is no `count()` method on either database, so on PostgreSQL, count with `aggregate((a) => ({ n: a.count() }))`. On MongoDB, counting takes two steps: `db.query`, the [pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder), builds the pipeline, and `runtime.query(...)` runs it.
...
- On PostgreSQL and SQLite, `{ onConflict: 'skip' }` as the second argument skips each row that collides with a unique constraint, and the result holds only the rows the database wrote. `conflictOn: ['email']` names the unique constraint by its columns; without it, a collision on any unique constraint skips the row. A variant stored in its own table refuses the option with `ORM.OPERATION_UNSUPPORTED`. The option needs capabilities that a contract emitted before `8.0.0-rc.12` does not list, so run `npx prisma contract emit` once first; otherwise it fails with `ORM.CAPABILITY_MISSING`.

#### Options

| Name | Type | Required | Description |
|---|---|---|---|
| The rows | Array of field objects | Yes | The rows to insert, passed as the first argument. There is no `data` wrapper. |
| `onConflict` | `'skip'` | No | Skip rows that collide with a unique constraint. PostgreSQL and SQLite. |
| `conflictOn` | Array of field names | No | With `onConflict`, the columns of the unique constraint to check. |
...
- It returns the number of rows the database inserted. With `{ onConflict: 'skip' }`, the skipped rows are not counted. The option works as on [`createAll()`](#createall).
...
These methods exist only on PostgreSQL collections. `aggregate()` treats every row the query matches as one group, while `groupBy().aggregate()` aggregates per group and `having()` filters those groups. A MongoDB model's collection has none of them, so to aggregate on MongoDB, use the pipeline builder, which is Prisma ORM's API for MongoDB's aggregation pipeline and is described in [MongoDB: Pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder).
...
- `String` fields and enums stored as text have all of them. A native enum field (`pg.enum(...)`) has all but `like` and `ilike`, because PostgreSQL has no pattern match for an `enum` type. `Uuid` and `DateTime` have all but `like` and `ilike`. A `Boolean` field has only the equality methods, `isNull()`, and `isNotNull()`. A `Json` field has only `isNull()` and `isNotNull()`.
...
- PostgreSQL. A text field has `fullTextMatches(query, options?)` to filter and `fullTextRank(query, options?)` to sort by relevance. The [SQL query builder](/orm/reference/sql-query-builder) has the same operations as `fns.fullTextMatches(column, query)`, `fns.fullTextRank(column, query)`, and `fns.fullTextHeadline(column, query)`, which returns the matching text with each match marked.
- `query` is a `tsquery`, not a string, and a bare string is a type error. Build it with a parser from `@prisma/orm-postgres/target/full-text`: `websearchToTsquery(text)` reads search-box text, `plaintoTsquery(text)` requires every word, and `phrasetoTsquery(text)` requires the words in order. `toTsquery(text)` takes PostgreSQL's operator syntax as written, so never pass user input to it.
- For user input inside operator syntax, such as a prefix match while the user types, use the `tsquery` template tag from the same package: `` tsquery`${term}:*` ``. It turns each interpolated value into one quoted term, so the input cannot add operators. Do not put quotes around the interpolation yourself.
- `language` sets the text-search configuration and defaults to `english`. The operation's `language` applies to the field, and each parser, and the tag as `` tsquery({ language })`...` ``, takes its own `language` for the query. Pass the same value to both.
- Add [`@@fullTextIndex`](/orm/contract-authoring/psl-syntax#full-text-indexes) to the model, with the same `language`, so PostgreSQL can use an index for the search. Without it the query still works, with a sequential scan.
- A native enum field (`pg.enum(...)`) has no full-text operations.
...
| `fullTextRank(query, options?)` | `language`, `normalization` (0 to 63), `coverDensity` | The relevance of the match, for `orderBy()`. `coverDensity: true` uses `ts_rank_cd` in place of `ts_rank`. |

#### Examples

##### Search and sort by relevance

```typescript title="src/search-posts.ts"
