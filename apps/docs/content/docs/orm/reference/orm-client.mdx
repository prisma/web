...
`Scalars` and `Shape` come from `@prisma/orm-postgres/family-contract/types` (`@prisma/orm-mongo/family-contract` on MongoDB); `ResultType` comes from `@prisma/orm-postgres/components/runtime`.
...
Use `Shape` to declare a function's return type once and let the compiler check the body at the `return`. Wrong field names, a relation under `"-"`, or a scalar in `"+"` next to a `"-"` are compile errors on that key. These types replace Prisma ORM 7's `Prisma.User` and `Prisma.UserGetPayload<...>`. They need `8.0.0-rc.10` or newer; a `contract.d.ts` emitted by an earlier release lacks the `Models` namespace until you run `prisma contract emit` again.
