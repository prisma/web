...
- On MongoDB, the `_id` of a loaded related document comes back as a driver `ObjectId`, not as the hex string a document's own id field comes back as. Wrap it in `String(...)` before you compare it.
- On MongoDB, TypeScript rejects a second argument to `include()`. If you get past the type system, the extra argument is ignored and the relation loads unfiltered.
...
- `sum()` and `avg()` take a numeric column: an integer, a floating-point number, or a decimal. They also take an interval or a `Time` column. They do not take a date or a timestamp: TypeScript rejects the field name, because the fields these two accept leave those columns out.
...
- `avg()` over an integer column returns a `number`, which rounds. `avgDecimal(field)` returns the exact average instead, as a decimal string.
...
`min('createdAt')` over a `DateTime` column compiles, and it returns a `Temporal.Instant` (the standard JavaScript date and time object; if your runtime has no global `Temporal`, import one, for example `import 'temporal-polyfill/full/global'`). `sum('createdAt')` and `avg('createdAt')` do not compile, because PostgreSQL has no `SUM` or `AVG` over a date or a timestamp. TypeScript rejects `'createdAt'` because it is not one of the fields those two accept.
:::
...
- On PostgreSQL, a variant that sets its own `@@map`, as `Bug` and `Feature` do, keeps its own fields in a second table alongside the base model's table. A variant without `@@map` keeps them in the base model's table. That changes how some writes behave; see [`createAndCount()`](#createandcount) and [`upsert()`](#upsert).
...
Always call `where()` before an update or a delete. On MongoDB you get an error if you forget, with the message `updateAll() requires a .where() filter. Call .where() before .updateAll()`. On PostgreSQL, TypeScript refuses to compile any of these methods without a `where()`, but nothing checks at run time. Once the type check is bypassed, `update()` and `delete()` change one arbitrary row, and `updateAll()` and `deleteAll()` change every row.
:::
...
- Calling it on such a variant throws an error whose `code` is `ORM.OPERATION_UNSUPPORTED`, with the message `createAndCount() is not supported for MTI variant "Bug" on model "Task". Use createAll() instead.`
...
- On MongoDB, `update()` takes a data object or a callback that returns field operations, such as `(p) => [p.content.set('Rewritten')]`. See [Field update operations](#field-update-operations).
- On MongoDB, the update input covers the base model's fields only. A field declared on a variant, such as `duration` on `Tutorial`, is not in it. [Field update operations](#field-update-operations) shows the workaround.
- On PostgreSQL, `update()` takes a data object only. Passing a function changes nothing and returns `null`; pass an object.
...
- Returns `null` when no row matches. On PostgreSQL it also returns `null` when the data you pass has no fields to change, which is what happens if you pass a function by mistake.
...
- On PostgreSQL, `upsert()` does not work on a multi-table inheritance variant, the same restriction as [`createAndCount()`](#createandcount). It throws an error whose `code` is `ORM.OPERATION_UNSUPPORTED`, with the message `upsert() is not supported for MTI variant "Bug" on model "Task". Use createAll() instead.`
- On MongoDB, the `_id` on the returned row is decoded the same way as on a read, so you get a hex string, not the driver's `ObjectId`.
...
- `sum()`, `avg()`, `min()`, and `max()` resolve to `null`, not `0`, over an empty result set. This is standard SQL: aggregating zero rows yields `NULL`. Their TypeScript return type carries `| null` for this reason, so `sum('amount')` over an `Int` column is `number | null`.
...
The selector's `agg` gives you `count()`, `count(field)`, `sum(field)`, `avg(field)`, `min(field)`, and `max(field)`. It also gives you `countBigInt`, `sumBigInt`, and `avgDecimal`, which answer with a `bigint` or an exact decimal instead of a `number`.

#### Return type

| Return type | Example | Description |
|---|---|---|
| Object of aggregate results | `{ total: 10 }` | One object with the requested aliases. `sum`, `avg`, `min`, and `max` can be `null`. |
...
- A to-one relation is filtered the same way, from either side: `p.user.some(...)`.
...
- The factories `exists`, `regex`, `elemMatch`, `all`, and `size` do **not** exist on `MongoFieldFilter`. Write those operators through `of`, which passes the operator string to MongoDB unchanged.
...
On MongoDB, `update()`, `updateAll()`, `updateAndCount()`, and `upsert()` accept a callback in place of a data object. The callback receives one argument, written `u` or `t` below, with one property per field: `u.bio`, `t.duration`. Call an operation on that property, such as `u.bio.set('Hello')`. The argument is also a function, so `u('address.city').set('San Francisco')` reaches a field inside an embedded object. Each call returns an update instruction; collect them in the array you return. PostgreSQL's `update()` has no callback form (see [`update()`](#update)).

All four methods require `.where()`. `update()` returns the updated document, or `null` when nothing matched. `updateAll()` returns the changed documents. `updateAndCount()` returns the number of documents it changed.
...
- MongoDB only.
- `set()`, `unset()`, `inc()`, and `mul()` are available.
...
`updateAndCount()` takes the callback in the same place. `upsert()` takes it as its `update` key, beside `create`: `upsert({ create: { ... }, update: (u) => [u.bio.unset()] })`. That callback cannot use a dot path, and `upsert()` rejects one.
...
- MongoDB. `push()`, `pull()`, `addToSet()`, and `pop()` exist on the callback argument. `push()` and `pull()` are tested against a live MongoDB. `addToSet()` and `pop()` are covered only by unit tests that check the operator they build.
- `push(value)` appends one element. `addToSet(value)` appends it only if it is not already there. `pull(match)` removes every element equal to the argument, or every element matching a partial object. `pop(1)` removes the last element, and `pop(-1)` removes the first.
- Applied to a field that is not an array, MongoDB rejects the write with its own error, not a Prisma ORM error. An array field is declared with `[]` in your contract, as in `tags String[]`.
- One update cannot touch the same field twice, so the lines below are four separate writes. The example schema has no array field, so they assume `tags String[]` on `User`.
...
- Switching from one way to the other throws an error whose `code` is `RUNTIME.ITERATOR_CONSUMED`. The message contains `already been consumed`. Awaiting a result and then looping it with `for await`, or the reverse, is what throws.
...
- `count()` is always a `number` (`0` over an empty set). `sum()`, `avg()`, `min()`, and `max()` carry `| null`: they resolve to `null`, not `0`, over an empty result set.
