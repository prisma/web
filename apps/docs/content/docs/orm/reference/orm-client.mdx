...
These methods narrow a query. They return a collection you can chain further methods on, and you run the query with a [read method](#read-terminals) such as `all()` or `first()`. MongoDB supports a smaller set than PostgreSQL; each method's Remarks state its availability.
...
|---|---|---|
| `Collection` | `db.orm.public.User.where(...)` | A collection narrowed by the filter, chainable; a read or write method runs it. |
...
A read method runs the query and gives you the rows. `all()` and `first()` are available on both databases; `aggregate()` and `groupBy()` are PostgreSQL only and are documented under [Grouped aggregates](#grouped-aggregates). For a task-oriented walkthrough, see [Reading data](/orm/fundamentals/reading-data).
...
For Prisma ORM 7 users, `upsert()` replaces the `where` conflict target with `conflictOn` (PostgreSQL):
...
- PostgreSQL. Which methods a field has depends on its type: `eq`/`neq`/`in`/`notIn` on any field that can be compared for equality; `gt`/`lt`/`gte`/`lte` on any field that can be ordered; `like` on text fields; `isNull`/`isNotNull` on every field.
...
The methods `all()` / `createAll()` / `updateAll()` / `deleteAll()` return an `AsyncIterableResult`, imported (if you need the type) from `@prisma/orm-postgres/components/runtime`. It has a dual interface:
