...
This page assumes a project where `prisma orm init` has already run and your tables exist: [Coming from Prisma ORM 7](/orm/coming-from-prisma-orm-7) has the packages to install, `prisma orm init`, and `prisma db update`, which replaces `prisma migrate`, and [How migrations work](/orm/migrations/how-migrations-work) creates the tables. For task-oriented walkthroughs, see the Fundamentals guides: [Reading data](/orm/fundamentals/reading-data), [Writing data](/orm/fundamentals/writing-data), [Relations and joins](/orm/fundamentals/relations-and-joins), and [Transactions](/orm/fundamentals/transactions).

## Example schema

All examples on this page run against the schema below, except the grouped-aggregate ones, which use a separate `Customer` / `Order` schema shown under [Grouped aggregates](#grouped-aggregates). Your schema now lives in `contract.prisma`, the contract, which replaces `schema.prisma` from Prisma ORM 7. Running `npx prisma contract emit` compiles it into two generated files: `contract.json`, read at run time, and `contract.d.ts`, read by TypeScript. You import both, as the examples below do. Run `npx prisma contract emit` again after every change to `contract.prisma`. A new project keeps all three files in `src/prisma/`.
...
- `@@type("pg/text@1")` on an `enum` block stores the enum as a text column. Always write it exactly like that. Other versions do not exist yet. The MongoDB form of the same attribute is `@@type("mongo/string@1")`. This page calls an enum written this way a text-backed enum.
...
To give a model your own methods on top of the built-in ones, import `Collection` from `@prisma/orm-postgres/orm-client`, subclass it, and register the subclass with `orm(...)` from that same package. You still create the client with `postgres(...)`, and `orm(...)` takes two things from that client. [Custom `Collection` subclass](#custom-collection-subclass) has the code.
...
These methods narrow a query. Each returns a collection. On this page, a collection is the query object you chain more methods on, such as `db.orm.public.User` or `db.orm.public.User.where(...)`. On MongoDB, `users` in `db.orm.users` is also the name of the database collection. You run the query with a [read method](#read-terminals) such as `all()` or `first()`. MongoDB supports fewer of these methods than PostgreSQL, and each method's Remarks say which databases it works on.
...
- On MongoDB, `select()` changes what the database returns but not the TypeScript type. Only the fields you named come back. The type still lists every field on the model, so read only the fields you named from each returned document and treat the rest as absent.
...
On PostgreSQL, the callback you pass to `include()` receives the related rows as a collection. This page calls that callback a refinement. You can filter, order, and paginate the related rows in it. You can also reduce them to a single value, or use `combine()` to return several results at once. To count or sum a whole model instead of a relation, see [Grouped aggregates](#grouped-aggregates), where `aggregate((agg) => ({ n: agg.count() }))` counts every row.
...
- `sum()` and `avg()` take a numeric column: an integer, a floating-point number, or a decimal. They also take a `Time` column. They do not take a date or a timestamp, and TypeScript rejects a date or timestamp field name here.
...
`sum('createdAt')` and `avg('createdAt')` do not compile. PostgreSQL has no `SUM` or `AVG` over a date or a timestamp.
:::
...
- On PostgreSQL, a native enum column sorts in the order its members are declared, not alphabetically. The example schema declares `Priority` text-backed, so it sorts by its stored text: `'high'`, then `'low'`, then `'urgent'`. Declare the same members in a `native_enum Priority` block instead and `.asc()` sorts them `Low`, `High`, `Urgent`. Do that when the sort order matters.
...
- Always call `orderBy()` before `cursor()`. TypeScript rejects the call if you do not. The library does not check this while the query runs, so code that skips TypeScript, such as plain JavaScript or a cast, gets no error: the cursor is ignored and every row comes back.
- The cursor object must name every column the `orderBy()` sorts on. Leave one out and the call throws an error whose `code` is `ORM.CURSOR_VALUE_MISSING`. With two sort columns, give both keys. The rows are compared as a pair, so you get the rows past the first column's value, plus the rows with that same first value and past the second.
...
- Start the `orderBy()` with the `distinctOn()` columns, as the example does. PostgreSQL rejects a query whose `ORDER BY` does not begin with the `DISTINCT ON` columns. The library does not check this, so a mismatch fails in the database.
...
- On PostgreSQL, a variant that sets its own `@@map`, as `Bug` and `Feature` do, gets its own table. That changes how some writes behave; see [`createAndCount()`](#createandcount) and [`upsert()`](#upsert).
...
There is no `count()` method on either database. On PostgreSQL, count with `aggregate((a) => ({ n: a.count() }))`. On MongoDB, count with `db.query`, the [pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder):

```typescript
const built = db.query.from('posts').count('total').build();
const [counted] = await (await db.runtime()).query(built); // counted.total is the count
```

Both are shown under [Count records](/orm/fundamentals/reading-data#count-records).
...
`first()` returns `null` on no match and does not check that only one row matched. `findUniqueOrThrow` and `findFirstOrThrow` both map onto `all().firstOrThrow()`, as the last line above shows. `firstOrThrow()` is a method on the result that `all()` gives you, and calling it does not count as using the result a second time. It throws an error whose `code` is `RUNTIME.NO_ROWS` when no row matched.
...
- `db.orm` cannot use your subclass. Call the `orm(...)` function with a `collections` object to get a second accessor with the same methods as `db.orm.public`. You now have two accessors, both of them work, and your existing `db.orm.public.X` calls are unaffected. Use the one `orm(...)` returns for the models you subclassed.
- `orm(...)` takes two things from the client that `postgres(...)` created under [Setting up the client](#setting-up-the-client). `client.connect()` gives you the `runtime` argument, and `client.context` is the `context` argument.
...
On PostgreSQL, that object can also carry a `create` or `connect` callback for a related row, as shown below. `posts.create([...])` and `tags.disconnect([...])` take arrays because those relations are to-many, and `user.connect({...})` takes one object because `Post.user` is to-one.
...
- On MongoDB, the update input covers the base model's fields only. Putting a field declared on a variant in the update, such as `duration` on `Tutorial`, makes TypeScript complain even though the update runs correctly. Put `// @ts-expect-error` on the line directly above `.update(...)`. The same is true of `updateAll()` and `updateAndCount()`.
...
- On PostgreSQL, `select()` and `include()` before `.update()` choose which fields and relations the returned row carries. They change nothing about what is written.
...
- That happens because the update runs by filter, while the read-back runs by the ids collected before the update.
...
- For a unique constraint on several columns together, name every one of those columns. The set you name has to match one `@@unique` constraint.
...
PostgreSQL supports aggregation over a result set. `aggregate()` treats the whole result set as one group. `groupBy().aggregate()` aggregates per group, and `having()` filters those groups. None of these methods exist on a MongoDB collection. To aggregate on MongoDB, use the pipeline builder, which is Prisma ORM's API for MongoDB's aggregation pipeline. It is described in [MongoDB: Pipeline builder](/orm/fundamentals/advanced-queries#mongodb-pipeline-builder).
...
Compute aggregates over the current result set. `aggregate()` runs the query, so you `await` it directly instead of calling `.all()`.

#### Remarks

- PostgreSQL only. Not available on MongoDB.
- `count()` counts rows. It takes no argument and always returns a number, `0` over an empty set.
- `count(field)` counts the rows where that field is not null.
- `sum()`, `avg()`, `min()`, and `max()` return `null`, not `0`, over an empty result set. Handle the `null`. `sum('amount')` over an `Int` column is typed `number | null`.
- `sum()` and `avg()` are offered on `Decimal` and `BigInt` columns. Over a `Decimal` column they return the exact value as a string. Over a `BigInt` column `sum()` returns a `number` and throws an error whose `code` is `RUNTIME.DECODE_FAILED` if the total is past `Number.MAX_SAFE_INTEGER`. Use `sumBigInt()` on a `BigInt` column whose total can grow that large.
...
The callback's `agg` argument gives you `count()`, `count(field)`, `sum(field)`, `avg(field)`, `min(field)`, and `max(field)`. Three more methods avoid JavaScript's `number` precision limits. `agg.countBigInt()` takes no argument and returns a `bigint`. `agg.sumBigInt('amount')` returns a `bigint`. `agg.avgDecimal('amount')` returns the average as a string, such as `'10.3333333333333333'`.
...
- `groupBy(...)` returns a `GroupedCollection`, which you never import or name yourself. You cannot `await` it. Call `.aggregate(...)` on it, which runs the query and gives you one row per group. Each row holds the grouping fields, typed as they are on your model, plus the keys you returned from the aggregate callback.
...
- `orderBy(...)` on a grouped collection sorts the groups by a field you grouped on. You cannot sort groups by an aggregate value. To do that, write the query with the SQL query builder (`db.sql`). See the example below.
...
##### Sort the groups by an aggregate, with `db.sql` (PostgreSQL)

```typescript
const topCustomers = db.sql.public.order
  .select((f, fns) => ({ customerId: f.customerId, orders: fns.count() }))
  .groupBy((f) => f.customerId)
  .orderBy((f, fns) => fns.count(), { direction: 'desc' })
  .limit(5)
  .build();
const rows = await db.runtime().query(topCustomers);
```
...
A condition says which rows match. You write one inside `where()`, and inside a relation filter such as `u.posts.some(...)`. Stacked `where()` calls are combined with AND, and you can use the shorthand object in one call and the callback in the next. PostgreSQL and MongoDB write conditions differently.
...
- PostgreSQL. The methods available on a field depend on the field's type. Autocomplete shows which ones apply. `isNull` and `isNotNull` are on every field. `eq`, `neq`, `in`, and `notIn` need a type that can be compared for equality. `gt`, `lt`, `gte`, and `lte` need a type that can be ordered. `like` and `ilike` need a text type. `Uuid` and `DateTime` can be compared for equality and ordered, so they have all of these except `like` and `ilike`. `Json` can do neither, so a `Json` field has only `isNull` and `isNotNull`.
...
:::note[No `.or()` method on MongoDB]
MongoDB filter expressions have `.and(...)` and `.not()`, but no `.or(...)`. Use `MongoOrExpr.of([...])` instead.
:::
...
- All three are also methods on a to-one relation, such as `Post.user`, which holds a single related row. There `some()` reads as "the related row matches" and `none()` as "it does not".
...
- On PostgreSQL, the shorthand object cannot filter a field whose type cannot be compared for equality, such as `Json`. Naming such a field throws an error whose `code` is `ORM.FILTER_UNSUPPORTED`. Use the callback form, which offers whatever that type does support. On a `Json` field that is `isNull()` and `isNotNull()`.
...
- `of(field, operator, value)`: pass the MongoDB operator as a string, including the `$`, for example `'$regex'`. Use it for operators the named helpers do not cover, such as `MongoFieldFilter.of('tags', '$size', 3)`. `of` does no checking of its own, so a misspelled operator such as `'$regexp'` fails at the database, not in TypeScript.
...
| `gt(field, value)` / `lt(field, value)` / `gte(field, value)` / `lte(field, value)` | Field + a value of any type | Greater than, less than, greater or equal, less or equal. |
...
- You can also write `.where({ 'address.city': 'San Francisco' })`. It works, but TypeScript rejects it unless you add a cast, because `.where()` only accepts your model's top-level field names.
...
On MongoDB, `update()`, `updateAll()`, `updateAndCount()`, and `upsert()` take a callback in place of a data object. The callback gets one argument, called `u` or `t` in the examples below, and returns an array of the operations you want. PostgreSQL has no callback form: pass an object to [`update()`](#update) instead.

- For a top-level field, read it off the callback's argument as a property: `u.bio.set('Hello')`.
- For a field inside an embedded object, call the callback's argument as a function with the dot path: `u('address.city').set('San Francisco')`.

All four methods require `.where()`. Without one they throw an error whose `code` is `ORM.WHERE_MISSING`. On `upsert()`, `.where()` picks the document to update, exactly as on `update()`, and the full example is under [`upsert()`](#upsert). `update()` returns the updated document, or `null` when nothing matched. `updateAll()` returns an [`AsyncIterableResult`](#asynciterableresult) of the changed documents. `updateAndCount()` returns the number of documents it changed.

### Available operations (MongoDB)

#### Remarks

- MongoDB only. These are the four operations for single-value fields: `set()`, `unset()`, `inc()`, and `mul()`. The four array operations follow under [Array operations](#array-operations-mongodb-unverified).
...
- After `variant(...)`, `t.duration.inc(10)` is a type error even though it runs correctly. Put `// @ts-expect-error` on the line directly above the `.update(...)` line, as the example below shows. If that type error ever goes away, TypeScript reports the comment as unused, and you delete the comment then.
...
`updateAndCount()` takes the callback in the same place. `upsert()` takes it as its `update` key, beside `create`, and [`upsert()`](#upsert) shows a full call. That callback cannot use a dot path. Given one, `upsert()` throws an error whose `code` is `ORM.OPERATION_UNSUPPORTED`. To change a field inside an embedded object in an `upsert()`, set the whole embedded object on its top-level field instead, as in `u.address.set({ street: '1 Market St', city: 'San Francisco', country: 'US' })`.
...
The callback's argument implements eight operations, and only these eight: `set`, `unset`, `inc`, `mul`, `push`, `pull`, `addToSet`, and `pop`. `min`, `max`, `rename`, and `currentDate` are absent from the API, and TypeScript rejects them: `rename` is not a property of `u.bio`.
:::
...
- MongoDB. `push()`, `pull()`, `addToSet()`, and `pop()` are available on the callback's argument. `push(value)` appends one element. `addToSet(value)` appends it only if it is not already there. `pop(1)` removes the last element, and `pop(-1)` removes the first. Those are MongoDB's own `$pop` values.
...
- These operations need an array field, declared with `[]` in your contract, as in `tags String[]`. Applied to any other field, the write fails with an error raised by MongoDB, not by Prisma ORM. In a `catch` block, `isRuntimeError(error)` is `false` for it and there is no `error.code` to match, so read the message.
...
PostgreSQL and MongoDB behave the same here, with the same error message and the same rule. The example below uses the PostgreSQL accessor; on MongoDB the same methods are on `db.orm.users` (see [Setting up the client](#setting-up-the-client)).

```typescript
import { isRuntimeError } from '@prisma/orm-postgres/components/runtime';
// all() returns the result object straight away and `await` is what runs the query, so this example holds it in a variable.
