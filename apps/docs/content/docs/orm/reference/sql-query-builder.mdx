...
On PostgreSQL you also get `ilike(expr, pattern)` for text columns, as in `fns.ilike(f.displayName, '%alice%')`, but not for a native enum column (`pg.enum(...)`), which has no text operations.

Full-text search on PostgreSQL uses `fns.fullTextMatches(column, query)` to filter, `fns.fullTextRank(column, query)` to score, and `fns.fullTextHeadline(column, query, options?)` to return the matching text with each match marked. `query` is a `tsquery`: build it with `fns.websearchToTsquery(text)`, `fns.plaintoTsquery(text)`, `fns.phrasetoTsquery(text)`, or `fns.toTsquery(text)`, or with the `tsquery` tag from `@prisma/orm-postgres/target/full-text`. `fullTextHeadline` takes `startSel`, `stopSel`, `maxWords`, `minWords`, and `highlightAll`, and all three take `language`. [Full-text search](/orm/reference/orm-client#full-text-search) on the ORM client reference covers the parsers, the options, and the index:
...
`cosineDistance(a, b)` compares two vector values and returns a `number`. It comes with pgvector, the PostgreSQL extension for vector columns, so it is there only if your contract composes pgvector. See [Extension types](/orm/contract-authoring/psl-syntax#extension-types).
