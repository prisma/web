...
All examples on this page run against the following schema. The select and mutation examples use the `user`, `post`, `post_tag`, and `tag` tables. The grouped-query examples use a separate `customer` and `order` schema, shown under [Grouped queries](#grouped-queries). No example uses `Address`, `Task`, `Bug`, or `Feature`. No example on this page uses `types`, `@@type`, `@@discriminator`, or `@@base`. See [Contract authoring](/orm/contract-authoring/psl-syntax).
...
In Prisma ORM 8 your `schema.prisma` is called `contract.prisma`. [Coming from Prisma ORM 7](/orm/coming-from-prisma-orm-7) gets you from a Prisma ORM 7 project to that file, and has the rest of the setup steps. Run `npx prisma contract emit` to generate `contract.json` and `contract.d.ts` beside it. The examples below import both. Install `@prisma/orm-postgres`.

You build queries from the client's `sql` property. Create a Postgres client with `postgres(...)`, then reach tables through `db.sql.public`. `public` is the PostgreSQL schema your tables are in, and Prisma ORM 8 puts every table in `public` today. Table accessors are keyed by table name, not model name. A table name is the model name with a lowercase first letter unless the model sets `@@map`, so the `User` model mapped to `user` is reached as `db.sql.public.user`, and the model for the `Post`/`Tag` join table, mapped to `post_tag`, is reached as `db.sql.public.post_tag`. Column names are the field names unless a field sets `@map`.

### The `db.sql` builder [#the-dbsql-facet]

`db.runtime()` returns the connection that runs a built query. `db.runtime()` is synchronous and returns immediately. Only `runtime.query()` is awaited. Hold the connection in a variable and reuse it. Write the type import path as `./contract.d`, exactly like that. A `select()`, `insert()`, `update()`, or `delete()` call starts a query. Finish it with [`build()`](#build), which returns the built query, and pass that to `runtime.query(...)`. The examples name the built query `plan`. You can run the same built query more than once.
...
Call `.as(alias)` on a finished `SELECT` query to use that query as a subquery. The aliased query is then something you can pass to `innerJoin()`, `outerLeftJoin()`, and the other join methods. For the example, see [Subquery via `.as()`](#subquery-via-as).
...
These methods build and refine a `SELECT`. They chain, and you resolve the query with [`build()`](#build) followed by `runtime.query(...)`. Chain the methods in any order, except that a join must come before any call that names the joined table's columns, and `distinctOn()` needs its columns first in `orderBy()`. `SelectQuery` in the tables below is the type they return.
...
The `where()`, `select()`, `orderBy()`, `update()`, and other callback forms receive two arguments. The first is `f`, which has one property per column in scope. After a join, a column name that both tables have is on `f` only under its table name, such as `f.post.id`. The second is `fns`, an object of [expression helpers](#expressions-and-functions) such as `fns.eq()` and `fns.raw`. For an enum column you pass the value stored in the database, the string after `=` in the enum, so `High = "high"` is matched by `fns.eq(f.priority, 'high')`.
...
A computed expression's result type comes from `.returns(...)` on `fns.raw`, or from the `fns.*` function you called. Every `fns.raw` fragment needs `.returns()`. A type id is `pg/` plus the PostgreSQL type name plus `@1`, such as `pg/int4@1` or `pg/uuid@1`. A few use a longer name, such as `pg/timestamptz-temporal@1` for `DateTime`. The common ones are `pg/text@1`, `pg/int4@1`, `pg/int8@1`, `pg/float8@1`, `pg/bool@1`, `pg/uuid@1`, and `pg/timestamptz-temporal@1`. To reuse a column's own type, pass the column to `.returns()`, as in `.returns(db.sql.public.user.columns.id)`. `db.sql.public.user.columns.id.codecId` holds the id string. See [`fns.raw` and `.returns()`](#fnsraw-and-returns).
...
- **Availability:** PostgreSQL only. On SQLite TypeScript rejects the call. If you get past the types, it throws an error whose `code` is `ORM.CAPABILITY_MISSING`. `outerLateralJoin()` is the left-join form: same arguments, same availability, and the subquery's columns are nullable.
...
- A column name that both tables have must be written under its table name. Outside the callback, the subquery's columns are under the alias you gave it, such as `f.latestPost.id`. Inside the callback, the joined table is still `post`, so the same column is `f.post.id`. A bare `select('id')` or `orderBy((f) => f.createdAt, ...)` throws, because a name both tables have is not in the query's scope on its own.
...
TypeScript also accepts a `nulls` option, but the builder ignores it and no `NULLS FIRST` or `NULLS LAST` reaches the SQL. Do not use it. To put nulls last, sort first by a computed value: an `fns.raw` fragment of the form `CASE WHEN <column> IS NULL THEN 1 ELSE 0 END` that returns `pg/int4@1`. Then call `orderBy()` again for the column itself.
...
- **Availability:** PostgreSQL only. On SQLite TypeScript rejects the call. If you get past the types, it throws an error whose `code` is `ORM.CAPABILITY_MISSING`.
- List the `distinctOn()` columns first in `orderBy()`, then the column that picks the winner.
...
Grouping starts with [`groupBy()`](#groupby), which turns a select query into a `GroupedQuery`. Call `groupBy()` after all your `select()` calls. A grouped query supports `having()` for group-level filtering, the aggregate functions, and the same `orderBy()`, `limit()`, `offset()`, and `distinct()` methods as a `SelectQuery`.

The examples below use a separate `order` schema, where `order.amount` is an integer column:
...
- A `count()` or integer `sum()` result larger than 2^53 - 1 fails with an error whose `code` is `RUNTIME.DECODE_FAILED`. To check for a code, catch the error and compare `error.code`. Use `countBigInt()` or `sumBigInt(field)` when totals can get that large.
...
// `.returns()` only tells TypeScript what type to expect. It does not convert the value, so yearPlaced is the string '2024'. Write Number(row.yearPlaced) to get 2024.
```

Here `.groupBy('yearPlaced')` groups by the alias given to the selected value. You can also repeat the same `fns.raw` fragment inside `groupBy()`. A name that is neither a column of the table you started from nor an alias you selected throws an error whose `code` is `ORM.COLUMN_UNKNOWN`, at the moment you call `groupBy()`. If you select a column you did not group or aggregate, the query fails when it runs, not when you build it. That failure comes back as PostgreSQL's own error, not a Prisma ORM code.
...
- Leave a column out and the `@default(...)` in `contract.prisma` is applied, such as `@default(uuid())` or `@default(now())`. `build()` generates default values once. Running the same built insert twice writes the same id twice, which fails on a unique column. Call `.build()` again for each run.
- Leave out a required column that has no default and PostgreSQL rejects the insert when the query runs.
...
// execute runs a write that returns no rows. id is filled in from @default(uuid())
```
...
Every value the callback returns must be an expression. To set one column to a fixed value in the callback form, wrap that value in a `fns.raw` fragment, as the second example below does.
...
const plan = db.sql.public.user
  ...
  .build();
const rows = await runtime.query(plan);
// one row, with displayName now 'CAROL' and a fixed new email
```
...
- Import `param` from `@prisma/orm-postgres/relational-core/expression`. `param` and `ResultType` are in different subpaths of the same package, so use the paths shown.
- You rarely need `param()`. Values passed to `insert()`, `update()`, and comparison functions such as `fns.eq(f.col, value)` each take their type from the column they are used with. `param()` is for the values that have no such column.
...
On PostgreSQL you also get `ilike(expr, pattern)` for text columns, as in `fns.ilike(f.displayName, '%alice%')`. `cosineDistance(a, b)` compares two vector values and returns a `number`. It comes with pgvector, the PostgreSQL extension for vector columns, so it is there only if your contract uses pgvector.
...
| `.returns(codecId)` | `string` | Yes, always | Declares the result type, for example `'pg/int4@1'`, `'pg/text@1'`. For a fragment used as a `where()` predicate, write `.returns('pg/bool@1')`. |

#### Remarks

- **Always call `.returns()`.** Nothing in the builder accepts a fragment without it.
- **`.returns()` only tells TypeScript what type to expect.** It does not convert the value. PostgreSQL computes `EXTRACT(...)` as `numeric`, which arrives as a string, so `yearPlaced` is `'2024'`. Convert it in JavaScript with `Number(...)`, or run the statement with [`db.raw.sql` and `.returnsRow()`](/orm/reference/raw-queries), which does decode.
...
`await runtime.query(plan)` collects every row into an array. This is the common case and what every example on this page uses. You can also loop the same result with `for await` to handle one row at a time. Use `for await` when you want to start handling rows before the array is assembled. It does not reduce memory on PostgreSQL, because both `await` and `for await` read the same result the client has already loaded.
