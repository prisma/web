...
Every later schema change is the same three commands: `contract emit`, `migration plan --name <slug>`, `db migrate --advance-ref db`. If you apply a migration without `--advance-ref db`, the next `migration plan` fails with `MIGRATION.PLAN_ORIGIN_UNKNOWN` rather than silently re-planning from an empty database. The error prints the hash to set with `migration ref set db <hash>`.
...
- `MIGRATION.PLAN_ORIGIN_UNKNOWN`: migrations exist but no `db` ref is set, usually because a `db migrate` ran without `--advance-ref db`. Run `npx prisma@latest migration ref set db <hash>` with the hash the error prints, then keep using `--advance-ref db`.
