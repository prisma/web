...
An extension is usually a versioned npm package with a `control` entrypoint for the config and a `runtime` entrypoint for the client; middleware ships a single entrypoint instead. The same layout adds a database: `@prisma/orm-postgres` is the reference for a SQL database and `@prisma/orm-mongo` for a document database. The [call for extension authors](https://www.prisma.io/blog/prisma-next-call-for-extension-authors) describes the layout. To write middleware, follow [Authoring custom middleware](/orm/middleware/authoring-custom-middleware).
