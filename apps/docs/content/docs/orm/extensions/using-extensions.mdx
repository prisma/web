...
Your contract is what Prisma ORM 8 calls your schema. In a project made with `npm create prisma@latest` it is `src/prisma/contract.prisma`, in place of `schema.prisma`, and you still describe your models in it. `npx prisma contract emit` writes `contract.json` and `contract.d.ts` from it, and the file where you create the client (`src/prisma/db.ts`) imports those two rather than the `.prisma` file. [The data contract](/orm/contract-authoring/the-data-contract) covers what goes in it.
...
The database package is the one whose functions you call, `ormConfig(...)` and `postgres(...)`, and every other extension goes in the `extensions` array of each. The example renames `defineConfig` to `ormConfig` so it is not confused with `definePrismaConfig`, and you can keep the original name.

An extension has one entry point for the command line tools and one for your application. For pgvector they are `@prisma/orm-extension-pgvector/control` and `@prisma/orm-extension-pgvector/runtime`. The database package uses `/config` and `/runtime` for the same two jobs, and Supabase publishes its command-line side as `/pack`. The extension's directory page tells you which to import.
...
The `pgvector` in `pgvector.Vector(1536)` is the name the extension registers its types under, and its directory page lists them. PostGIS, for example, gives you `postgis.Geometry(4326)`.
...
2. `npx prisma migration plan`, which writes the extension's own migration under `migrations/pgvector/`. That directory is part of your project's migrations, so commit it with the rest of `migrations/`. Skip this and the next command stops with an error whose `code` is `MIGRATION.CONTRACT_SPACE_LAYOUT_VIOLATION`.
3. `npx prisma db init` on a new database, or `npx prisma db update` on one that already has tables, which [Applying a migration](/orm/migrations/applying-a-migration) covers. This runs the extension's migration, which is the `CREATE EXTENSION IF NOT EXISTS vector` statement, and creates the `post` table.
...
`public` is the PostgreSQL schema your tables are in.

Searching for the rows closest to a query vector uses the operations the extension adds, and you write that search with the SQL query builder rather than with `db.orm`. `db.orm` is the ORM API, `db.sql` is the [SQL query builder](/orm/fundamentals/advanced-queries), and `db.runtime()` is what runs a built query. Under `db.orm` you address a model by the name in your contract, so the model `Post` is `db.orm.public.Post`. Under `db.sql` you address a table by the name it has in the database, so the same data is `db.sql.public.post`. In the callback of each builder method, `f` holds the row's columns and `fns` holds the functions you can call on them, including the ones pgvector adds. The query vector is a `number[]` of the same length as the stored embeddings:
...
`.select('distance', ...)` names the computed column `distance`, and `.build()` returns the built query as a plain object that `db.runtime().query(...)` runs. Each row that comes back has `id`, `title`, and `distance`, and `distance` is a `number`.
...
When your contract uses an extension and `db.ts` does not list it in `extensions`, `postgres(...)` throws an error whose `code` is `RUNTIME.MISSING_EXTENSION_PACK` before any query runs. The error calls an extension a pack, which is also the name Supabase uses for its command-line entry point. Add the extension's `runtime` import to `db.ts` to fix it. If the database cannot install the extension, `db init` or `db update` reports it.
...
The packages that add support for a database are extensions themselves, which is why `@prisma/orm-postgres`, `@prisma/orm-sqlite`, and `@prisma/orm-mongo` are in the table.
