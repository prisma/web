...
Your contract is the Prisma ORM 8 name for your schema: `contract.prisma` in a project made with `npm create prisma@latest`, in place of `schema.prisma`. It is the same file under a new name, and you still describe your models in it. `npx prisma contract emit` writes `contract.json` and `contract.d.ts` from it, and those two files are what `db.ts` imports. [The data contract](/orm/contract-authoring/the-data-contract) covers what goes in it.
...
Add the extension to the `extensions` array in your config file.
...
The database package is the one you build on: `@prisma/orm-postgres` is what provides `ormConfig(...)` for the config and `postgres(...)` for the client. Every other extension, pgvector here, is passed to those two calls in their `extensions` array. `ormConfig` is a rename: `@prisma/orm-postgres` exports that function as `defineConfig`, and the import above renames it so that it is not mistaken for `definePrismaConfig` from `prisma/config`. The name is yours to choose.

An extension package has two entry points, and which one you import depends on which file you are in. `control` is the entry point the `prisma` command line tools read, so that is the one `prisma.config.ts` imports. `runtime` is the entry point your application code reads, so that is the one `db.ts` imports in the next step.
...
1. `npx prisma contract emit`, so that `contract.json` and `contract.d.ts` pick up the new column. Run it again after every later change to `contract.prisma`, because the migration commands read `contract.json` rather than the `.prisma` file.
2. `npx prisma migration plan`, which writes the extension's own migration under `migrations/pgvector/`. That directory is part of your project's migrations, so commit it with the rest of `migrations/`. Until it exists, the next command stops with an error whose `code` is `MIGRATION.CONTRACT_SPACE_LAYOUT_VIOLATION`.
3. `npx prisma db init` on a new database, or `npx prisma db update` on one that already has tables. This runs the extension's migration, which is the `CREATE EXTENSION IF NOT EXISTS vector` statement, and creates the `post` table.

You never write that `CREATE EXTENSION` statement yourself, but the PostgreSQL server has to have pgvector available for it to succeed. The local development server the quickstart sets up has it. A stock PostgreSQL image may not, and then the migration fails on that statement.
...
Searching for the rows closest to a query vector uses the operations the extension adds, and you write that search with the SQL query builder rather than with `db.orm`. `db.orm.public.Post` is the model, so it uses the name from your contract, and `db.sql.public.post` is the table, so it uses the name as it is in the database. `db.orm` is the ORM API, `db.sql` is the [SQL query builder](/orm/fundamentals/advanced-queries), and `db.runtime()` is what runs a built query. The query vector is a `number[]` of the same length as the stored embeddings:
...
In the callback, `f` holds the row's columns and `fns` holds the functions you can call on them, including the ones pgvector adds. `.select('distance', ...)` names the computed column `distance`. `.build()` returns the query as a plain object, and `db.runtime().query(plan)` runs it and returns the rows.
...
The packages that add support for a database are extensions themselves, which is why `@prisma/orm-postgres`, `@prisma/orm-sqlite`, and `@prisma/orm-mongo` are in the table. A project has one database package and any number of other extensions listed next to it in the two `extensions` arrays.

The ones marked experimental work today, but their methods and options are still changing between releases. They are also incomplete in places: ParadeDB's full-text index takes only one option so far, `key_field`, and SQLite is published without a quickstart to follow. Each name in the table links to that extension's directory page, and from there to the package README. Middleware wraps queries rather than adding a database feature, so it is listed on [How middleware works](/orm/middleware/how-middleware-works) instead.
...
The client is where the real difference is, because you build it with `supabase()` from `@prisma/orm-extension-supabase/runtime` rather than with `postgres()`, and it takes the secret it needs to check JWTs:
...
`db.asUser(jwt)` gives you a client that runs as that user, `db.asAnon()` one that runs as the anonymous role, and `db.asServiceRole()` one that bypasses row-level security, and each of them has the same `orm` and `sql` you use on `db`. The [runnable example](https://github.com/prisma/orm/tree/main/examples/supabase) shows the whole setup in one project.
