...
description: 'Extensions add database capabilities like vector search, geospatial data, and full-text search to a Prisma ORM project.'
...
The extension's types are now available in your contract, so declare a vector column with an explicit dimension:
...
Each extension names what it adds under a key like `pgvector.cosine`, but you never write these keys by hand. Registering the extension in the config records them in your contract, and registering it in `db.ts` provides them at runtime.
