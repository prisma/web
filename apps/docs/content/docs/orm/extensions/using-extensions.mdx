...
The extension's contract describes Supabase's `auth` and `storage` tables as they are in one reference Supabase build, supabase/postgres 17.6.1.106, check constraints included. Run [`npx prisma db verify`](/cli/db-verify) to compare your database with it: if your Supabase build has a different set of check constraints, `db verify` lists the ones it cannot find. You cannot fix that with a migration, because Prisma ORM never changes Supabase's own tables. Report the difference to Prisma instead, so the reference build can be updated.

[`npx prisma db sign`](/cli/db-sign) checks that a database matches your contract and then records in the database which contract it matches. When an update to `@prisma/orm-extension-supabase` changes the extension's contract, that record no longer matches, so run `npx prisma db sign` again against every database you signed with the earlier version. If you are also re-emitting your own contract with `npx prisma contract emit`, run that before `db sign`.
