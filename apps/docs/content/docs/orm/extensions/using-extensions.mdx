...
The extension's contract describes Supabase's `auth` and `storage` tables as they are in a reference Supabase build, supabase/postgres 17.6.1.106, check constraints included. If your build differs, `db verify` reports the difference. When an update to `@prisma/orm-extension-supabase` changes that contract, run `npx prisma db sign` again against every database signed with the earlier version. If you also re-emit your own contract, do that first.
