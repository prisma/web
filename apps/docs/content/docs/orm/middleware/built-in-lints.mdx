...
A rule set to `error` throws before the database sees anything, so the call you were awaiting rejects. It throws an ordinary `Error` with five extra properties on it: `code`, `category`, `severity`, `message`, and `details`. Use the `isRuntimeError` type guard, exported from `@prisma/orm-postgres/components/runtime`, so that TypeScript knows those properties are there and you can read `code` off the error.
