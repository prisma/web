...
| `deleteAll()` or `updateAll()` after a `.where(...)` with a condition in it | none |
| `deleteAll()` or `updateAll()` after `.where({})` | `LINT.DELETE_WITHOUT_WHERE` or `LINT.UPDATE_WITHOUT_WHERE` |
| the query builder's `delete()` or `update()` with no `where()` | `LINT.DELETE_WITHOUT_WHERE` or `LINT.UPDATE_WITHOUT_WHERE` |

So `await db.orm.public.User.limit(50).all()` sets off `LINT.SELECT_STAR`, while `await db.orm.public.User.select('id', 'email').limit(50).all()` does not. In `db.orm.public.User`, `public` is the PostgreSQL schema and `User` is your model name. The SQL query builder reaches the same table by its name in the database, as `db.sql.public.User`. The model and the table have the same name here because the model sets no `@@map`.

For a `warn` finding, `lints` builds the record below, with the code of the rule, a message, and the table the query touched, and passes it to the client's logger. The `postgres(...)` client has no logger you can set yet, as the note above says, so today the record is thrown away and you cannot see it anywhere.
