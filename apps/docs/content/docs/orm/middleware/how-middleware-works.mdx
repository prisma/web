...
If you used `$use` in Prisma ORM 7, the `middleware` array is its replacement. There is no `next()`: you pick the hook that runs at the moment you care about. [Coming from Prisma ORM 7](/orm/coming-from-prisma-orm-7) lists every Prisma ORM 7 feature next to the Prisma ORM 8 form it takes.
...
`npm create prisma@latest` generates this `src/prisma/db.ts` for you. Everything in it except the `middleware` option is already there, and that option is what you add.
...
- `entry.codecId` is the stored type, one of `pg/text@1`, `pg/int4@1`, `pg/int8@1`, `pg/float8@1`, `pg/bool@1`, `pg/uuid@1`, and `pg/timestamptz-temporal@1`.
