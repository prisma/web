...
The `budgets` middleware stops one careless query from reading far more of your data, or taking far more time, than you meant to give it. You give it a row limit and a time limit, and it checks every query you send against both. When a query goes over either limit, your call fails with an error and you get no rows back.
...
Register `budgets` in the `postgres(...)` call that creates your client, with the row and time limits you want to enforce. The file below is the `src/prisma/db.ts` that `create-prisma` generates, and the only line added to it is the `middleware` option. Your contract is the Prisma ORM 8 name for your schema, `contract.prisma` in place of `schema.prisma`, and `npx prisma contract emit` writes from it the `contract.json` and `contract.d.ts` that this file imports. [The data contract](/orm/contract-authoring/the-data-contract) covers them in full.
