...
Register `budgets` in the `postgres(...)` call that creates your client, with the row and time limits you want to enforce. The file below replaces the `src/prisma/db.ts` that `create-prisma` generates, and the `middleware` option is the only part specific to `budgets`. Your contract is the Prisma ORM 8 name for your schema, `contract.prisma` in place of `schema.prisma`, and `npx prisma contract emit` writes from it the `contract.json` and `contract.d.ts` that this file imports. [The data contract](/orm/contract-authoring/the-data-contract) covers them in full.
