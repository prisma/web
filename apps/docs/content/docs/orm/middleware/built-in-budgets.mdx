...
Every `SELECT` needs a `LIMIT` anyway, but a limit can still be larger than `maxRows`. The check before the query knows how big a table is only from the number you give it in `tableRows`, so with a real number there, a query like that on a big table fails before it runs rather than part way through reading its rows, as [Common gotchas](#common-gotchas) shows. The two numbers in the example are placeholders for how many rows your own tables really hold, so a `Post` table with two million rows in it is written `tableRows: { Post: 2_000_000 }`. A `count` from the database is enough to get each number, such as `select count(*) from "public"."User";` for the `User` table, and you can leave `tableRows` out altogether, in which case every table is assumed to hold `defaultTableRows` rows.

Each key is the table name as it is in the database: the model's `@@map` value, or the model name exactly as written when the model has no `@@map`. So the model `User` is keyed `User`, not `public.User`. `budgets` looks a table up by its name alone, so two tables with the same name in different PostgreSQL schemas share one key and one number. If you misspell a table name, `budgets` silently uses `defaultTableRows` for that table and never warns you.
...
When a query that used to work starts failing with `source: 'observed'`, the table has grown past the number you gave for it in `tableRows`. Put a fresh count there, and the check before the query then stops that query with `source: 'ast'` before the database does any work, rather than part way through the read. The query is still over budget either way, because its limit is larger than `maxRows`. To make it run, raise `maxRows`, or lower its `.limit(...)` to `maxRows` or less and read the rest a page at a time with `.offset(...)`, as [Sort and paginate](/orm/fundamentals/reading-data#sort-and-paginate) describes:

```ts title="scripts/export/read-users.ts"
const pageSize = 10_000;
for (let offset = 0; ; offset += pageSize) {
  const page = await db.orm.public.User
    .orderBy((u) => u.id.asc())
    .limit(pageSize)
    .offset(offset)
    .all();
  // handle this page of rows
