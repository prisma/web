...
The SQL query builder is the other way to write a query on the same client, `db.sql.public.User...` in place of `db.orm.public.User...`. With it you build a query and run it in two steps: `.build()` gives you the query as a plain object, the plan, and a method on `db.runtime()` runs it. Which of the two methods you call decides which hook you get. `db.runtime().query(plan)` returns rows and reaches `afterQuery`, while `db.runtime().execute(plan)` reports how many rows the statement changed and reaches `afterExecute`.
...
Every `SELECT` on `User` now runs with `WHERE "User"."name" = $1` added, and a query that already had a `WHERE` clause keeps it, joined to yours with `AND`. The condition is built once, when you create the client, so every query on this client is filtered to the same value. Swap `name` for the column that holds your tenant and `'Mia'` for a tenant ID, and every `SELECT` on `User` through this client returns only that one tenant's rows.

Copy each import as it is written, including the one that says `type` and the one that does not.

The id depends on the field's type in your contract:

| Type in your contract | Id |
| --- | --- |
| `String` | `pg/text@1` |
| `Int` | `pg/int4@1` |
| `BigInt` | `pg/int8@1` |
...
The last three checks answer only a `SELECT` that reads the `User` table directly, in the same way the `beforeCompile` example picks its queries. Matching on the SQL text instead, such as `plan.sql.includes('FROM "public"."User"')`, also catches `DELETE FROM "public"."User"`, so an ORM `.delete()` would get a made-up row back and delete nothing. On an `UPDATE` or a `DELETE` the table name is at `plan.ast.table.name`. A hook cannot tell which model or which ORM call a query came from, so the table name in the tree is the only thing you have to match on.

The first check, on `ctx.scope`, leaves queries inside a transaction to the database, as the cache does. An ORM `.update()` or `.delete()` first reads the row it is about to change, inside a transaction, and if the fixture answered that read, the write would go to the row whose `id` is `1` rather than the row you asked for. [The context object](#the-context-object) covers `ctx.scope`.
