...
The SQL query builder is the other way to write a query on the same client, `db.sql.public.user...` in place of `db.orm.public.User...`. With it you build a query and run it in two steps, and the step you run it with decides which hook you get. Calling `.build()` gives you the plan, and `db.runtime()` is the method on your client that runs one. `db.runtime().query(plan)` returns rows and reaches `afterQuery`, while `db.runtime().execute(plan)` returns how many rows the statement changed and reaches `afterExecute`.
...
`npm create prisma@latest` generates this `src/prisma/db.ts`, and the only line added here is the `middleware` option. If you set your project up by hand, [The data contract](/orm/contract-authoring/the-data-contract) covers where `contract.json` and `contract.d.ts` come from.
...
- If a middleware throws, the ones listed after it do not run. Put your logger first and it still logs the queries that a later middleware rejects.
...
Prisma ORM decodes the rows you return the same way it decodes rows that came from the database, so your caller still gets a `Date` where the contract says so even though you handed back a string.
