...
The `dev` script watches `src/index.ts` and keeps running after the output appears, so stop it with Ctrl+C when you are done. If you see the query result but no `[query-logger]` lines, the query ran on a client that does not have the middleware registered; check that your script imports `db` from the file you edited in step 2.
...
Prisma ORM groups databases into families: `sql` (PostgreSQL, SQLite) and `mongo` (MongoDB). Set `familyId: 'sql'` when your middleware touches anything SQL-shaped, like `plan.sql` in the logger above. The runtime then rejects it on MongoDB with `RUNTIME.MIDDLEWARE_FAMILY_MISMATCH` when it builds its runtime, on the client's first use, instead of failing inside your hook.

Leave `familyId` out only when your middleware uses hooks that work on both families, which is every hook except `beforeCompile`, and reads only the result fields both databases have. Such a middleware runs on PostgreSQL and MongoDB alike; the [cache](/orm/middleware/built-in-cache) works this way.
...
`beforeCompile` sees the typed AST before it becomes SQL, so return a new draft to rewrite the query. This middleware adds a predicate to every `SELECT` on the `user` table:
...
Projects scaffolded with `create-prisma@latest` install [Prisma ORM skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent. Prompts that map to this guide:
