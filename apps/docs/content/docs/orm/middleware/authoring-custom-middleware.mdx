...
Every `SELECT` whose `FROM` clause names `User` now runs with `WHERE "User"."name" = $1` added, and a query that already had a `WHERE` clause keeps it, joined to yours with `AND`. The condition is built once, when you create the client, so every query on this client is filtered to the same value.
...
- "Add a beforeCompile middleware that adds a fixed `orgId` condition to every SELECT that reads the User table in its FROM clause."
