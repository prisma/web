...
Run each request's queries inside `currentName.run(...)`:

```ts title="src/routes/users.ts"
import { currentName, db } from '../prisma/db';

const users = await currentName.run('Mia', () => db.orm.public.User.select('id', 'name').all());
```

Every `SELECT` whose `FROM` clause names `User` then runs with `WHERE "User"."name" = $1` added, and a query that already had a `WHERE` clause keeps it, joined to yours with `AND`. A query that runs outside `currentName.run(...)` gets no condition, because `getStore()` returns `undefined` there. If every query must have one, throw in the function instead of returning `undefined`.
