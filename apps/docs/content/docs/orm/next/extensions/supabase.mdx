...
- `migration plan` compares your schema against the database and writes a migration under `migrations/app/`. The first run also snapshots the Supabase extension's contract under `migrations/snapshots/`, referenced from `migrations/supabase/`; commit the whole `migrations` directory, since it lets CI and production apply migrations without loading the extension package.
