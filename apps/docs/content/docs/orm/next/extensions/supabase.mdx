...
Prisma 8 ships today as a Release Candidate of the `prisma@next` CLI and `@prisma/orm-*` packages. The API surface is stable, and we are collecting feedback before general availability. Try it with your Supabase project and [share what you find in Discord](https://pris.ly/discord).
...
This installs `@prisma/orm-postgres` and the `@prisma/cli` package, then creates several files:
...
If the directory has no `package.json`, the scaffold creates one with `"type": "module"`. It also runs the first `contract emit` for you, so the generated types exist from the start.
...
The CLI's own binary is named `prisma-cli`, so its output shows next-step hints like `prisma-cli migrate`. With the setup in this guide, run those through npx as `npx prisma@next migrate`.
