...
To run a multi-document MongoDB transaction today, use the MongoDB driver directly, sharing one `MongoClient` between Prisma 8 and your driver code. See the [Fundamentals transactions guide](/orm/next/fundamentals/transactions#transactions-on-mongodb) for the driver-session pattern.
:::
