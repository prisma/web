...
url: /orm/next/middleware/built-in-cache
metaTitle: 'Prisma Next built-in middleware: cache'
metaDescription: Wire the Prisma Next cache middleware, opt queries in with cacheAnnotation and a TTL, and understand cache keys, scopes, and hits.
badge: early-access
---
...
Install the package and register the middleware first in the chain, so cache hits short-circuit before any other middleware does work:
...
On the ORM client, pass the annotation through the query's meta callback:
...
const models = orm({ runtime, context: db.context, collections: {} }).public;
...
Cache hits are still visible to the rest of the middleware chain: `afterExecute` fires on every middleware with `result.source` set to `'middleware'` instead of `'driver'`, so logging and metrics keep working for cached reads.
...
| `clock` | `() => number` | `Date.now` | Time source for TTL expiry, useful in tests |
...
- [Authoring custom middleware](/orm/next/middleware/authoring-custom-middleware), including how `intercept` short-circuiting works
