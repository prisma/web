...
metaDescription: Wire and configure the budgets middleware in Prisma Next to cap row counts, report query latency, and understand the errors it raises.
badge: early-access
---
...
In permissive mode, the same information is emitted through the runtime logger as a warning. To fix an unbounded-select violation, add a limit to the query ([`take(...)` on the ORM API](/orm/next/fundamentals/reading-data#sort-and-paginate), `.limit(...)` on the SQL query builder) or raise the budget deliberately.
