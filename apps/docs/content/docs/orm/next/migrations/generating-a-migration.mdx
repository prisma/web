...
metaTitle: Generating a migration in Prisma Next
...
We'll start from a minimal project. If you don't have one, `npx create-prisma@next` scaffolds it.
...
Now plan. `--name` sets the human-readable part of the directory name; without it the directory is just called `migration`:
...
Two notes on reading it: the `col(...)` calls mirror your contract fields, and you never write `codecRef` by hand — the planner fills it in (it pins how values convert between TypeScript and Postgres), so treat it as noise when reviewing.
