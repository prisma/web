...
1. **Change your contract**: edit your `.prisma` file, then run `prisma-next contract emit`, which compiles the schema into the `contract.json` that every other command reads.
...
These `from`/`to` hashes are what link migrations into a [graph](/orm/next/migrations/the-migration-graph) instead of a numbered list. Thinking about your migrations as a graph instead of a linear set of actions enables Prisma Next migrations to help you in complex scenarios that other migration systems can't. If you want a simple migration system, you can ignore this.
