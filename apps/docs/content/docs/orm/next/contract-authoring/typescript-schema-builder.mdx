...
Choose [PSL](/orm/next/contract-authoring/psl-syntax) when you want the most compact syntax, or when you start from an existing database, since [`prisma-next contract infer`](/cli/next/contract-infer) writes PSL.
