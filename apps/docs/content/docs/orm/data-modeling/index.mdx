...
On MongoDB, the primary key is the document's mandatory `_id` field, and it must be an `ObjectId`. `@map("_id")` gives the field that name in the database. The field takes no `@default`, because MongoDB assigns the id itself when you create a document without one. `@@map("users")` names the collection and is optional: without it, the collection is the model name exactly as written, `User`.
...
A `Decimal` reaches your code as a string, so nothing is rounded through a JavaScript number, and a `Bytes` value reaches you as a `Uint8Array`. A `DateTime` is a JavaScript `Date` on MongoDB, but on PostgreSQL it reaches you as a `Temporal.Instant`, the standard JavaScript type for a point in time. `Temporal` is built into Node.js 26.8.2 and later, but Homebrew's build of Node.js 26 leaves it out, so check with `node -p "typeof Temporal"`. If that prints `undefined`, install the `temporal-polyfill` package and add `import "temporal-polyfill/full/global"` to your app's entry file, before any query runs. That import covers your app only, and `contract emit` needs its own `Temporal` when a date or time field has a fixed default, as [Default values](/orm/contract-authoring/psl-syntax#default-values) explains. To work with a JavaScript `Date` on PostgreSQL instead, and need no polyfill, type the field `TimestamptzJsDate`, but a `Date` drops the microseconds PostgreSQL stores.
...
A trailing `?` makes a field optional: `name String?`. A trailing `[]` makes it a list: `tags String[]`. A `@default(...)` must be written in a form the field's type accepts, such as the `json` literal ``@default(json`{}`)`` on a `Jsonb` field and `@default(1.50)` on a `Decimal` field; [Default values](/orm/contract-authoring/psl-syntax#default-values) lists them.
