...
A trailing `?` makes a field optional: `name String?`. A trailing `[]` makes it a list: `tags String[]`. A `@default(...)` must be written in a form the field's type accepts, such as ``@default(json`{}`)`` on a `Jsonb` field and `@default(1.50)` on a `Decimal` field; [Default values](/orm/contract-authoring/psl-syntax#default-values) lists them.
