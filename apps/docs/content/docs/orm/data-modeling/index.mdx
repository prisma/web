...
In Prisma ORM, you define this structure in a `contract.prisma` file. This file becomes the shared contract between your application code, your database migrations, and your developer tools.

If you are coming from Prisma ORM 7, `contract.prisma` plays a similar role to the `schema.prisma` file you used before.

This page introduces the four building blocks of every Prisma ORM contract:
...
- A relation field, typed as the other model, like `author User`. It stores nothing itself; it tells Prisma ORM how to navigate the connection in queries.
...
Projects scaffolded with `create-prisma@latest` install [Prisma ORM skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent; the `prisma-8` skill covers contract authoring. Prompts that map to each section:
