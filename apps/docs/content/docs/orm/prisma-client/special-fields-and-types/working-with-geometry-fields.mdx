...
If you omit the `srid` field, the SRID defined in your schema will be used:
...
const location = await prisma.location.create({
  data: {
    ...
    position: {
      ...
      // srid defaults to 4326 from schema
    },
  },
});
```
...
**How it works:** This filter uses PostGIS `ST_Within` function to test if a geometry is fully contained within the polygon. For a point to match, it must be inside the polygon boundary (not on the edge).

**When to use:** Use `within` when you need to validate if locations fall inside a specific area - for example, checking if a delivery address is in your service zone, or filtering properties within a neighborhood boundary.
...
Food delivery and logistics companies need to validate if a customer's address falls within their service area before accepting orders. This example stores delivery zones as polygons and checks if a given address point is within any zone:
...
// Create a small polygon around the customer's location
// This checks if any delivery zone contains this point
const availableZones = await prisma.deliveryZone.findMany({
  where: {
    boundary: {
      within: {
        polygon: [
          [customerLocation[0] - 0.001, customerLocation[1] - 0.001],
          [customerLocation[0] - 0.001, customerLocation[1] + 0.001],
          [customerLocation[0] + 0.001, customerLocation[1] + 0.001],
          [customerLocation[0] + 0.001, customerLocation[1] - 0.001],
          [customerLocation[0] - 0.001, customerLocation[1] - 0.001],
...
// If we found zones, delivery is available
...
Prisma Client automatically generates TypeScript types for your geometry fields. Since geometries are stored as JSON in the database and represented as GeoJSON objects in your code, Prisma uses `JsonValue` types for geometry fields:
...
type LocationCreateInput = {
  name: string;
  position?: Prisma.InputJsonValue; // GeoJSON-like object
};
...
type Location = {
  id: number;
  name: string;
  position: Prisma.JsonValue | null; // GeoJSON object or null
};
```

### Working with geometry types

You can use the `Prisma.JsonValue` type to work with geometry data:

```ts
import { Prisma } from "@prisma/client";

// Type guard for geometry
function isValidGeometry(value: Prisma.JsonValue): value is {
  type: string;
...
if (location?.position && isValidGeometry(location.position)) {
  console.log(`Location type: ${location.position.type}`);
  console.log(`Coordinates: ${location.position.coordinates}`);
  console.log(`SRID: ${location.position.srid}`);
}
```
...
When you create a table with a geometry column, PostGIS automatically creates a GIST index:

```sql
-- Automatically created by PostGIS
CREATE INDEX location_position_idx ON "Location" USING GIST (position);
```

:::tip

All native geometry filters (`near`, `within`, `intersects`) are optimized to use GIST indexes, providing fast spatial queries even with millions of records.
