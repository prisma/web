...
description: contract.json and contract.d.ts are the deterministic artifacts every other part of Prisma ORM consumes. Here is what is inside them.
...
| `executionHash` | Defaults Prisma ORM applies before writes, such as `uuid()` generators | Generated defaults change |
...
The hashes are how Prisma ORM connects the contract to a live database. [`db sign`](/cli/db-sign) checks that the database satisfies the contract and records the hashes in a marker inside the database. From then on, [`db verify`](/cli/db-verify) and the runtime compare the contract they hold against the marker, so a stale deploy or an unmigrated database is caught by verification rather than by a failing query.
...
- **`execution`**: defaults Prisma ORM applies before writes, such as UUID generation, kept out of the database's DDL.
