...
- **`roots`**: one entry per table that stores a model. The key is the name of that table, so for a model `User` mapped to a table `users` the key is `users`. The value names the model. You can query the model's data in two ways: `db.sql.public.User` uses the table name, and `db.orm.public.User` uses the model name. In this example both are `User`, because the model sets no `@@map`. The [ORM reference](/orm/reference) covers both.
