...
- **`roots`**: one entry per model. The key is the name of the table that stores the model, so for a model `User` mapped to a table `users` the key is `users`. The value names the model. The [ORM reference](/orm/reference) covers both ways of reaching it, `db.sql.public.user` by table name and `db.orm.public.User` by model name.
...
- One type per hash, such as `StorageHash` and `ProfileHash`. The hash is part of the type name, so a `contract.d.ts` from one run does not type-check against a `contract.json` from another.
...
You never write these types yourself. Both files come from the one `npx prisma contract emit` run, so they only drift when someone commits one and not the other. When that happens, the hash types no longer agree and TypeScript catches it before `prisma db verify` has to.
