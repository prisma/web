...
PostgreSQL reports `true` for all three `sql` keys above, and SQLite reports `false` for them. This table is a sample, not the full list. Open `src/prisma/contract.json` in your project to see every key you have. The build-time check runs for every database. The run-time check only exists on the SQL query builder, and MongoDB has no SQL query builder. The [ORM reference](/orm/reference) lists the methods each database has.
