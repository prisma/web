...
`field.temporal.timestamptzJsDate()` is a `timestamptz` column that your code reads and writes as a JavaScript `Date`, so it needs no `Temporal` polyfill. For creation and update times as a `Date`, use `field.temporal.createdAtJsDate()` and `field.temporal.updatedAtJsDate()`.
