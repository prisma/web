...
[PSL](/orm/contract-authoring/psl-syntax) is the preferred way to write the contract, and both forms produce the same two files, so you give up nothing by staying with PSL. Use the TypeScript builder for the cases PSL does not cover, and reach for it when:
...
If neither applies, write PSL, which is more compact and is what [`contract infer`](/cli/contract-infer) writes.
...
For a primary key whose value your app generates, use `field.id.uuidv4String()`, `field.id.uuidv7String()`, `field.id.ulid()`, `field.id.nanoid()`, `field.id.cuid2()`, or `field.id.ksuid()`. They all generate the ID in your app, and `uuidv4String` is the common choice. Each marks the field as the primary key for you, and `field.id.uuidv4Native()` and `field.id.uuidv7Native()` do the same in a `uuid` column. There is no auto-increment helper, so use one of those `field.id.*` helpers. For a key the database generates instead, write ``field.uuidNative().default(sql`gen_random_uuid()`).id()``, with `sql` imported from `@prisma/orm-postgres/contract-builder`.

`field.temporal.createdAt()` sets the column to the current time when the row is created, while `field.temporal.updatedAt()` sets it on every create and every update. Prisma ORM sets both in the client, from your application's clock, so the column has no database default, and a row you insert with raw SQL must supply the value. For a value the database generates, use `field.dateTime().default(now())` instead. `field.temporal.timestamptzJsDate()` is a `timestamptz` column that reads and writes a JavaScript `Date`, with no `Temporal` polyfill, and `field.temporal.createdAtJsDate()` and `field.temporal.updatedAtJsDate()` are the matching presets. `field.namedType(x)` takes an enum or a type from an extension pack.

Every field builder supports chained modifiers:

- `.optional()` makes the field nullable.
- `.default(value)` sets a literal default. It also takes a default the database computes: `.default(now())`, `.default(autoincrement())`, or any other SQL expression as a `sql` template, such as ``.default(sql`gen_random_uuid()`)``. Import `now`, `autoincrement`, and `sql` from `@prisma/orm-postgres/contract-builder`. `` sql`now()` `` and `` sql`autoincrement()` `` are refused; use the named helpers. `.defaultSql(expression)` still works but is deprecated, and it will be removed in `8.0.0`.
...
The ids, unique constraints, indexes, and foreign keys of one model must all have different names. A name used twice is a type error at `defineContract`.

For PostgreSQL [full-text search](/orm/reference/orm-client#full-text-search), `fullTextIndex` from `@prisma/orm-postgres/contract-builder` creates the GIN index the search operations use. It takes one text column and a `name` or `map`, plus optional `language` and `where`:

```typescript title="src/prisma/contract.ts (excerpt)"
...
Give the index and the search operations the same `language`, which defaults to `english`; otherwise PostgreSQL does not use the index.
...
TypeScript and [PSL](/orm/contract-authoring/psl-syntax) authoring produce the same `contract.json` and `contract.d.ts` for an equivalent contract, so you can move between the two forms without changing anything downstream. A project names exactly one contract file in its config.
...
Projects created with `npm create prisma@latest` include the [Prisma ORM skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent. In an existing project, run `npx prisma skills sync` to add them. The `prisma-8` skill covers TypeScript authoring, so ask your agent to:
