...
- `.default(value)` sets a fixed default. A decimal default is a string, as in `.default("1.50")`, and a JSON default is the object or array itself, as in `.default({ plan: "free" })`. A `field.dateTime()` default is the time as a string with a UTC offset, as in `.default("2024-01-01T00:00:00Z")`, because `.default()` does not accept the `Temporal.Instant` your code reads from that column.
...
For PostgreSQL [full-text search](/orm/reference/orm-client#full-text-search), `fullTextIndex` from `@prisma/orm-postgres/contract-builder` creates the GIN index that a text field's `fullTextMatches` and `fullTextRank` query functions use. It takes one text column and a `name` or `map`, plus optional `language` and `where`:
