...
The first non-blank line of every contract file is `// use prisma-8`, because `contract emit` reads only the `.prisma` files that start with it. When `contract` names one file and that file lacks the line, `contract emit` fails with `CONTRACT.SOURCE_LOAD_FAILED`, and the error's finding is `PSL_NO_OPTED_IN_SCHEMA_FILES`. When `contract` is a [glob that matches several files](#split-the-contract-across-several-files), a file without the line is left out of the contract without a warning, and `contract emit` fails only when none of the files has it.
...
Models declare fields with a type, an optional `?` marker, and attributes. [Scalar fields](/orm/data-modeling#scalar-fields) lists the types a field can hold, among them `String`, `Int`, `Boolean`, `Decimal`, `DateTime`, `Json`, and `Bytes`. On PostgreSQL, plain `String` is a `text` column, `Int` is `int4`, and `DateTime` is `timestamptz`, and you can write a PostgreSQL type wherever you would write `String` or `DateTime`: `VarChar`, `Char`, `Numeric`, `Timestamp`, `Timestamptz`, `Time`, `Timetz`, `Date`, `Uuid`, `Inet`, `SmallInt`, or `Real`.

On PostgreSQL, `DateTime`, `Timestamp`, `Timestamptz`, `Time`, and `Date` reach your code as `Temporal` values, such as a `Temporal.Instant` for `DateTime`, so your app needs a global `Temporal` to read and write them; [Scalar fields](/orm/data-modeling#scalar-fields) says how to check for it. When you want a string instead, use `DateString`, a `date` column your code gets as a string, or `TimestampString`, `TimestamptzString`, and `TimeString`, which do the same for `timestamp`, `timestamptz`, and `time`. `TimestamptzJsDate` is a `timestamptz` column your code gets as a JavaScript `Date`, so it needs no `Temporal`, but a `Date` drops the microseconds PostgreSQL stores.
...
- `@default(...)` sets a default. Database defaults are a literal, `@default(now())`, `@default(autoincrement())`, or a SQL expression written as a `sql` tagged literal, such as ``@default(sql`gen_random_uuid()`)``; the database fills them in. See [Default values](#default-values) for how to write each kind of literal. Generated defaults are `uuid()` (a version 4 UUID, the same as `uuid(4)`), `uuid(7)`, `cuid(2)`, `ulid()`, and `nanoid()`, and `nanoid(n)` sets the length, from 2 to 255. Prisma ORM computes them when it writes the row, and the database will not fill them in for you, so a row written by raw SQL or another application gets no value. Prisma ORM 7's `cuid()` is rejected with a hint to use `cuid(2)`.
...
- A JSON default is a `json` tagged literal, whose body is the JSON document itself. A JSON array is also one `json` literal, as `tags` shows.
...
- A text default is a quoted string, and so is a fixed date or time, as `startsOn` shows. On a column that reads as a `Temporal` value, such as `Date` or `DateTime`, `contract emit` checks the date with `Temporal`, so the Node.js that runs `contract emit` needs a global `Temporal`.
...
`@@fullTextIndex([field])` creates the GIN index that PostgreSQL [full-text search](/orm/reference/orm-client#full-text-search) uses. It takes one text field, and a `name:` or a `map:`:
...
`language:` sets the text-search configuration and defaults to `english`. A search sets its language with the `language` option of `fullTextMatches` and `fullTextRank`, as in `fullTextMatches(query, { language: "german" })`, and that option also defaults to `english`. Give the index and the search the same language, because PostgreSQL does not use the index for a search in another language. `where:` makes it a partial index, as on `@@index`. A field typed `pg.enum(...)`, a PostgreSQL enum type as [Enums](#enums) describes, cannot have a full-text index. PostgreSQL only.
