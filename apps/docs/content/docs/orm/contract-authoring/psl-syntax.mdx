...
Every contract file starts with `// use prisma-8`. `contract emit` reads only the files that start with this line, so keep it at the top. A file without it is left out of the contract without a warning, and when no file has it, `contract emit` fails with `PSL_NO_OPTED_IN_SCHEMA_FILES`.
