...
Every contract file starts with `// use prisma-8`. `contract emit` reads only the files that start with this line, so keep it at the top. A file without it is left out of the contract without a warning, and when no file has it, `contract emit` fails with `PSL_NO_OPTED_IN_SCHEMA_FILES`. `// use prisma-next`, from earlier release candidates, still counts.
...
Set `contract` to a glob, such as `"./src/prisma/**/*.prisma"`, to write the contract in several files. Every matching file that starts with `// use prisma-8` becomes part of one contract, and a file you add later joins it on the next `contract emit`, with no change to the config. A model in one file can refer to a model in another. `contract emit` writes `contract.json` and `contract.d.ts` into the fixed part of the glob, here `./src/prisma/`. See [Configuration](/cli/configuration#split-the-schema-across-several-files).

## Models and fields

Models declare fields with a type, an optional `?` marker, and attributes. [Scalar fields](/orm/data-modeling#scalar-fields) lists the types a field can hold, among them `String`, `Int`, `Boolean`, `Decimal`, `DateTime`, `Json`, and `Bytes`. You can write a PostgreSQL type wherever you would write `String` or `DateTime`. The PostgreSQL types you can write are `VarChar`, `Char`, `Numeric`, `Timestamp`, `Timestamptz`, `Time`, `Timetz`, `Date`, `Uuid`, `Inet`, `SmallInt`, and `Real`, plus `DateString`, `TimestampString`, `TimestamptzString`, and `TimeString`, which store the same columns but read back as text, and `TimestamptzJsDate`, a `timestamptz` column that reads and writes a JavaScript `Date` and needs no `Temporal` polyfill. A `Date` keeps only milliseconds. On PostgreSQL, plain `String` is a `text` column, `Int` is `int4`, and `DateTime` is `timestamptz`.
...
A value you write in `@default(...)` has a data type, and how you write it decides which one. A column takes the value only if its own data type accepts that type; otherwise `contract emit` fails with `PSL_DEFAULT_TYPE_INCOMPATIBLE` and names the types the column accepts. A quoted string is text, so it does not work as a default for a JSON, decimal, or floating-point column:
...
- A JSON default is a `json` tagged literal, whose body is the JSON document itself. A JSON list on a column that holds one JSON value is also one `json` literal, as `tags` shows.
- A decimal default is a bare number, and it keeps every digit you write, trailing zeros included.
- `NaN`, `Infinity`, and `-Infinity` are written without quotes.
- A text default, or the value of an enum member, is a quoted string.
...
Prisma ORM 7's `dbgenerated("...")` is not accepted, and `contract emit` reports `PSL_UNKNOWN_DEFAULT_FUNCTION` with the replacement. [`contract infer`](/cli/contract-infer) writes every default in these forms.
...
`language:` sets the text-search configuration and defaults to `english`. Give the index and the query operations the same language, because PostgreSQL does not use the index for a query in another language. `where:` makes it a partial index, as on `@@index`. A `pg.enum(...)` column cannot have a full-text index. PostgreSQL only.
...
PostgreSQL has no `LIKE`, `ILIKE`, or text search for an `enum` type, so a `pg.enum(...)` column has no text operations: `like`, `ilike`, and the full-text operations are type errors on it. Compare it with `eq` or `in` instead. An `enum` block stored as text, such as `Priority` above, keeps every text operation.
...
Models outside any block are in `public`. In queries, the block name is the segment after `db.orm`, so this model is `db.orm.audit.AuditLog`. Two `namespace` blocks with the same name in one file are one namespace, so you can open a block again further down the file. A relation can point at a model that another extension pack owns in a different schema, written `<pack>:<schema>.<Model>`, for example `supabase:auth.AuthUser`; the [Supabase extension](/orm/extensions) documents that form.
