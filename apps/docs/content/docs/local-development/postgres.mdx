...
If you want to connect via Prisma ORM, hit <kbd>h</kbd> on your keyboard and copy both `DATABASE_URL` and `SHADOW_DATABASE_URL` into your `.env` file. Prisma ORM connects through the first one. Prisma Migrate uses the second one as the [shadow database](/orm/v7/prisma-migrate/understanding-prisma-migrate/shadow-database) when it checks for drift. Without it, the first `prisma migrate dev` run works but later runs fail with a `relation "..." already exists` error.
...
Then point the `datasource` block of your Prisma config at both variables:
...
If the first `prisma migrate dev` run against a fresh instance fails with `Error: P1017 Server has closed the connection` before it creates any tables, you have hit [prisma/prisma#29366](https://github.com/prisma/prisma/issues/29366): the query that reads the `_prisma_migrations` table drops the connection when that table does not exist yet. Create the table yourself, then run the command again:

```sql title="create-migrations-table.sql"
...
```npm
npx prisma db execute --file create-migrations-table.sql
```

`prisma db push` does not read that table, so it works as an alternative if you do not need migration files yet.
