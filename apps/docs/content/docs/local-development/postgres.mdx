...
If the first `prisma migrate dev` run against a fresh instance fails with `Error: P1017 Server has closed the connection` before it creates any tables, you have hit [prisma/orm#29366](https://github.com/prisma/orm/issues/29366): the query that reads the `_prisma_migrations` table drops the connection when that table does not exist yet. Create the table yourself, then run the command again:
