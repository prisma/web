...
```npm
npx prisma service show
```

## How do I deploy?

Deployments come from a git push, the [Console](https://pris.ly/pdp), or the [`deploy` command](/cli/deploy) for a [Prisma Composer](/composer) app. [Connect a GitHub repository](/compute/github) and push, or declare the app with Composer and run `npx prisma deploy module.ts` for CLI-driven deploys.
...
```bash
PRISMA_SERVICE_TOKEN=... npx prisma service show \
  --project my-app \
  --service web \
  --json \
  --no-interactive
...
```npm
npx prisma service domain add shop.acme.com --service web
npx prisma service domain wait shop.acme.com --service web
```
