...
No, it links a project to a GitHub repository and turns on automation for future branch and push events. It does not deploy anything by itself, so if you want to ship right now, run `composer deploy`. To set the connection up, see the [GitHub integration docs](/compute/github).
...
Your first deploy is promoted to production automatically. After that, deploying to the production branch needs an explicit `--prod` flag. Re-run with `npx @prisma/cli@next composer deploy --prod`, or deploy from a preview branch instead.
