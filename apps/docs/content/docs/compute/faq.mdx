...
Yes, Compute is free to use during public beta. We've published the rates we expect to charge once production billing turns on, so you can understand the model and estimate future usage before then. See [Pricing](https://pris.ly/pricing-compute) for the expected rates, worked examples, and what is not billed separately.
