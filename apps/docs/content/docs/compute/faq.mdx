...
Deployments come from a git push, the [Console](https://pris.ly/pdp), or the [`deploy` command](/cli/v8/deploy) for a [Prisma Composer](/composer) app. [Connect a GitHub repository](/compute/github) and push, or declare the app with Composer and run `npx prisma@next deploy module.ts` for CLI-driven deploys.
