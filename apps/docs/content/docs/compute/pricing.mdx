...
These examples are estimates, not benchmarks. The inputs are round numbers so the math is easy to follow, and every meter is priced from zero — in practice your plan's included requests cover the request line first.
...
Database usage is available from the [CLI](/cli/v8/postgres) (`postgres usage`) and API, including as JSON, and an agent can read it to estimate a cost, explain a change, or suggest a fix. Equivalent usage reporting for Compute is still being built.
