...
The `prisma-compute` skill teaches your agent the Compute workflow (auth, config, deploys, logs, and domains), so it follows the right steps. Supported agents pick it up automatically. See [Agent Skills](/ai/tools/skills) for the full catalog. The Prisma CLI's own [`skills` commands](/cli/v8/skills) manage a different set: the skills that ship inside the Prisma packages your project installs, such as Composer's.
