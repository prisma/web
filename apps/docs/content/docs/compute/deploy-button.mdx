...
Variables from the URL come first and keep their examples; config entries are added after them. Since your repository is public, treat those committed values like `env-example` values: placeholders only, never real keys.
