...
Prisma Compute is in [Public Beta](/console/more/feature-maturity#public-beta). You manage it with the unified [Prisma CLI](/compute/cli-reference) (published as `@prisma/cli@next` until launch). Deployments come from a git push, the [Console](https://pris.ly/pdp), [Prisma Composer](/composer), or a one-shot `app deploy` with the earlier beta CLI (`@prisma/cli@latest`); see [Deployments](/compute/deployments).
...
The default Git branch (usually `main`) is the **production** branch. Every other branch is a **preview** branch with isolated resources, so preview work never touches production. After you [connect a GitHub repository](/compute/github), pushing to a branch builds and deploys that branch automatically.
