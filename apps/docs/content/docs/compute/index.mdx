...
Once you have deployed something, [Deployments](/compute/deployments) covers logs, promoting, and rolling back, and [Environment variables](/compute/environment-variables) covers configuration per scope. [Configuration](/compute/configuration) documents every field of `prisma.compute.ts`, and the [CLI reference](/cli/v8) every command and flag. Before relying on the beta in production, read the [known limitations](/compute/limitations).
