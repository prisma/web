...
Platform builds and the [`service` commands](/cli/service) read the file. It never selects your project, branch, or production target. You select those explicitly. It also does not configure a database. See [Environment variables](/compute/environment-variables) for the connection string.
