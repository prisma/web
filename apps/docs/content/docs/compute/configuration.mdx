...
- **Deploy everything** with a bare `app deploy` — it deploys every app in order. This is the default when no target is named or inferred:
...
`app build` and `app run` always target a single app in a multi-app config — unlike `deploy`, they have no all-apps form. Pass a target, or run from inside the app's `root`.
