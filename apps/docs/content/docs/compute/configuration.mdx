...
The file is read by platform builds and the [`service` commands](/cli/v8/service). It never selects your project, branch, or production; those stay explicit. It also does not configure a database; see [Environment variables](/compute/environment-variables) for the connection string.
