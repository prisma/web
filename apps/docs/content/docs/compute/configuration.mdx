...
The file is read by platform builds and the [`service` commands](/compute/cli-reference#service). It never selects your project, branch, or production; those stay explicit. It also does not configure a database; see [Environment variables](/compute/environment-variables) for the connection string.

## A minimal config

Export a single `app` with `defineComputeConfig`. The config keys are named `app` and `apps`; each entry describes what the rest of the docs and the CLI call a **service** (the `service` commands read this file):
