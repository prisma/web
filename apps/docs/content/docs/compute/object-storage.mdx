...
You can manage buckets from the [Console](https://pris.ly/pdp), with the CLI's [`bucket` commands](/cli/v8/bucket), or over the [Management API](/management-api) with a [service token](/management-api/authentication). This page uses the API, since that is what you would automate against. The Console and CLI cover the same operations, so the steps map one to one.
