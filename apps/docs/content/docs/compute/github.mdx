...
```npm
npx @prisma/cli@next git connect https://github.com/acme/shop
```
...
```npm
npx @prisma/cli@next git connect https://github.com/acme/shop --json --no-interactive
```

Disconnect when you're done:

```npm
npx @prisma/cli@next git disconnect
```
...
Auto-deploy handles monorepos that declare several services in `prisma.compute.ts`: one pushed commit fans out into a targeted build per service. Watch a build from CI or a terminal with a service token and explicit targets:
