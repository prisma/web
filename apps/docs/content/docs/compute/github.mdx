...
For an app with several services, declare them with [Prisma Composer](/composer) and deploy from CI with the [`deploy` command](/cli/v8/deploy). Watch a service from CI or a terminal with a service token and explicit targets:
