...
- The platform command groups are `auth`, `project` (including `project env`), `postgres`, `bucket`, `branch`, `git`, `service` (including `service version` and `service domain`), and `agent`, plus the root-level [`dev`](/cli/v8/dev) and [`deploy`](/cli/v8/deploy) verbs for Composer apps. The same binary also carries the [Prisma 8 data commands](/cli/v8) (`contract`, `db`, `migration`). There is no `compute` namespace: Compute is managed through the resource groups above.
