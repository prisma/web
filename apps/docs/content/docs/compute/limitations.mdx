...
- The command groups are `version`, `init`, `agent`, `auth`, `project` (including `project env`), `git`, `branch`, `build`, `postgres`, and `app`. There is no `schema` or `migrate` command, and no product-branded `compute` namespace.
...
- `composer deploy --framework` accepts `nextjs`, `nuxt`, `astro`, `hono`, `nestjs`, `tanstack-start`, `custom`, and `bun`.
...
- `build logs` streams the live deployment or a specific one.
...
- No multi-region deployments. Each app lives in one region, chosen at creation with `composer deploy --region`: `us-east-1` (the default), `us-west-1`, `eu-west-3`, `eu-central-1`, `ap-northeast-1`, or `ap-southeast-1`.
