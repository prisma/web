...
- Committed configuration lives in [`prisma.compute.ts`](/compute/configuration) (or `prisma.compute.json`). `.prisma/local.json` is a gitignored local pin of the workspace and project, and `PRISMA_PROJECT_ID` / `PRISMA_SERVICE_ID` override the pinned project and service for CI.
...
- These docs cover deployment and runtime config only: not schema migrations or data cloning. Databases are managed with the [`postgres` commands](/compute/cli-reference#postgres).
