...
Prisma Compute apps run on Bun. Bun includes [`Bun.Image`](https://bun.com/docs/runtime/image), a native image pipeline for decoding, resizing, rotating, flipping, adjusting, and re-encoding images.

Use it when you want image transformations to be part of your app instead of a separate image transformation service. A transformation route on Compute is billed and operated like any other app route: it handles a request, uses CPU and memory, and returns bytes.
...
Portable Compute routes should prefer JPEG, PNG, and WebP. Bun's HEIC and AVIF support depends on operating-system image backends, so do not assume those formats are available unless you add and test your own encoder path.

## Minimal service

Create a Hono or plain Bun HTTP app. This example uses Hono because it matches many Compute examples, but the `Bun.Image` code is the same in any framework.
...
Use placeholders for low-quality image placeholders in HTML or React components.
