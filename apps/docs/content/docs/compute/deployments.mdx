...
- **[Prisma Composer](/composer).** `composer deploy` does not build your services; it assembles the services you have already built and provisions them. This is the CLI-driven path for multi-service apps.
...
```npm
npx prisma@latest service show --service web
npx prisma@latest service open --service web
npx prisma@latest service deployment list --service web
npx prisma@latest service deployment show dep_123
```
...
```npm
npx prisma@latest build logs bld_123 --follow
```
...
```npm
npx prisma@latest service deployment promote dep_123 --service web
```
...
```npm
npx prisma@latest service deployment rollback --service web
npx prisma@latest service deployment rollback --service web --to dep_123
```
...
```npm
npx prisma@latest service deployment stop dep_123
npx prisma@latest service deployment start dep_123
npx prisma@latest service deployment delete dep_123
```
...
```npm
npx prisma@latest service remove --service web
```
