...
The unified CLI does not have a standalone `deploy` command for a single service; it manages the deployments those flows create. During the beta, the earlier CLI (`@prisma/cli@latest`) still offers a one-shot `app deploy` that builds and deploys a single service from your terminal; the deployments it creates are managed with the same commands below.
