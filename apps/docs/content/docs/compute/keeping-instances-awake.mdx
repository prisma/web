...
export default {
  fetch(req: Request) {
    waitUntil(processInBackground(req), {
      // Optional. Prevents a hanging Promise from indefinitely preventing sleep
...
`waitUntil` follows the same pattern as [Cloudflare Workers](https://developers.cloudflare.com/workers/runtime-apis/context/#waituntil) and the [Service Workers API](https://developer.mozilla.org/en-US/docs/Web/API/ExtendableEvent/waitUntil): the instance stays awake until the promise settles. Prisma Compute adds an optional caller-owned `signal` value so you can bound the keep-awake lifetime if a promise hangs or never settles.
...
`KeepAwakeGuard` accepts a caller-owned `signal` option. Use it as a safety fallback to release the guard if your code does not reach `release()`.
...
The `signal` option is a fallback, not the primary way to control your work. Prefer `using` or `try`/`finally` for normal guard lifetimes. Use `signal` to cap the maximum lifetime of a guard and avoid dangling guards.
...
Keeps the instance awake until `promise` settles. Pass `options.signal` as a caller-owned fallback to release the keep-awake lifetime if the promise hangs or never settles.
...
Creates a guard that keeps the instance awake until the guard is disposed, released, or its optional caller-owned `signal` aborts.
