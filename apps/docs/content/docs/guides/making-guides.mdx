...
If a topic cannot be ported because Prisma 8 does not support it yet (for example Cloudflare D1, or a third-party adapter that requires Prisma Client), move the page under `guides/v7/`, add its filename to the matching `guides/v7/<category>/meta.json` and its link to `guides/v7/index.mdx` (a redirect alone leaves it out of the Prisma 7 navigation), and add a redirect from the old URL in the live region of `redirects()` in `apps/docs/next.config.mjs`, near the `/llms/next.txt` entry (`apps/docs/vercel.json` is reserved for the generated REST API endpoint redirects). Then run `pnpm run audit:redirects:strict` in `apps/docs`, which also catches legacy redirects whose destination just moved. Do not leave a Prisma 7 page at a Prisma 8 URL without a version marker.
