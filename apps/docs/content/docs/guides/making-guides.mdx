...
- For ` ```text ` and ` ```dotenv ` blocks, use fence metadata such as `highlight=2;add showLineNumbers` or `highlight=1;delete|3;add showLineNumbers`
...
If you need to show changes in a plain-text file, use fence metadata instead of inline `[!code ...]` markers:
