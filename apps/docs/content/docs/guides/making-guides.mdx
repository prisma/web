...
- Use inline `[!code ++]`, `[!code --]`, and `[!code highlight]` comments only in languages where those comments are valid
- For ` ```text ` and ` ```dotenv ` blocks, describe the change in prose and show the final file contents instead of relying on `[!code ...]`, `highlight=...`, or `showLineNumbers`
...
If you need to show changes in a plain-text file, explain the change in prose and then show the final file contents:
