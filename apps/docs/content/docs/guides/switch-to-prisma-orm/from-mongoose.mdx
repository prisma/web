---
title: Mongoose
description: 'Migrate an existing Mongoose app to Prisma ORM step by step, against the same MongoDB database, without moving any data.'
...
metaDescription: 'Add Prisma ORM to a Mongoose project, describe your collections in a contract, replace Mongoose queries and populate calls with typed Prisma ORM queries, and retire Mongoose.'
---
...
Prisma ORM 8 is the current release. Prisma ORM 7 remains fully supported but has no MongoDB connector; the Prisma ORM 7 version of this guide, which uses Prisma ORM 6.19 for MongoDB, is at [/guides/v7/switch-to-prisma-orm/from-mongoose](/guides/v7/switch-to-prisma-orm/from-mongoose).
...
### 1.3. Add Prisma ORM to the project
...
✔ npm add -D @prisma/cli-engine@0.4.0
✔ Emit the contract
│  target:     mongodb
│  authoring:  psl
...
Install the [Prisma agent skills](/cli/skills) for your coding agent with [`init`](/cli/init), which also adds a `postinstall` hook that resyncs them on every install:
...
## 3. Replace Mongoose queries with Prisma ORM
...
### 4.3. Let Prisma ORM own the schema
...
- [Understand migrations](/orm/migrations/how-migrations-work) now that Prisma ORM owns the schema.
