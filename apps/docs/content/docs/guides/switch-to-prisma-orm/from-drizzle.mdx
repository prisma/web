---
title: Drizzle
description: 'Switch an existing Drizzle app to Prisma ORM: infer a contract from your database, sign it, and replace Drizzle queries route by route.'
...
metaDescription: 'Move a Drizzle app to Prisma ORM without touching the database: run orm init, infer and review a contract, sign the database, and swap the queries one route at a time.'
---
...
Prisma ORM 8 is the current release. Prisma ORM 7 remains fully supported; the Prisma ORM 7 version of this guide is at [/guides/v7/switch-to-prisma-orm/from-drizzle](/guides/v7/switch-to-prisma-orm/from-drizzle).
...
## 1. Add Prisma ORM to the project
...
- In a long-running server, do not call `db.close()` in route handlers; the connection pool is shared across requests. Close it only on process shutdown, and never in a Next.js route.
...
- [Evolve the schema with migrations](/orm/migrations/how-migrations-work) now that Prisma ORM owns it.
