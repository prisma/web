...
This guide is written for `@prisma/orm-mongo@8.0.0-rc.12`. MongoDB support in Prisma ORM 8 still changes between release candidates, so a newer version may behave differently from what this guide shows.
...
|---------|----------------------|---------------------------------|
| First line | (nothing) | `// use prisma-8`. `contract emit` reads only the files that start with it |
