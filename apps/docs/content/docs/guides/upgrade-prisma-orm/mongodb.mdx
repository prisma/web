...
Prisma ORM MongoDB support evolves quickly, so check anything below against the `@prisma/orm-*` version you install. This guide targets `@prisma/orm-mongo@8.0.0-rc.12`.
...
Prisma ORM 8 addresses collections by **storage name**, not model name: `db.orm.users` (the `@@map` name, or the model name exactly as written), never `db.orm.User` when the model maps to `users`. Whatever you `@@map` in this step is the exact name every client call uses in step 3.
