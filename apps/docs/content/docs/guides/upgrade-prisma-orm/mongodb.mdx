...
- **A replica set** if your app uses [transactions](/orm/fundamentals/transactions). Prisma ORM 8's ORM doesn't wrap `$transaction` yet, so multi-document atomicity goes through the driver's sessions (see step 3d), which MongoDB only allows on a replica set.
