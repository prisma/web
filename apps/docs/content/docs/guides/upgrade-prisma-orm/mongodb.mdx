...
- **A replica set** if your app uses [transactions](/orm/fundamentals/transactions). The ORM API in Prisma ORM 8 doesn't wrap `$transaction` yet, so multi-document atomicity goes through the driver's sessions (see step 3d), which MongoDB only allows on a replica set.

:::info

Install the `latest` version of `@prisma/orm-mongo`, which is what `orm init` in step 1 does. This guide was written for `8.0.0-rc.12`, and MongoDB support in Prisma ORM 8 still changes between release candidates, so a newer version may behave differently from what this guide shows. To see which version you have, run `npm ls @prisma/orm-mongo`.
...
Run `npx prisma contract emit` to generate `contract.json` from your `.prisma` contract file. Re-run this whenever you change the contract.
...
On MongoDB, `db.orm` holds your models by collection name. The collection name is the model's `@@map` value, or the model name exactly as written when the model has no `@@map`. So a model `User` with `@@map("users")` is `db.orm.users`, and `db.orm.User` does not exist. PostgreSQL works differently: there `db.orm` uses the model name, as in `db.orm.public.User`. Whatever you `@@map` in this step is the exact name every client call uses in step 3.
...
|---------|----------------------|---------------------------------|
| First line | (nothing) | `// use prisma-8`. Without it, `contract emit` fails with `CONTRACT.SOURCE_LOAD_FAILED` |
...
Prisma ORM 8 has no `transaction` method for MongoDB yet. For multi-document transactions, use the `mongodb` driver's [Transaction API](https://www.mongodb.com/docs/drivers/node/current/crud/transactions/transaction-conv/).
...
Your database already has collections and data, so bootstrap it with `db update`. The command diffs the live database against your contract, applies the delta (the strict validators, plus any indexes the contract declares that v6 never created), and signs the database so Prisma ORM 8 recognizes it from then on. The `--advance-ref db` flag records the resulting contract state as the [`db` ref](/orm/migrations/the-migration-graph#name-important-states-with-refs), the starting point later migration plans diff from.

```bash
npx prisma contract emit
npx prisma db update --db "$DATABASE_URL" --dry-run   # preview the delta first
...
```bash
npx prisma contract emit                             # planning diffs the emitted contract
npx prisma migration plan --name add_posts_indexes   # plans from the db ref
npx prisma db migrate --db "$DATABASE_URL" --advance-ref db
npx prisma db verify --db "$DATABASE_URL"   # check the DB matches the contract
```
...
- Prisma ORM 8 adds strict `$jsonSchema` validators by default. Make sure existing documents pass them before running in production: once the validators are live, writes to documents that don't match the contract fail with `Document failed validation`.
...
- **Validators.** Your existing documents pass the strict validators Prisma ORM 8 adds to each collection (see *Good to know* in step 4).
...
- **Read-only trial run.** Run Prisma ORM 8 read-only alongside your v6 app for a while and compare the results. This surfaces any differences while v6 is still handling all the writes.
...
See the [Prisma ORM docs](/orm) for [pipeline builder patterns](/orm/reference/pipeline-builder), [relation loading strategies](/orm/reference/orm-client), and [advanced contract features](/orm/contract-authoring/psl-syntax). This guide gets you across the bridge. From here on, the Prisma ORM docs are the source of truth.
