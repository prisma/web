...
**Expected result:** `8.0.0-rc.17` (or newer). The CLI's version line is separate from `@prisma/orm-postgres`'s.
...
Prisma ORM 8 can also read `prisma/schema.prisma` directly, so you keep one schema file while Prisma ORM 7 owns migrations. Set `contract: prisma7Schema("prisma/schema.prisma")` in `prisma.config.ts`, with `prisma7Schema` imported from `@prisma/orm-postgres/config`, and skip steps 2.3 and 2.4. After each `prisma7 migrate dev`, run `npx prisma contract emit` and then `npx prisma db sign`. [`prisma orm init --from-prisma7-schema prisma/schema.prisma`](/cli/orm-init#on-a-prisma-orm-7-project) sets this up, and does phase 1 as well. [Use a Prisma ORM 7 schema](/cli/configuration#use-a-prisma-orm-7-schema) lists what Prisma ORM 8 cannot read. The steps in phase 4 are written for the inferred `prisma8/contract.prisma`.

:::

### 2.4. Edit the inferred contract

Delete the `PrismaMigrations` model. `contract infer` picks up Prisma ORM 7's `_prisma_migrations` ledger table. Prisma ORM 8 must not manage it, and extra tables in the database are fine. Remove the whole model.

The models need no `@@map`. A model without `@@map` names its table exactly as written, so `User` reads and writes the `"User"` table that Prisma ORM 7 created.
...
Keep `// use prisma-8` as the first line: `contract emit` reads only files that start with it.
