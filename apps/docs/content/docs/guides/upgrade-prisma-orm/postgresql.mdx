...
This guide targets `prisma@latest`, `@prisma/orm-postgres@8.0.0-rc.12`, and `@prisma/prisma7@7.10.0`, and the example app in step 1.1 starts on Prisma ORM 7.10.0. `prisma@latest` is the Prisma ORM 8 CLI, `8.0.0-rc.17` at the time of writing. It is versioned and released separately from the ORM packages, so its version number will not match theirs. It installs `@prisma/cli-engine@0.6.1` as its own dependency, so you do not install that package yourself.
...
Prisma ORM 8 can also read `prisma/schema.prisma` directly as its contract, so you keep one schema file while Prisma ORM 7 owns migrations. On this path `schema.prisma` does not need a `// use prisma-8` line. Skip steps 2.3 and 2.4, and change two lines in `prisma.config.ts`:
...
After step 2.5, run `npx prisma db sign`, which checks that the database matches the contract Prisma ORM 8 read from `schema.prisma` and records in the database that it does. Step 4.1 describes `db sign` in full. Because Prisma ORM 7 keeps changing the database on this path, run `npx prisma contract emit` and then `npx prisma db sign` again after each `prisma7 migrate dev`. [Use a Prisma ORM 7 schema](/cli/configuration#use-a-prisma-orm-7-schema) lists the parts of a Prisma ORM 7 schema that Prisma ORM 8 cannot read.

If you have not started phase 1 yet, [`prisma orm init --from-prisma7-schema prisma/schema.prisma`](/cli/orm-init#on-a-prisma-orm-7-project) does phase 1 and this setup in one command.

Phase 4 is written for the inferred `prisma8/contract.prisma`, and does not yet cover a project that reads `schema.prisma` through `prisma7Schema(...)`.
...
Keep `// use prisma-8` as the first line of `prisma8/contract.prisma`, because `contract emit` reads only the `.prisma` files that start with it. A `schema.prisma` that Prisma ORM 8 reads through `prisma7Schema(...)`, as in the tip under step 2.3, does not need the line.
