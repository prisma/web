...
- **TypeScript 5.9 or newer** with `"strict": true` and a `module` setting that supports import attributes; [step 2.6](#26-include-the-generated-types) gives the setting for each kind of project
...
The Prisma ORM 8 client imports `contract.json` with the `with { type: "json" }` import attribute. TypeScript accepts that attribute only when `module` is `esnext`, `node18`, `node20`, `nodenext`, or `preserve`. Prisma ORM 8 does not need `nodenext` in particular. Start from your current setup:
...
- **A project run through `tsx`, `vite`, `next`, `esbuild`, or another bundler.** Use `"module": "preserve"` and `"moduleResolution": "bundler"` with `"resolveJsonModule": true`. This accepts imports without extensions. It is also what `prisma orm init` writes.
