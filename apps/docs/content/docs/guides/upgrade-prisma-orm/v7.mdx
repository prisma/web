...
:::note[Upgrading to Prisma ORM 8?]

Prisma ORM 8 is the current release. If you are moving a Prisma ORM 7 project to Prisma ORM 8, use the [PostgreSQL](/guides/upgrade-prisma-orm/postgresql) or [MongoDB](/guides/upgrade-prisma-orm/mongodb) upgrade guide instead.
...
The `output` field is now **required** in the generator block. Prisma Client will no longer be generated in `node_modules` by default. You must specify a custom output path.
...
This change is part of the move to make the main Prisma Client as lean and open as possible. For
...
If you complete the Prisma ORM 7 upgrade before removing Accelerate, you can keep the Accelerate URL and extension temporarily. Do **not** pass the Accelerate URL to a driver adapter, because `PrismaPg` expects a direct database connection string and will fail with `prisma://` or `prisma+postgres://`.
