...
If you used Prisma Accelerate (including Prisma Postgres' `prisma+postgres://` URLs) in v6, keep using the Accelerate URL with the Accelerate extension. Do **not** pass the Accelerate URL to a driver adapter, because `PrismaPg` expects a direct database connection string and will fail with `prisma://` or `prisma+postgres://`.
