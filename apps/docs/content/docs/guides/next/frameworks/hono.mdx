...
3. Start `npm run dev` and verify `curl http://localhost:3000/users` returns the seeded users.
...
5. Deploy: check `npx @prisma/cli@latest auth whoami`; if I am not signed in, stop and ask me to run `npx @prisma/cli@latest auth login`. Then run `npx @prisma/cli@latest app deploy --env .env` and verify the live URL's /users endpoint.
