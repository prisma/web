...
3. Start `npm run dev` and verify http://localhost:4321 lists the seeded users and `curl http://localhost:4321/api/users` returns them as JSON.
...
5. Deploy: check `npx @prisma/cli@latest auth whoami`; if I am not signed in, stop and ask me to run `npx @prisma/cli@latest auth login`. Then run `npx @prisma/cli@latest app deploy --env .env --env HOST=0.0.0.0` (without HOST the live URL answers `Service not found`) and verify the live URL's /api/users endpoint with curl.
...
The live URL's root page renders the same users server-side. The first deploy creates a project, pins the directory to it in a gitignored `.prisma/local.json`, and promotes to production. For previews per Git branch and deploy-on-push, see [Deploy your first app](/prisma-compute/deploy).
