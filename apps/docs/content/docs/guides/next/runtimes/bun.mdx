---
title: Bun
description: Scaffold a Prisma Next app with Bun, run your first typed query, serve users over HTTP, and deploy to Prisma Compute.
url: /guides/next/runtimes/bun
metaTitle: How to use Prisma Next with Bun
metaDescription: Set up a Prisma Next project with Bun and Prisma Postgres, from scaffold to first typed query, then serve users over HTTP and deploy to Prisma Compute.
---
...
1. Scaffold: `bunx create-prisma@next create my-bun-app --provider postgres --package-manager bun --prisma-postgres --yes` (or pass `--database-url "<url>"` instead of `--prisma-postgres` if I give you a connection string).
...
5. Deploy: check `npx @prisma/cli@latest auth whoami`; if I am not signed in, stop and ask me to run `npx @prisma/cli@latest auth login`. Then run `npx @prisma/cli@latest app deploy --env .env --framework bun --entry src/server.ts` and verify the live URL's /users endpoint.
