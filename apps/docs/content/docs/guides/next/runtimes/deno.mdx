...
2. In `my-deno-app`, run `deno install`, then `deno run -A --env-file=.env npm:prisma-next contract emit` and `deno run -A --env-file=.env npm:prisma-next db init`.
...
4. Replace `src/index.ts` with a script that creates a user and reads all users back, following https://www.prisma.io/docs/guides/next/runtimes/deno.md. Keep the `.ts` extension on relative imports. Verify `deno task dev` prints the created user and the user count.
