...
```text
Create a new Prisma Next project on Deno, seed it, and run a first typed query.

1. Scaffold: `deno run -A npm:create-prisma@next create my-deno-app --provider postgres --package-manager deno --prisma-postgres --yes` (or pass `--database-url "<url>"` instead of `--prisma-postgres` if I give you a connection string).
