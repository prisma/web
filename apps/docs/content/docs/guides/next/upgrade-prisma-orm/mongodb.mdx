...
1. **Set up**: scaffold Next and add the MongoDB driver.
1. **Port the schema**: convert your v6 models to a Prisma Next contract.
1. **Port the code**: update client calls, transactions, and aggregations.
1. **Adopt migrations**: replace the `db push` workflow.
1. **Verify**: check index parity and validators, then soak-test reads.
1. **Cut over**: a code-only switch.
