...
```npm
npm install prisma@prev typescript tsx @types/node @types/pg --save-dev
...
The flag may also initialize a Git repository in the `web` folder. If that happens, please remove the `.git` directory by running `rm -r .git`.
