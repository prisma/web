...
6. For the production database, run `npx prisma migration check`, `npx prisma db migrate --show --db "$PRODUCTION_DATABASE_URL"`, then `npx prisma db migrate --db "$PRODUCTION_DATABASE_URL"`. Do not use `db update` against production.
```
...
Commit `migration.ts`, `ops.json`, and `migration.json` together. `npx prisma migration check` fails in CI if they drift apart.
