...
If your database is managed with [Prisma Next](/orm/next), Studio also shows a **Migrations** view alongside your tables, with a visual diff of what each applied migration changed. See [Studio with Prisma Next](/studio/prisma-next).
