...
If your database is managed with [Prisma 8](/orm/next), Studio also shows a **Migrations** view alongside your tables, with a visual diff of what each applied migration changed. See [Studio with Prisma 8](/studio/prisma-next).
