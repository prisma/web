...
The `allowBuilds` block matters. pnpm does not run dependency install scripts unless you approve them, and since pnpm 10.3 an unapproved script fails the install (`strictDepBuilds` is on by default). The Prisma 8 CLI's toolchain pulls in three packages with install scripts, so approve them up front. The `allowBuilds` key exists since pnpm 10.26, so use that version or later; without the key you get `ERR_PNPM_IGNORED_BUILDS` the first time `orm init` runs `pnpm add`.
