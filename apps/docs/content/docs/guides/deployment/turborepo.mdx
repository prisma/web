...
pnpm does not run dependency build scripts until you allow them, and since pnpm 10.3 it fails the install when it finds scripts it was not told about. The `allowBuilds` key exists since pnpm 10.26, so use that version or later. Prisma 8's toolchain ships three packages with build scripts, so declare them in `pnpm-workspace.yaml` before you install anything:
