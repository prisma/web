...
```bash
npx prisma@next orm init --yes --target mongodb --authoring psl
```

`init` writes a `prisma.config.ts` at the repo root. MongoDB is selected by importing the `@prisma/orm-mongo` facade, not by a `provider` string in the schema. Point the connection at the **same database** your v6 app uses.
...
```bash
npx prisma@next contract emit                             # planning diffs the emitted contract
