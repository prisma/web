...
5. Deploy: check `bunx prisma@next auth whoami`; if I am not signed in, stop and ask me to run `bunx prisma@next auth login`. Then point the `build` script at the server (`esbuild src/server.ts --bundle --platform=node --format=esm --outfile=dist/server.mjs`), run `bun run build`, then `bunx prisma@next deploy module.ts`, and verify the live URL's /users endpoint returns `[]`. The deployed app provisions its own fresh Prisma Postgres database, so the local users are not there; do not pass the local DATABASE_URL. Do not use the scaffold's `deploy` script, which still calls the removed `composer` subcommand.
...
```bash
bun run build
bunx prisma@next deploy module.ts
```
...
The deploy creates a project named after your module and provisions its own Prisma Postgres database on the platform, declared in `module.ts`; the users you created locally are not in it. Verify the live endpoint responds with an empty list from the fresh database:
...
The scaffold's `deploy` and `dev:composer` package scripts still call the CLI's former `composer` subcommand, which the current release candidate removed. Run `bunx prisma@next deploy module.ts` and `bunx prisma@next dev module.ts` directly until the template catches up.

:::

## Prompt your coding agent

Run [`bunx prisma@next init`](/cli/v8/init) once to install the [Prisma 8 skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent and keep them matching your installed packages. Prompts that map to this guide:
