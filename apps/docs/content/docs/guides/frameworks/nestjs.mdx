---
title: NestJS
description: 'Set up Prisma ORM in a NestJS app with create-prisma, from scaffold to seeded API to a live deploy on Prisma Compute.'
url: /guides/frameworks/nestjs
metaTitle: How to use Prisma ORM with NestJS
metaDescription: 'Scaffold a NestJS API with Prisma ORM and Prisma Postgres, initialize the database, query it over HTTP, and deploy to Prisma Compute.'
---

## Introduction

This guide shows you how to use Prisma ORM in a NestJS API. You scaffold a project with a users controller backed by Prisma ORM, initialize the schema, query it over HTTP, and deploy the API to [Prisma Compute](/compute).
...
Answer the prompts for contract authoring style and package manager. The scaffold generates the NestJS app with Prisma ORM wired in, installs dependencies, and emits the contract your queries are type-checked against.
...
- `src/prisma.service.ts`: the injectable that exposes the Prisma ORM client and query helpers
- `src/prisma/users.ts`: the `listUsers` query the service runs
- `src/prisma/db.ts`: the Prisma ORM client itself
