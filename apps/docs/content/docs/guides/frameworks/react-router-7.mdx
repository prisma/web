---
title: React Router
description: 'Add Prisma ORM to a React Router app in framework mode with orm init, then build user and post pages with loaders and a form action.'
url: /guides/frameworks/react-router-7
metaTitle: How to use Prisma ORM with React Router
metaDescription: 'Add Prisma ORM and Prisma Postgres to a React Router framework-mode app: orm init, a typed contract, a seed script, loaders that read users and posts, and a form action that creates posts.'
---
...
Prisma ORM 8 is the current release. Prisma ORM 7 remains fully supported; the Prisma ORM 7 version of this guide is at [/guides/v7/frameworks/react-router-7](/guides/v7/frameworks/react-router-7).
...
3. Edit `src/prisma/contract.prisma` so `User` has `id`, `email`, `name`, `posts`, `createdAt`, `updatedAt` and `Post` has `id`, `title`, `content`, `published`, `author`, `authorId`, `createdAt`, `updatedAt`, following https://www.prisma.io/docs/guides/frameworks/react-router-7.md. Run `npx prisma contract emit` and `npx prisma db init`.
...
## 2. Add Prisma ORM to the project
