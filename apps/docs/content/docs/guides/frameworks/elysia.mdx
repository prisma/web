---
title: Elysia
description: Build an Elysia API on Prisma ORM with the elysia template and deploy it to Prisma Compute.
url: /guides/frameworks/elysia
metaTitle: How to use Prisma ORM with Elysia
metaDescription: 'Scaffold an Elysia API backed by Prisma ORM and Prisma Postgres, seed it, serve users over HTTP, and deploy it to Prisma Compute as a Bun app.'
---

## Introduction

In this guide, you scaffold an Elysia API backed by Prisma ORM, initialize and seed a PostgreSQL database, serve data over HTTP, and deploy the API to [Prisma Compute](/compute) as a Bun app. The `elysia` template generates the server, so most of the work is understanding the pieces.
...
The route handler is ordinary Elysia code calling an ordinary Prisma ORM query; there is no framework adapter in between. To add write routes, follow the same pattern as the [Hono guide's POST route](/guides/frameworks/hono#4-add-a-post-route); the query code is identical.
...
Run [`bunx prisma@latest init`](/cli/init) once to install the [Prisma ORM skills](/ai/tools/skills#available-skills-for-prisma-8) for your coding agent and keep them matching your installed packages. Prompts that map to this guide:
