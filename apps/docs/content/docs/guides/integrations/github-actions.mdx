...
metaDescription: 'Provision and manage Prisma Postgres databases per pull request with GitHub Actions and the Prisma CLI: create the database, run migrations and seed it in CI, and clean it up on close.'
---
...
Prisma ORM 8 is the current release. Prisma ORM 7 remains fully supported; the Prisma ORM 7 version of this guide is at [/guides/v7/integrations/github-actions](/guides/v7/integrations/github-actions).
...
```npm
npx prisma project create prisma-gha-preview
```
...
```npm
npx prisma postgres create pr-test --project proj_abc123def456 --json
```
...
```npm
npx prisma postgres list --project proj_abc123def456
```
...
```npm
npx prisma postgres delete db_abc123def456 --project proj_abc123def456 --confirm db_abc123def456
```
    ...
    steps:
      ...
      - name: Delete database
        run: |
          ...
          npx prisma postgres delete "$DB_ID" --project "$PRISMA_PROJECT_ID" --confirm "$DB_ID"
```

The cleanup job checks out the repository and installs dependencies too, so `npx prisma` runs the same CLI version as the provision job.
...
```npm
npx prisma migration ref set staging 20260910T1549_init
```
...
```yaml title=".github/workflows/migrate-staging.yml"
...
When the job fails with `MIGRATION.MARKER_MISMATCH`, [Drift](/orm/migrations/rollbacks-and-recovery#drift-when-the-database-isnt-where-migrations-left-it) says what to do. To see what the job would apply before you push, run `npx prisma migration status --to staging --db "$STAGING_DATABASE_URL"` from your machine; it changes nothing.
...
- "Add a `migrate-production.yml` workflow like the staging one in step 8 that runs on a published release, with a `production` ref and a `PRODUCTION_DATABASE_URL` secret."
