...
```bash
npx pgfence analyze prisma/migrations/**/migration.sql
```
...
```bash
npx pgfence analyze prisma/migrations/20240115_add_index/migration.sql
```
...
```bash
npx pgfence analyze --output json prisma/migrations/**/migration.sql
```
...
```bash
npx pgfence analyze --ci --max-risk medium prisma/migrations/**/migration.sql
```
  ...
  pgfence:
    ...
    steps:
      ...
      - name: Run pgfence analysis
        run: npx pgfence analyze --ci --max-risk medium prisma/migrations/**/migration.sql
```
...
```bash
npx pgfence analyze --stats-file pgfence-stats.json prisma/migrations/**/migration.sql
```

The stats file contains row counts and table sizes from `pg_stat_user_tables`. See the [pgfence documentation](https://github.com/flvmnt/pgfence) for details on generating this file.
