...
The commands and code on this page target Prisma ORM 7, which remains fully supported. To start a new Prisma ORM 8 project, see the [Prisma ORM 8 quickstart](/prisma-orm/quickstart/postgresql); to add Prisma ORM 8 to an existing app, see [Add to an existing project](/prisma-orm/add-to-existing-project/postgresql). A Prisma ORM 8 version of this guide is planned.
...
```npm
deno install npm:@prisma/client@7 npm:@prisma/adapter-pg npm:pg npm:prisma@prev npm:@types/pg
```
...
```npm
deno run -A npm:prisma@prev migrate dev --name init
deno run -A npm:prisma@prev generate
```
...
2. Under **Build configuration**, hit **Edit** and add `deno run -A npm:prisma@prev generate` to the build command
3. Click **Save**
...
- **Extend your data model** - Add relationships, validations, and indexes to your Prisma schema
