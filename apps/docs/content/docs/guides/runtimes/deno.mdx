---
title: Deno
description: Run a minimal Prisma ORM and PostgreSQL app on Deno.
url: /guides/runtimes/deno
metaTitle: How to use Prisma ORM with Deno
metaDescription: 'Scaffold a Prisma ORM project for Deno, initialize Prisma Postgres, and run the generated typed query end to end.'
---
...
- The generated ORM tasks currently use the Deno-compatible Prisma ORM ORM CLI entry point. The consolidated `npm:prisma@latest` CLI still loads Node-only credential storage under Deno, so use the generated tasks rather than invoking a CLI package directly.
...
- [Read the Prisma ORM overview](/orm) for contracts, typed queries, and migrations.
