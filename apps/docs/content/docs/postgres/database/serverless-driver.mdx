...
The Prisma Postgres serverless driver is currently in [Early Access](/orm/v7/more/releases#early-access) and not yet recommended for production scenarios.
...
- Automatic connection pooling across [all available Prisma Postgres regions](/postgres/faq#what-regions-is-prisma-postgres-available-in)
...
For standard Node.js environments, use the [`node-postgres` driver](/orm/v7/core-concepts/supported-databases/postgresql#using-driver-adapters) for lower latency with direct TCP connections.
...
The serverless driver automatically uses connection pooling [across all available Prisma Postgres regions](/postgres/faq#what-regions-is-prisma-postgres-available-in).

Connection pooling is enabled by default and requires no additional configuration.

Connections are reused regardless of your deployment region, which reduces connection overhead.
