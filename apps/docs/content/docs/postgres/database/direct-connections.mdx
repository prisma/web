...
Prisma Postgres supports both direct and pooled TCP connections, whether you connect to it via [Prisma ORM](/orm) or any other ORM, database library / tool of your choice. This page covers the direct, non-pooled connection string. For pooled TCP connections, see [Connection pooling](/postgres/database/connection-pooling).
...
1. Copy the direct connection string that is generated below.

![Animated walkthrough of generating a direct connection string in Prisma Console: navigating to the database, clicking Connect, generating a new connection string, and copying the result.](/img/postgres/connection-string.gif)
...
When using direct TCP to connect to a Prisma Postgres instance, every request is counted as a [billable operation](/postgres#billing-and-limits). Learn more on our [pricing page](https://www.prisma.io/pricing).
