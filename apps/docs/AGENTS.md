...
5. `pnpm lint:versions` after pinning a Prisma package version. Every pin in the current docs must be what a reader installs today: the package's `latest` on npm, and for `@prisma/cli-engine` the version `prisma@latest` depends on, which is what `orm init` installs. Write "since `8.0.0-rc.10`" or use an "Added in" column for history.
