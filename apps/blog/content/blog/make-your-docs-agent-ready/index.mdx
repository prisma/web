...
1. **Run the audit.** Put your docs through the [Agent Readiness Score](https://www.mintlify.com/score). It crawls the site and reports per-check results, so you start from concrete failures instead of a general sense that things could be better. If you would rather not create an account, [plym's Agent Readiness Score](https://plym.io/tools/agent-readiness) runs 23 checks from the Agent-Friendly Documentation spec with no signup, and [its report for the Prisma docs](https://plym.io/tools/agent-readiness/r/prisma.io-0df35e) is public if you want to see the output before running your own.
