...
Coding agents have gotten good at building software. What they can't do is provision the paid infrastructure that software needs: a database, a host, anything with a card behind it. That's the wall every agentic workflow hits: it can write the app, then it has to stop and wait for a human to go sign up, verify an email, and enter a credit card.
...
There are four plans. You can see them any time with `stripe projects catalog prisma`:
...
`status` shows what's provisioned and on which plan. `open` mints a short-lived link (five minutes, single use) that drops you straight into your workspace dashboard in the Prisma Console, already authenticated as the right account. If you're signed into the Console as someone else, it tells you instead of silently mixing accounts. You never _have_ to open the Console, but it's there when you want it.
