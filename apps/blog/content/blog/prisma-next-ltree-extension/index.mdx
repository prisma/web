...
This post is part of the Prisma Next series, a sibling to the [call for extension authors](https://www.prisma.io/blog/prisma-next-call-for-extension-authors).

For years, as a Prisma user, needing a type the ORM did not model usually meant I reached for whatever it already supported: occasional raw SQL, later typed SQL once that shipped, but mostly just the models Prisma made easy. If the ORM didn't model something, I usually didn't bother with it.
...
`=` and `IN` only test path equality. Descendant queries need Postgres `@>` / `<@`.

Hierarchies like this show up in category trees, file systems, org charts, and taxonomies. An org chart path like `company.engineering.platform` uses the same shape as `essays.drafts.chapter_1`, and [Prisma users have asked for native `ltree` support](https://github.com/prisma/prisma/issues/2568) since May 2020. Prisma never shipped a native `ltree` type, and that issue sat next to plenty of similar requests for other database features.

Once this project needed a real tree type, Prisma Next's extension system let me add `ltree` without waiting for it to land in core.

I shipped [`prisma-ltree`](https://github.com/slovakian/prisma-ltree): typed columns, operators, path validation, and the database setup to enable the extension. I leaned on an agent while reading the Prisma Next codebase; that made understanding the extension model, and then writing the pack, faster than digging through it by hand alone.
...
Prisma Next and `prisma-ltree` are both Early Access. Prisma Next is not production-ready yet: keep Prisma 7 for production apps. Community packs can still add Postgres types like `ltree` without waiting for every feature to land in core.

The operators above are what I actually use for this tree (`isDescendantOf`, `isAncestorOf`, `matchesLquery`, `lca`, plus the rest of the pack surface). Typed `ltree` columns and path validation are there too.
...
If you would rather poke at a taxonomy than at writing collections, the repo also contains an `examples/family-tree` app that seeds a Tree of Life and uses the same operators.
