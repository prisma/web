...
Two honesty notes. If your app genuinely fits Neon's 0.25 CU minimum around the clock, always-on compute drops to about $19/month and the comparison tightens; size that honestly against real production traffic. And the advantage inverts as query volume grows: past roughly 24M operations a month (against the $49 Pro plan), the always-on compute becomes the cheaper option again, as in example 2.
...
- **Idle periods with latency-sensitive users:** the shape operation billing is built for. Neon has to run always-on (about $77/month per CU) to avoid cold starts in front of users, while moderate query counts stay inside Prisma's plan allowances (example 4: $33 vs $78).
