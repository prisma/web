...
Prisma 8 treats pgvector as an **extension pack**: a package that teaches the whole stack about a new column type. `@prisma-next/extension-pgvector` ships three things:
...
- [`@prisma-next/extension-pgvector`](https://github.com/prisma/prisma-next) is a Prisma 8 extension pack: a codec (`Vector<4>` in your types), typed `cosineDistance`/`cosineSimilarity` operations, and a baseline migration that installs the extension for you.
