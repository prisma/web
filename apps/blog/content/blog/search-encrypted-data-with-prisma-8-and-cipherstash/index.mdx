...
Standard B-tree and GIN indexes work on those terms: in our [benchmarks][benchmarks], encrypted exact-equality and JSON-containment lookups run in 0.1–0.8 ms on tables of up to 10 million rows. Those are query-only medians, measured before decryption. Pattern-match and range queries are slower, and the report breaks them out per scenario.
...
Plaintext values cannot accidentally appear in a JSON response, application log, or AI prompt simply because an object was serialized: encrypted values serialize as opaque placeholders that contain no plaintext. Reaching plaintext requires an explicit decryption operation in application code.
...
CipherStash's Access Intelligence makes those cryptographic decisions observable, so that developers, security teams, and auditors can understand not only who accessed sensitive data but also why, when, and under what authority.
