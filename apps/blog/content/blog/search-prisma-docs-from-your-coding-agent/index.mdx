...
It comes with the Prisma MCP server you already use to spin up databases: no new server to register, no extra auth, no client-side config.

## A docs assistant that lives where you work

You usually hit the docs in the middle of something else: your agent is mid-task and you need one specific answer. Switching to a browser, searching, and coming back breaks the loop.

`search_prisma_documentation` closes it. Ask a natural-language question, get a cited answer assembled from Prisma's official docs, right where your agent is working. The tool is read-only, so it also works when your client runs in a restricted mode.
...
## One tool, the whole platform

The answers come from the full Prisma docs, so the same tool spans every product. Ask it about [Prisma ORM](https://www.prisma.io/orm) and Prisma Next, [Prisma Postgres](https://www.prisma.io/postgres), or [Prisma Compute](https://www.prisma.io/compute), and the answer comes back without leaving the editor.
