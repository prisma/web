...
excerpt: "How I built Open Chat, durable and resumable AI chat where the answer is safe the instant it exists, on Durable Streams, Prisma Streams, TanStack DB, and Prisma Compute."
...
Most chat apps stream tokens to the browser and save the finished message afterwards. That ordering is the bug. If the message only becomes real _after_ the stream ends, then anything that interrupts the stream loses data.
