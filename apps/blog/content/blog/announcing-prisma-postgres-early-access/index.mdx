...
We are excited to announce Prisma Postgres, **a managed PostgreSQL service that gives developers an _always-on_ database with _pay-as-you-go_ pricing for storage and queries** (no fixed cost, no cost for compute). It has the pay-as-you-go model of a serverless database, but avoids the cold-start delays that usually come with it, plus a generous free tier.
...
### Prisma Postgres: A "serverless" database with a generous free tier and minimal cold starts
