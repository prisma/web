...
It speaks standard Postgres on the wire, so `psql`, GUIs, and standard Postgres drivers connect to it. The trade-offs: the local server accepts one connection at a time, so a second client waits for the first to let go rather than failing outright, and it is a development tool, ideal for iteration and tests rather than something you self-host. The [local development docs](https://www.prisma.io/docs/postgres/database/local-development) cover the full instance-management reference.
...
The `url` line is the only change; keep whatever else your config already declares. That snippet is the Prisma ORM 7 setup, where connection config lives in `prisma.config.ts`. On Prisma ORM 6 the same job is done by `directUrl` in the schema's `datasource` block; that field is deprecated in 7, and its value belongs in the config's `url` as above. For local development, set `DIRECT_URL` to the same value as `DATABASE_URL`; the local server has no pooler, so the two are one URL.
...
**Starting fresh in production.** If your local data is disposable prototype data, replay the migration history against the hosted database and reseed. This assumes you have a `prisma/migrations` folder. If you prototyped with `npx prisma db push` and never created migrations, make one first: point `.env` back at your local database, run `npx prisma migrate dev --name init` there to turn the current schema into an initial migration, then point `.env` at production again before continuing. The deploy pipeline at the end of this post needs migration files to replay.
...
`migrate deploy` is the production counterpart of `migrate dev`: it applies the committed migration files in order, with no schema drafting, no prompts, and no drift detection. That makes it safe to put in CI or your deploy pipeline, where it belongs. The [development and production workflows docs](https://www.prisma.io/docs/orm/prisma-migrate/workflows/development-and-production) explain the split. If you have a seed script for production defaults, run it once now, while `.env` points at production.
...
The flow is the same whether the source is Docker Postgres or a local `prisma dev` instance; both accept direct `postgres://` connections, and `pg_dump` handles a `prisma dev` database like any other Postgres. Two caveats for the `prisma dev` case. It serves one connection at a time, so close anything holding that connection (your app, Prisma Studio) before dumping, or `pg_dump` will wait behind it; `prisma dev` itself stays running. And trim the printed connection string back to `?sslmode=disable`, because `pg_dump` rejects Prisma's own pool-tuning parameters with `invalid URI query parameter: "connection_limit"`.

Export from the local database. Use the compose URL for Docker, or the trimmed `prisma dev` URL, where a database named `template1` is normal, since that is the one `prisma dev` serves:
...
# prisma dev (port from `npx prisma dev ls`)
pg_dump -Fc -v -n public -f db_dump.bak \
  -d "postgres://postgres:postgres@localhost:51214/template1?sslmode=disable"
```
