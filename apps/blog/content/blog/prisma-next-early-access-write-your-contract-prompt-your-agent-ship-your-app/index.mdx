...
Imagine this as your data contract. If you're already using Prisma ORM, the syntax is the familiar PSL. Not every Prisma ORM feature carries over yet, but the day-to-day shape is the same:
...
The framework produces a real, reviewable [TypeScript migration file](https://www.prisma.io/blog/typescript-migrations-in-prisma-next) alongside an authoritative `ops.json`. The `migration.ts` is reviewable authoring sugar; the framework applies the operations from the JSON.
...
This writes `prisma-next.config.ts`, scaffolds a starter contract and `db.ts` under `prisma/` (pass `--schema-path src/prisma/contract.prisma` to put it under `src/prisma/`), installs the runtime, emits the contract, and registers the agent skills, all without touching your framework or build setup.
