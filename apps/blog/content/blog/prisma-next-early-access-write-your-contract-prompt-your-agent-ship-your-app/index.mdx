...
<AgentPrompt
  ...
  before={`const books = await db.orm.Book.find();`}
  after={`// !mark
...
<AgentPrompt
  ...
  language="prisma"
  fileName="contract.prisma"
  skill="@prisma-next/migration-skill"
  before={`model Book {
  id           String    @id @default(uuid())
  title        String
  author       Author    @relation(fields: [authorId], references: [id])
  ...
  terminalCommand="prisma-next migrate plan"
  terminalLines={[
    "→ Detected contract change: Book.publishedAt added (timestamptz, nullable)",
    "→ Planned 1 operation",
    "✓ Wrote migrations/20260515T0900_add_book_published_at/migration.ts",
  ]}
/>
...
<AgentPrompt
  ...
  terminalCommand="pnpm prisma-next upgrade"
  terminalLines={[
    "→ Reading recipe for prisma-next 0.10 → 0.11",
    "→ Applying codemod: rename .find() → .findFirst()",
...
The interactive scaffolder lets you pick a JavaScript framework (Next.js, Vite, Hono, and others) and wires Prisma Next in with your chosen database (PostgreSQL or MongoDB). You finish with a runnable app, a starter contract, and the agent skills already registered.
...
This writes `prisma-next.config.ts`, scaffolds a starter contract and `db.ts` under `src/prisma/`, installs the runtime, emits the contract, and registers the agent skills, all without touching your framework or build setup.
