...
### CipherStash encryption: async codec, partner shape

The CipherStash team built an encryption extension that uses async codecs to encrypt on write and decrypt on read, with key material managed by their service. It currently lives in their own repo as [PR #364](https://github.com/cipherstash/stack/pull/364). Extensions don't have to live in the Prisma Next monorepo, and partner-owned packages on the public SPI work the same way as in-tree ones. This is the natural template for codec-heavy extensions that need async I/O.
...
We, the Prisma Next team, are available on Discord and eager to help you build, as we have with ParadeDB, CipherStash, and Pothos already.
...
- **Shipping a codec that needs async I/O (encryption, lazy loading, or external services)?** Reference the [CipherStash extension](https://github.com/cipherstash/stack/pull/364)
