...
The numbers so far are aggregates. To see the full shape of the latency — and to check whether Prisma Compute pays any penalty reaching these stores — I ran a focused probe: `1,000` individual `10 KiB` reads and `1,000` writes against each store, issued once from a Prisma Compute machine in Frankfurt and again, as a baseline, from an AWS Lambda function. The targets were the Tigris global bucket, an R2 bucket in `EEUR`, AWS S3 in `eu-central-1`, and [S3 Express One Zone](https://aws.amazon.com/s3/storage-classes/express-one-zone/) — AWS' low-latency, single-AZ storage class.
...
First, for the three stores available in Frankfurt — Tigris, R2, and standard S3 — Prisma Compute and AWS Lambda behave almost identically. Each store's distribution is nearly the same whether the requests come from a Prisma Compute machine or an in-AWS Lambda function, and the paired ridges sit on top of each other (standard S3 reads, for instance, have a `32 ms` median from both). Reaching these stores from Prisma Compute is not paying a platform tax; the gaps are between the stores, not the compute.
...
Everything above is about latency — the time for one small operation. Bandwidth is a separate axis, and a virtual filesystem occasionally has to move a large blob, not just `10 KiB` files. So I ran one more probe across the same eight combinations: a single `50 MiB` object, one `PutObject` and one `GetObject` per trial, three trials each.

![Average bulk throughput in MiB/s for a 50 MiB upload and download, three trials each, across the eight runtime-and-store combinations. Upload bars extend left, download bars right. AWS S3 from Compute is fastest, about 91 up and 84 down; S3 Express downloads at about 90. Tigris from Compute is slowest at roughly 14 to 15. R2 is in the middle and downloads faster than it uploads. Tigris is markedly faster from Lambda than from Compute.](/evaluating-object-storage-providers-for-prisma-compute/imgs/bulk-throughput.svg)

On this axis the order from the latency tests inverts. AWS S3, built for exactly this, is the throughput champion — about `91 MiB/s` up and `84 MiB/s` down from Compute — with S3 Express close behind on download at `~90 MiB/s`. Tigris, which had the lowest per-operation latency, is the slowest at bulk transfer: roughly `15 MiB/s` from a Compute machine in Frankfurt. R2 sits in the middle and downloads noticeably faster than it uploads.

Two details stand out. First, Compute and Lambda diverge here, where for latency they were nearly identical. Tigris is about `3x` faster from Lambda than from Compute, while S3 is faster from Compute than from its own in-region Lambda. The likely cause is raw network bandwidth — an AWS Lambda's throughput scales with its memory size, while a Compute machine has its own pipe — so on this axis the result says as much about the client as the store.

Second, latency and bandwidth are decoupled. S3 Express still downloads at `~90 MiB/s` from Compute even though that path crosses from Frankfurt to Ireland and pays a heavy per-operation latency penalty. For a single large transfer you pay the round trip once and then stream, so the cross-region hop that dominated the small-object workload barely dents bulk throughput.

With only three trials per combo there is real noise — some were steady, like Compute to S3 at `87/95/92 MiB/s`, while others bounced, like Compute to Tigris at `16/16/10` — so read the averages below as rough.

| Runtime | Store | Upload MiB/s | Download MiB/s |
| --- | --- | --: | --: |
| Compute | Tigris | 14.1 | 15.3 |
| Compute | R2 | 20.4 | 52.8 |
| Compute | S3 | 91.4 | 84.3 |
| Compute | S3 Express | 68.8 | 90.5 |
| Lambda | Tigris | 39.7 | 46.7 |
| Lambda | R2 | 27.8 | 40.5 |
| Lambda | S3 | 61.0 | 73.4 |
| Lambda | S3 Express | 48.0 | 90.1 |

None of this changes the verdict for `accelerated-fs`: that workload is thousands of small operations, where latency rules and Tigris wins. Bulk transfer is simply a different regime — and one where, if it were our main workload, S3 would be the obvious pick.

## What this means for Prisma Compute

This is not a universal object storage benchmark. It does not say that every app should pick Tigris over R2. If your workload is mostly bulk upload/download — where S3 pulled well ahead above — or if your application already lives inside Cloudflare's network, your results may look different.
