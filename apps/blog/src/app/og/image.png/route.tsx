...
/* Prism palette, inlined because satori resolves no CSS custom properties.
   Each value is the light-mode eclipse token named beside it. */
const INK = "#151515"; // --color-foreground-neutral-strong
const INK_MUTED = "#646567"; // --color-foreground-neutral-weak
const PAPER = "#f9faf5"; // --color-paper
const WHITE = "#ffffff"; // --color-background-default
const CYAN = "#009aaa"; // --color-prism-cyan-600
const CYAN_WASH = "#eeffff"; // --color-background-ppg

/* The six spectrum stops, cyan wrapping back to cyan — the same sequence as
   `--gradient-spectrum` in @prisma/eclipse. */
const SPECTRUM = "linear-gradient(90deg, #01d7e4, #f3c306, #f37a03, #f43531, #f00e5c, #01d7e4)";
...
// Sora Medium is the brand display face; Inter carries body copy. Both are
// fetched from Google here rather than from the woff2 files vendored in
// @prisma/eclipse, because satori (behind `next/og`) cannot parse woff2.
const FONT_DEFINITIONS = [
  { name: "Sora", family: "Sora", weight: 500 },
  ...
  return (
    <div
      style={{
        ...
        background: `linear-gradient(to bottom, ${WHITE}, ${PAPER})`,
        padding: 60,
        position: "relative",
      }}
    >
      {/* The spectrum hairline — the brand's interaction signature, held
          still. It is the only saturated element at full width. */}
      <div
        style={{
          position: "absolute",
          top: 0,
      ...
      <div
        style={{
          ...
          justifyContent: "center",
          width: "auto",
          backgroundColor: CYAN_WASH,
          color: CYAN,
          border: `${BADGE_BORDER_WIDTH}px solid ${CYAN}`,
      ...
      >
        <div style={{ display: "flex", width: 28, height: 2, background: CYAN }} />
        prisma.io/blog
      </div>
