...
- `.aggregate()` and `.groupBy()` now respect `limit`, `offset`, `cursor`, and `distinct` set earlier in the chain. Those clauses were silently dropped before, so affected queries now return a *different, correct* number. ([#30067](https://github.com/prisma/orm/pull/30067), [#30092](https://github.com/prisma/orm/pull/30092))
