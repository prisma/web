...
import { CodeBlock, CodeBlockTabs, CodeBlockTabsList, CodeBlockTabsTrigger, CodeBlockTab, Pre } from "@prisma/eclipse";
...
- `onValueChange` - Tab change callback ((value: string) => void, optional). Receives the normalised tab value (see the note under `CodeBlockTab`), not the original label
...
**Note:** Tab values become DOM ids, so `value` on `CodeBlockTabs`, `CodeBlockTabsTrigger` and `CodeBlockTab` is normalised to an id-safe token (lowercased, whitespace and punctuation replaced, a short hash appended when that changes the string). Human labels such as `"Relational databases"` are fine as values and are matched after normalisation; the visible trigger text is whatever you render as `children`.
