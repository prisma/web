...
**Note:** Tab values become DOM ids (Radix derives `id`, `aria-controls` and `aria-labelledby` from them), so every `value` is normalised to an id-safe token before it reaches Radix: lowercased, whitespace and punctuation replaced, and, when that changes the string, a short hash of the original appended. Values that are already id-safe (`"npm"`, `"account"`) pass through untouched. You can keep passing human labels such as `"Relational databases"` to `value`, `defaultValue`, `TabsTrigger` and `TabsContent`; they are matched after normalisation. `onValueChange` receives the normalised token, not the original label, so compare against `escapeTabValue(label)` (exported from `@prisma/eclipse`) rather than the label itself.
