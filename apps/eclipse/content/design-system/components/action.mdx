...
- `asChild` - Render the action's styling onto its single child element instead of a `<div>` (boolean, default: false). Use it whenever the action is interactive, e.g. `<Action asChild><button type="button" aria-label="Copy">…</button></Action>`; a click handler on a `<div>` gives a control with no role, no place in the tab order and no accessible name
