  ...
  return (
    <div className="flex min-w-0 shrink-0 grow-0 basis-full flex-col px-5 pb-6 pt-7 sm:px-9 sm:pb-8 sm:pt-9">
        ...
        <div className="rounded-xl border border-border/70 bg-muted/40 p-6 sm:p-7">
          ...
          <ul className="mt-6 flex flex-col gap-3">
            {scenario.typical.map((item) => (
              ...
              >
                <X className="mt-0.5 size-3.5 shrink-0 text-foreground/35" strokeWidth={3} aria-hidden />
                {item}
              </li>
            ))}
          </ul>
          ...
          <div className="relative flex h-full flex-col rounded-xl border border-border bg-card p-6 shadow-[0_1px_2px_rgba(21,21,21,0.04),0_12px_24px_-12px_rgba(21,21,21,0.12)] sm:p-7">
            ...
            <ul className="mt-6 flex flex-col gap-3">
              {scenario.prisma.map((item) => (
                ...
                >
                  <CheckBold
                    className="mt-0.5 size-3.5 shrink-0 text-prism-cyan-500"
                    aria-hidden
                  />
                  {item}
                </li>
      ...
      <div className="mt-7 flex flex-col gap-2 border-t border-border/70 pt-5 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
        ...
        <Button asChild variant="ghost" className="spectrum-ink-text -ml-3.5 shrink-0 self-start sm:-mr-3.5 sm:ml-0">
          ...
          <div>
            ...
            <Reveal delay={0.08}>
              <p className="mt-5 max-w-[54ch] text-pretty leading-relaxed text-muted-foreground">
                Start free with the ORM, which will always be free. Prisma Postgres has a
                free tier with a hard cap, so you can build without a credit card. When
                you outgrow it, you&apos;re on operation-based pricing with spend limits,
                and data transfer is always included.
              </p>
            </Reveal>
