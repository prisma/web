...
type VersusFrontmatter = {
  ...
  competitorA: string
  competitorB: string
  features: { name: string; a: boolean; b: boolean }[]
}
...
export async function generateStaticParams() {
  const slugs = getContentSlugs("versus")
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  try {
    const entry = getContentBySlug<VersusFrontmatter>("versus", slug)
    if (!entry) return {}
    const url = `${siteConfig.url}/versus/${slug}`
    return {
        ...
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <header className="mb-12">
            ...
            <h1 className="text-4xl font-bold tracking-tight sm:text-5xl">
              {frontmatter.competitorA} vs {frontmatter.competitorB}
            </h1>
              ...
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">
                    {frontmatter.competitorA}
                  </TableHead>
                  <TableHead className="text-center">
                    {frontmatter.competitorB}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
