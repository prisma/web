...
for f in "$@"; do
  hits=$(awk '
    ...
    /^url: / { next }
    { print NR": "$0 }
  ' "$f" | sed -E 's/`[^`]*`//g; s/\]\([^)]*\)/]/g; s/\[#[^]]*\]//g; s/href="[^"]*"//g' | grep -E -i "$pat" || true)
